...
- **Detalle por póliza** ("Pólizas sin contraparte"): pinpointea QUÉ pólizas de la cuenta 515 componen el descuadre. Endpoint `GET transfers-ledger-detail` sobre `analytics.gl_poliza_lines` (partida doble por póliza, ADR-041) — parea entrada 515-001 ↔ salida 515-002 por importe exacto; lo no-pareado se lista con su **referencia** (localizador en Kepler, ej. "TRAS DE CEDIS A ZAMORA T-7904"). `unpaired_entrada − unpaired_salida = Δ` (verificado jun/jul). Sin importer/migración (`gl_poliza_lines` ya poblada, may–jul).

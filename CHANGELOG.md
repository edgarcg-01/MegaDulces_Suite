...
### Added — panel visual de administración de usuarios del tablero (CV.18, 2026-09-03)
- Nueva página `apps/catalogo-kp/public/usuarios.html`: tabla de cuentas con alta, edición de rol/sucursales/estado, restablecer contraseña y "Eliminar acceso" (desactiva, no borra — reversible con un clic). Reemplaza la necesidad de la herramienta de línea de comandos para el uso diario; ésta queda como respaldo sin navegador.
- Verificación end-to-end en producción real reveló que el rol de base dedicado tampoco tenía permiso de escritura para crear cuentas (mismo tipo de agujero que el incidente de abajo, encontrado y corregido antes de anunciar la pantalla como lista) — corregido en el cluster y en el script de rol versionado.
- Enlace agregado en el catálogo interno. Detalle en [`FASE_CV`](docs/IMPLEMENTACION/FASES/FASE_CV_CATALOGO_TIENDA_MAYOREO.md).
- Agregado borrado definitivo (a pedido, mismo día): sólo sobre una cuenta ya desactivada, con confirmación tecleando el correo exacto. Necesitó un tercer permiso de base de datos que al rol dedicado tampoco le habían dado — corregido y verificado en producción con una cuenta desechable antes de avisar.

### Changed — "Actualizar Wix" ahora vive dentro del catálogo, con selector de sucursal (CV.19, 2026-09-03)
- Se movió de una página aparte a una segunda pestaña dentro del catálogo interno, compartiendo la sesión y la lista de sucursales que el catálogo ya tenía cargada.
- Se agregó lo que faltaba: elegir de qué sucursal sale el precio publicado a Wix (por default la que surte la tienda en línea). Al construirlo se encontró que el precio nunca se filtraba por sucursal — para los códigos cuyo precio sí varía entre plazas, el resultado dependía del orden en que la base devolviera las filas, no de una sucursal elegida a propósito. Corregido con el mismo criterio que ya se usaba en el verificador de precios de mostrador.

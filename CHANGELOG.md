...
**Contraseñas de las 6 altas:** el script genera una distinta por tienda, aleatoria y sin caracteres ambiguos porque se teclean en un kiosco; no se imprimen ni entran a git, y las verifica con **prueba negativa** — el hash acepta la generada y rechaza otra. `must_change_password = false` con motivo: es credencial de puesto, y forzar el cambio deja al resto del turno afuera. ⚠️ **Por decisión del lead (2026-09-09) las 6 quedaron en `123456`**, con el riesgo declarado antes de aplicar y reafirmado: **un token válido no es lo mismo que un permiso**, porque el guard global es no-op en una ruta sin `@RequirePermissions`. Las 2 en uso no se tocaron (alcance derivado de `last_login_at IS NULL`).

...
- **Verificado:** builds api+view verdes (exit code real, sin pipe) · el contexto de tenant sobrevive al trabajo detached (ALS probado con control negativo; `TenantKnexService.run()` abre su propia transacción y no depende del COMMIT del interceptor) · smoke nuevo `database/tests/http-finance-jobs-test.js` en la regresión, **pendiente de correr hasta el próximo restart de la API**.

...
- **Error tracking (Sentry)** wireado en backend y frontend, **inerte sin DSN** (seguro en dev/local). Backend: `apps/api/src/instrument.ts` (`Sentry.init` gated por `SENTRY_DSN`, cargado 1º en `main.ts`) + `SentryModule.forRoot()` en AppModule (registra el filtro global de excepciones). Frontend: `Sentry.init` gated por `environment.sentryDsn` + `ErrorHandler` de Sentry en `app.config.ts`. **Builds api+view verdes** (prod, sin caché). `@sentry/nestjs`+`@sentry/angular` v10.68. **DSN frontend wireado** (2026-07-24, proyecto `node-nestjs` reusado; DSN público en `environment.sentryDsn` + allowlist en `.gitleaks.toml`). **Pendiente:** setear `SENTRY_DSN` en Railway env (backend) + opcional proyecto Angular dedicado para separar streams.

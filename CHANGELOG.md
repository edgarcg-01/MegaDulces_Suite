...
### Added — Decode de `kdpv_prod_util` + dry-run del mayoreo real (2026-08-21)
- **Hallazgo medido en prod: hoy no hay descuento por volumen.** En **10,019 de 10,024** productos el precio más barato ya aplica desde **1 pieza** — `MAYOREO`/`BASE-MXN`/`P1` traen `min_qty=1` con el precio igual o más bajo que los tiers, así que los mínimos de `P2/P3/P4` (mediana 3/5/12, alcanzables) nunca ganan. Los 5 "descuentos" restantes son fracciones de centavo ($14.2010 → $14.1980). El motor por cantidad (FIQ.3) está bien; es la data la que deja la escalera plana.
- **`kepler_ods.kdpv_prod_util` decodificado**: `c1`=SKU · `c2`=**presentación** · `c3`=tier 0-3 · `c4`=mínimo · `c5`=**máximo** del rango · `c7`=precio. 7 sucursales, ~42k filas cada una, 9,492 SKUs. Tramos monótonos (23,140/23,194 pares bajan o quedan igual; 54 suben). El mayoreo varía por sucursal en 1,576 de 42,416 combos (12.33% de diferencia promedio).
- **`import-volume-tiers.js` (DRY-RUN, `--apply` bloqueado)**: moda retail 01-06 con CEDIS como fallback (misma regla que `repoint-catalog-prices`) y **solo la presentación que es la unidad de venta** del producto. Rinde **1,810 productos / 3,526 tiers** con escalera real (arrancan en 3/6/10). Diff vs lo que se cobra hoy: a qty 10, **1,090 bajan · 359 suben · 356 igual**; +39.5% promedio simple pero **−1.4% ponderado por rotación** — o sea los aumentos caen en SKUs que no se mueven.
- **Por qué `--apply` está bloqueado:** `c4` vale 3/6/10 igual para PZA, PAQ y CJA → el mínimo está en unidades de *esa* presentación, y `order_lines.quantity` está en unidad de venta (PZA en 9,094 de 11,127 productos). Cargarlo crudo haría que pedir 3 piezas active el precio de 3 cajas. La conversión tampoco se puede derivar: de 1,087 pares con precio en PZA y en PAQ/CJA, solo **280 (26%)** cumplen `precio_PAQ ≈ precio_PZA × factor_sale` (ratio promedio 3.62, no el factor). Los 7,682 SKUs que solo cotizan en PAQ/CJA quedan reportados como hueco, no adivinados.

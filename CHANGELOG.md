...
- Confirmado con el equipo (2026-09-08, "todo es por Railway"): el script apunta a `https://trademarketing-production-5084.up.railway.app` — misma URL que ya usa `apps/vendor` para llegar al backend desde un WebView nativo.
- **Modelo elegido: centralizado, con opción local por sucursal.** `$destinos` (sucursal → ruta de red) copia el archivo recién generado al PC de mostrador cuando está configurado; sin entrada ahí, esa sucursal corre el mismo script localmente. `$destinos` queda vacío por ahora — no se inventaron rutas de red reales.
- **`/api/salud` recuperado y portado a `apps/api`** (`src/modules/salud/`) — se había perdido en la absorción de `catalogo-kp` pese a que `apps/api` "ya tiene `db-health`": son cosas distintas. `db-health` es un reporte de frescura gateado por permiso, para un humano en el panel de admin; `/api/health` es el healthcheck de DEPLOY de Railway, deliberadamente sin tocar la base (para que un blip de red no tumbe un deploy). Ninguno de los dos sirve como vigilante público sin sesión que distinga "la base rechaza las credenciales" (no reiniciar, hace falta una persona) de "la base no responde" (red/servidor caído) — la distinción que ya costó 47 reinicios inútiles en 9 horas cuando no existía. Pool propio (`pg.Pool`, `max:1`) deliberadamente aislado de `KNEX_NEW_DB`, para que un atasco de la base no se lleve el chequeo por delante.
- **Bug real encontrado y corregido antes de mergear:** el port inicial de `/api/salud` copió `DATABASE_URL_NEW` (rol `postgres`, solo migraciones) del app standalone, donde esa SÍ era la connection string de runtime. En `apps/api` la conexión real de runtime es `DATABASE_URL_NEW_RUNTIME` (rol `app_runtime`, la que usa `KNEX_NEW_DB`/`KpService` de verdad) — con la variable equivocada, `/api/salud` habría reportado "ok" con el rol de migraciones vivo aunque `app_runtime` estuviera caído, el mismo tipo de falso positivo que esta ruta existe para evitar. Encontrado revisando el código, no en producción.
- **Validado end-to-end contra datos reales** (`192.168.0.245/platform_test`, rol `dev_sistemas`): API completa arrancada localmente, los 4 endpoints responden real (`/api/salud` → `ok`, `/api/sucursales` → 7 plazas, `/api/kp/precio` y `/api/kp/precios-todos`), y el generador corrió de punta a punta contra ese servidor: **7 de 7 sucursales generadas**, ~9,485 productos c/u, sello y guardas de encoding/estructura verificados en el archivo real.
- **Pendiente:** llenar `$destinos` sucursal por sucursal; validar específicamente con el rol `app_runtime` (se probó con `dev_sistemas` por no tener esa password a mano).

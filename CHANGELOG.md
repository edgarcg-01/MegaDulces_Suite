...
- Nuevo `svc.detail()` (consume el endpoint `GET /finance/supplier-payments/:suc/:folio?doc_prefix=` que ya existía). Frontend-only; build view verde. **Aplica solo a `/finanzas/pagos-comprobantes`** (pendiente replicar a Cobranza y Órdenes de entrada si se desea).

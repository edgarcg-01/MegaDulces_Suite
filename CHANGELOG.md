...
### Fixed — pérdidas de trabajo y color que no existía en tema oscuro (2026-08-28)
- **Salir de *Pendientes* tiraba la bandeja sin preguntar**: cada hoja ya costó una llamada de visión y un lote de CEDIS son treinta. Y *Ajustes* avisaba "hay cambios sin guardar" y después te dejaba salir en silencio. Los dos con guard de navegación + aviso del navegador; el guard ya existía en el repo y no se estaba usando acá.
- **`--danger-fg` no existe**: tres reglas de *Órdenes* lo usaban con un rojo literal de respaldo, así que el color quedaba fijo en los dos temas. Junto con otros 13 hex y dos fondos `#00000010` que **desaparecen sobre fondo oscuro** justo donde tienen que contrastar con el papel blanco.
- **El link "ver todo" mentía**: el Centro de control lo enlazaba con la sucursal filtrada y *Órdenes* **ignoraba el parámetro** — caías en las primeras 300 de la red entera. También paginaba 150 filas en memoria mientras el servidor mandaba 300 y el indicador contaba miles: ahora es paginación de servidor con el total a la vista.
- **La fecha de arranque se corría un día**: el campo nativo parseaba `2026-08-01` como medianoche UTC y en México eso es el 31 de julio.

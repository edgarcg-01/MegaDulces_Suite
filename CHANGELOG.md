...
- **Pendiente de prod:** 2 migraciones a Railway + el script `--apply` por sucursal + redeploy del api. Las migraciones se aplicaron en local **una por una** (`knex migrate:up <archivo>`), NO con `migrate:latest`: hay una pendiente de otra sesión en el árbol.

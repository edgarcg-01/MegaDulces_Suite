...
**Lo que esto destraba en RD**: §2.4a de la fase declaraba $1.77M de Canindo *"sin ninguna fuente diaria"* y culpaba al decode de `c67`. La pregunta del `c67` sigue **sin contestar y ya no bloquea**: el push trae la venta a nivel línea y día sin pasar por la réplica del branch.

**Cierre el mismo día**: se dieron de alta las **5** vans de Canindo — la flota completa quedó en **11 camionetas** (6 PH + 5 Canindo). `502` y `505` entraron **solas** en la corrida de `\Kepler\Intraday` de las 10:15: la regla de "ruta nueva" las detectó y trajo su historia completa desde el 11-ago sin intervención. Es la prueba negativa del arreglo — antes de él, esas dos vans habrían arrancado su ventana el día de la corrida y su agosto se habría perdido en silencio. Cobertura de agosto contra las mismas celdas del Excel: **16.9% → 84.7%**; la ruta 504, de **0%** a 82.7%. Hueco runner-vs-plataforma: **$0** en las 11.

Lo que **no** cierra y queda declarado: **el residuo del ~15% es sistemático, no un hueco por ruta.** Con tres rutas podía ser coincidencia; con las cinco cayendo en una banda de tres puntos (**82.7% – 85.7%**) es estructural — el `CONCENTRADO` captura consistentemente más que nuestro SUBTOTAL. Refuerza, no resuelve, la duda de §2.3. Y el traslape total quedó en **3 días-ruta y $18**: siempre una sola línea de $6, la firma del primer disparo del Kepler local, presente en las 5 vans.
...
2. **Editar un importer despliega a prod dentro de la hora, no de un día para otro.** Este feed está en **los dos** grupos de `run-prod-feeds.js` —`intraday` (línea 107) y `nightly` (línea 141)— y la tarea `\Kepler\Intraday` corre **cada hora**. La corrida de las **19:15 MX** levantó esta edición y sanó ella misma las 12,210 líneas antes de que yo corriera el `--apply`, que ya sólo hizo el incremental. El antes/después hay que capturarlo **antes** de guardar el archivo: la ventana es de minutos.
...
**En la plataforma** — `fix([RD.1])`: `wincaja.fecha_mx_date()` corría **todas** las fechas Wincaja un día hacia atrás. 100% de las filas, 21 sucursales, **542,684 documentos de 2026**. Lo causó RS.12b, una migración de *performance* que declaró en su propio comentario "business_date NO cambia" sin medirlo: el `TimeZone` del Postgres de prod es `Etc/UTC` y la vista se evalúa en la DB, no en el contenedor de la app. En la frontera de mes, **$793,080** de venta de ruta caían en el mes equivocado. Tres árbitros independientes coincidieron con la fecha cruda: el workbook (el SUBTOTAL casa 98.0% contra ella y **0.0%** contra la corrida), el día de la semana (con el bug las rutas trabajaban **domingo** —60,439 líneas— y descansaban **sábado** —105—) y la mecánica de Access, que guarda la fecha sin hora.

**En el Excel** — diez defectos que movían dinero, corregidos al migrar y cada uno con su prueba negativa: el tabulador cerraba con `IF(venta<400000,"5%")` sin rama `else`, así que una venta de $400,000 pagaba **comisión cero**; las rutas 22 y 23 usaban un umbral de `169,999.99` que el resto no; la 322 se quedaba sin factor de supervisor y su chofer cobraba el **100%**; `NOMINA BANCO` tenía **seis valores** para el mismo concepto repartidos entre hojas, así que el neto dependía de cuál se imprimiera; `OPERACION!K5` daba **$/km = 1** porque su `SUMIF` apuntaba a la columna PERIODO de su propia hoja, y de ahí colgaban `COSTO POR KM` y `% DE RENTABILIDAD`; y el total de gasto (`Z7`) sumaba bloques rotulados con rutas **24, 25, 300 y 301** que no existen en los datos, subdeclarando el combustible **$332,000**.

**Lo entregado**: `analytics.v_rd_route_daily`, venta por ruta×día declarando su procedencia — la columna `importe` mezclaba el neto de Wincaja con el bruto del push · motor de comisiones con las reglas **como filas y no como `if`s**, que reproduce **163 de 163** celdas al centavo cuando se lo alimenta con el input del Excel · `logistics.route_expenses` con las **782 filas / $848,610.04 / 34,718.24 lts** del gasto de flota, cuadradas contra la columna cruda de la hoja · `analytics.route_cost_snapshot`, porque el costo de Wincaja **se re-expresa en cada corrida** y el margen de un mes cerrado cambia solo cada noche · `logistics.route_odometer` + `analytics.v_route_operation_period` con el **$/km real de 6.12–9.13** · `scope='route'` en `commercial.sales_targets` · y la pantalla `/comercial/comisiones`.

**Lo que se declara en vez de dibujarse como cero**: $2.78M del Excel sin fuente diaria — Canindo desde el cutover del 15-ago, y **no es un problema de decode**: de 6,194 documentos, **uno solo** usa el vendedor de ruta y el resto dice `SUCURSAL CANINDO PISO`; más Morelia 321/322, cuyos `.mdb` dejaron de copiarse el **02-jul**, exactamente donde se corta el dato. Las 15 lecturas de odómetro con dígitos mal tecleados se rotulan y **no** se corrigen: poner el dígito que falta sería inventar la lectura. Las 5 filas de gasto sin tipo entran como `SIN CLASIFICAR` en vez de adivinarse por su descripción. Y el resolvedor de costo devuelve **las dos** cifras con su brecha, sin elegir. **10 dudas abiertas** en §9 de [`FASE_RD`](docs/IMPLEMENTACION/FASES/FASE_RD_INDICADORES_RUTA.md).

8 migraciones (sólo en `.245`; prod pendiente), 3 candados nuevos en la suite con 56 aserciones, builds api + view verdes.
...
- Lección nueva en [`docs/GOTCHAS.md` §39](docs/GOTCHAS.md): correr un importer **a mano** mientras existe su tarea programada = te lo matan a los 13 min (`kill-stale-feeds.ps1`), y el síntoma es un `FATAL 57P01` de Postgres que parece de Railway. El backfill se hizo en **escalera** (30 → 90 → 180 → 260 d) para que ninguna corrida cruce ese techo. *(Decía §38 y ese número se lo llevó el incidente del lock de CH.1 — peor: la lección **nunca se había escrito** en GOTCHAS, vivía sólo en la memoria de una sesión. Se escribió el 2026-09-09 como §39.)*

### Added — una cuenta por checador, con un token que no expira cada mañana (CH.1, 2026-09-09)

Un kiosco es una pantalla que se prende una vez y se queda prendida. Con el TTL global de 12 h alguien tiene que ir a teclear la contraseña cada mañana, y el día que nadie va, la pantalla muestra el login en vez de su trabajo. Ahora la vida del token se declara **por cuenta** (`identity.users.token_ttl_days`, mig `20260909130000`): `NULL` = el default global, 365 = un año. **No se subió `JWT_EXPIRES_IN`** a propósito — eso le alargaría el token a todos, incluidos los admin, que es justo donde un token largo duele más.

**Lo que hace defendible la vida larga ya existía, y se midió antes de escribir código:** `[AUTHZ-HARD.2]` relee `identity.users` en **cada** request (cache 30 s) → `activo = false` mata el token en ≤30 s sin esperar su `exp`; y `PermissionsCacheService` relee los permisos de DB por request, así que el mapa del JWT sólo gatea la UI y un token viejo **no** conserva privilegios viejos del lado del servidor. Eso es también la razón de que vaya **una cuenta por dispositivo**: revocar es por cuenta, y apagar un kiosco comprometido no puede implicar apagar los otros ocho. Lo que NO resuelve, dicho de frente: un token filtrado sirve hasta que alguien desactiva esa cuenta — no hay revocación por token ni rotación.

- **Rol `checador_kiosco`** (mig `20260909131000`) con UNA clave (`HR_ATTENDANCE_CHECAR`) y alcance recortado a 6 dimensiones calcando `etiquetas_anaquel`. Permiso **restrictivo**: no entra a `role-presets` ni se reparte a ningún rol existente.
- **`tokenSignOptions()` en `libs/platform-core/.../token-ttl.ts`**, no inline en el servicio (ADR-056: el primitivo lo va a querer cualquier kiosco). Documenta la trampa que evita: devuelve `{}` y **no** `{ expiresIn: undefined }` — Nest mergea `{...signOptions, ...options}`, así que la clave presente en `undefined` **borra** la expiración del merge y emitiría un token eterno **para todo el mundo**. Y en segundos, porque `expiresIn` como string exige un literal de tipo `ms`.
- **`provision-checadores.js`** calcando el de etiqueteras: cuentas `checador.NN` **derivadas** de `commercial.warehouses` con zona (no una lista a mano), dry-run con ROLLBACK, `--apply` sólo contra prod, contraseñas sin caracteres ambiguos a un archivo fuera del repo, y 4 gates — entre ellos que **las cuentas quedaron con el TTL pedido**, que es lo único que el script existe para hacer y lo que reportaría éxito igual si el INSERT lo perdiera.
- **Candado `test-newdb-device-token-ttl.js` — 19/19, sin API**: carga los `.ts` reales vía ts-node y firma reproduciendo el merge de `JwtModule`. Las negativas son el punto: la cuenta normal **sigue en 12 h** (si alguien subiera el TTL global, el caso positivo pasaría igual), ningún TTL inválido emite un token **sin `exp`**, la DB rechaza el 0, y **desactivar la cuenta la rebota igual con token de 365 d** sin tocar a la vecina.
- ⚠️ **Estas cuentas todavía no tienen a dónde ir:** la pantalla del checador es `[CH.0.10]` y no existe. Por eso la clave entra al `AUTHZ_TREE` **sin `route`** — visible y quitable en `/admin/roles` (un permiso fuera del árbol no tiene casilla: nadie lo ve ni lo revoca), pero los 403/404 no lo ofrecen como salida navegable.
- ⚠️ **Hallazgo de inventario:** los checadores **no están en la DB de la app**. Viven en una base dedicada **`hr` en `.245`** (`DATABASE_URL_HR`) con **11 equipos y 159,041 checadas**; en `platform_test` y en **prod** el schema `hr.*` está **vacío** (0 y 0). Y `label`/`site_code` siguen NULL en 10 de 11 (`[CH.0.7]`) → la correspondencia kiosco ↔ reloj no se puede establecer sin adivinar, así que el script provisiona por SITIO y lo declara en pantalla.
- ⚠️ **Incidente al aplicar a prod, y la lección vale más que la feature.** La migración de la columna se aplicó **sin `lock_timeout`**, descartado porque `identity.users` tiene ~150 filas y `ADD COLUMN` nullable es metadata-only. **El tamaño nunca fue el riesgo:** el `ALTER` pide ACCESS EXCLUSIVE, quedó esperando, y en Postgres **una petición de lock que espera encola detrás de sí a todo el que llegue después** — como el guard lee `identity.users` en cada request, **el login entero se apiló detrás por ~7 minutos**. Se salió cancelando la propia sentencia (`pg_cancel_backend` sobre el pid del ALTER); prod quedó intacta (columna 0, sin fila en `knex_migrations`, sin constraint huérfano, lock de migraciones libre). Fix: `SET LOCAL lock_timeout = '3s'` dentro de las dos migraciones, con prueba negativa medida (**3025 ms, `55P03`**). [`GOTCHAS.md` §38](docs/GOTCHAS.md).
- ⚠️ **Y el bloqueador no era el feed: era `pg_dump`.** La query visible era un `COPY` del ODS, pero `application_name` + `pg_locks` dicen que el dueño del lock era el **respaldo diario** (`TradeMarketing-DailyBackup`, 17:00), que toma `AccessShareLock` sobre **todas** las tablas hasta terminar. Desde que el 2026-09-08 se corrigió ese script para apuntar de verdad a prod, dumpea **15.9 GB** → **mientras corre el respaldo, prod es una ventana sin DDL**. Nadie lo había visto porque hasta anteayer respaldaba la base equivocada.
- **Estado de prod:** el rol quedó **aplicado** (batch 341, en los 2 tenants — sólo hace INSERTs y `RowExclusive` no choca con el respaldo) y la **columna también** (batch 346, aplicada una vez que cerró la ventana del respaldo; 126 usuarios, 0 con TTL propio).

### Changed — el TTL se administra desde la app, y se retiró lo que vivía afuera (CH.1.7–CH.1.12, 2026-09-09)

Revisión de la propia entrega de CH.1 contra los lineamientos. El hallazgo no fue un bug: fue **dónde había quedado parado cada pedazo.** Medido: `token_ttl_days` aparecía en 9 archivos —el primitivo, la firma del login, la migración, el script de alta, el smoke y tres documentos— y en **cero** líneas de `libs/trade/` (la capa que administra usuarios) y **cero** de `apps/view/`. El hueco no era parcial, era total: la columna existía y la aplicación no sabía nombrarla. Consecuencia concreta: había una cuenta en prod con un token de 365 días y **ninguna forma de verla ni de quitarle el TTL desde la app**.

- **Retirado `database/scripts/provision-checadores.js`** y borrada la cuenta `checador.03` de prod. El script se justificaba con *"un hash de contraseña no va en un archivo versionado"* — eso argumenta contra una **migración**, no contra la UI, que ya le pide la contraseña al admin. Se defendió contra la alternativa equivocada. La causa raíz de que el alta terminara afuera era un hardcode: `must_change_password: true` clavado en el `insert`, que hacía imposible dar de alta un kiosco por el endpoint.
- **La cuenta se borró con DELETE duro, y por una razón medida:** `users_tenant_username_unique` es unique **plano** sobre `(tenant_id, username)` y el `create` no filtra por `deleted_at` → el soft-delete de la app **quema el username** y `/admin/users` nunca podría volver a crear `checador.03`. Antes de borrar se midió (no se supuso): `last_login_at` null, **195/195 triggers encendidos** (`pg_trigger.tgenabled`, no `convalidated`, que miente), las 5 FKs `RESTRICT` en 0, y un solo hijo en `user_roles` que puso `trg_sync_primary_role`. `COMMIT` condicionado a `rowCount == 1`.
- **`/admin/users` da de alta, ve, audita y revoca la sesión larga.** Interruptor Persona/Dispositivo que **sólo aparece en el alta** (editar a una persona no ofrece convertirla: la invariante es una cuenta por dispositivo); presets 12 h / 30 d / 180 d / 1 año en vez de un campo numérico libre —la única razón para tipear un número es tipear el equivocado, y así el techo de 3650 no se duplica en el front—; contraseña generable con el alfabeto sin caracteres ambiguos que traía el script (se teclea una vez en el piso, leyéndola de un papel); y un chip **info, no warn** en el resumen porque una sesión larga es una decisión declarada, no un pendiente.
- **Dos heurísticas que el TTL volvía falsas:** `hasAccessAlert()` y `activityDotColor()` marcaban como cuenta dormida todo lo que no entra en 90 días. Un kiosco entra **una** vez, el día que se instala: sin excluirlo, la pantalla que funciona perfecto contamina para siempre el contador que se mira para encontrar cuentas abandonadas.
- ⚠️ **El número que decidió el diseño de la compuerta: 125 de las 126 cuentas de prod.** La regla nueva (`must_change_password: false` exige declarar `token_ttl_days`) se evalúa sobre el **CAMBIO**, no sobre la fila resultante. `must_change_password` nació con default `false` (la columna es anterior a `[ID.8]`), así que casi todo el padrón está en la combinación que ahora se prohíbe — incluidas las **8 etiqueteras**. Si la compuerta mirara la fila, **editarle el nombre a cualquiera de 125 cuentas daría 400** y la administración de usuarios quedaba rota para el 99.2% del padrón. Y el mismo problema del otro lado: el front manda `getRawValue()` completo en cada PUT, así que el campo viaja **sólo si cambió** — si no, editar el nombre de un kiosco le borraba el token en silencio.
- **Emitir una sesión larga lo hace un superadmin**, reusando el mecanismo anti-escalada que ya estaba en el archivo (`assertCanAssignRole`) en vez de estrenar un permiso: `USUARIOS_GESTIONAR` está diseñado para que **RH** dé de alta personas (`role-presets`, grupo `usuarios` primario de `rh`), y emitir una credencial de un año es otra cosa. Un permiso declarado pero **no repartido** es exactamente la deuda de LC.6.2, así que el permiso dedicado queda anotado como estado final, no implementado a medias.
- **Otorgar y revocar quedan asentados** en `identity.user_events` — hoy ni el alta ni la edición asentaban nada, y "auditable" era el objetivo. El asiento de la revocación dice explícito lo que la pantalla podría contar mal: la duración nueva aplica al **próximo** ingreso y no acorta el token ya emitido.
- **Pruebas: 35/0** en `test-newdb-device-token-ttl.js` (eran 19) y **33/0** en `test-newdb-user-dto.js`. La compuerta es pura, así que se la llama sobre el prototipo del servicio **real** — mide el código que corre en prod, sin Nest ni API. Se agregó el **techo** del CHECK (antes sólo se probaba el piso 0) y un **censo declarado**, no un check, de las cuentas que la regla nueva no dejaría crear.
- ⚠️ **Se arreglaron dos LOADERS de tests, no el código:** `ts-node` con `skipProject: true` descarta los `paths` de `tsconfig.base.json`, así que un DTO que importa del barrel de una lib revienta con `MODULE_NOT_FOUND` — rompió `test-newdb-user-dto.js` en cuanto el DTO trajo `MAX_TOKEN_TTL_DAYS`. Se registra `tsconfig-paths`: la alternativa era un deep-import entre libs, que es lo que el lint de tags prohíbe. Un test tiene que poder cargar el código real, no una versión del código que se deja cargar por el test.
- **Punto ciego cerrado en un candado que ya existía:** `test-newdb-user-dto.js` comparaba el `FormGroup` sólo contra el DTO de **update**, así que un control que el CREATE no acepta se descartaba en silencio y el test seguía verde — que es exactamente cómo el toggle "Estado activo" del alta no hace nada desde siempre. Ahora se compara también contra el create: **1 control** (`activo`) se descarta, y se **declara** en vez de convertirse en fail (arreglarlo es subir `status` a `UserWriteDto`, otro ticket).
- ⚠️ **Deuda preexistente que esta revisión encontró y NO arregla:** `/auth/login` (el login legacy, `apps/api/src/modules/auth/`) firma sin opciones → `'12h'` hardcodeado, **ignora `token_ttl_days`**, se saltea `kind='servicio'` (`[ID.17]`) y `expires_at` (`[ID.13]`), no escribe `last_login_at`, y lee `role_permissions` **sin filtro de tenant**. Verificado que `apps/view` no lo usa (`login.component.ts:85` llama `loginMt`), pero mientras el endpoint esté montado la afirmación "el token de esta cuenta vive lo que dice su fila" es falsa por ahí.
- **NO medido:** la vía HTTP (no hay API local arriba; el 4200 es el dev server de Angular) y la validación visual en el browser. Los 14 casos HTTP que faltan quedan enumerados en el tracker en vez de escritos como aserciones que nunca corrieron.

### Added — el verificador de precios de mostrador vuelve a ser una PANTALLA de la app (CV.24, 2026-09-08)

PR #68 lo entregaba como venía viviendo en `.163`: un `.ps1` en el Task Scheduler regenerando a diario un HTML autocontenido de 2 MB en `tools/verificador-precios/`. El lead lo rechazó, y el motivo es estructural: **una superficie entregada fuera de la app no tiene ruta, ni permiso, ni diseño, ni tests, ni deploy** — tiene una tarea programada que nadie mira hasta que falla. Es el mismo patrón que la Fase CV vino a retirar del backend, repetido en el frontend.

Rehecho como página del módulo Tienda: **`/tienda/verificador`**, componente standalone, superficie **Operations §O.3 Mostrador/POS** (foco permanente en la captura, el precio domina la jerarquía, feed al tope sin paginación, modo kiosco con overlay + Fullscreen API + Escape, dark de primera clase). **Backend: cero líneas nuevas** — consume los `@Public()` que el consolidado de esta misma fecha dejó en `apps/api/src/modules/kp/` (`/api/kp/precio`, `/api/kp/precios-todos`, `/api/sucursales`), todos derivados de `kepler_ods.*`.

**El offline se armó con la infra que la app ya tenía**, sin generar un solo archivo: `dataGroups` del service worker (`freshness` 2 s para la consulta puntual, `performance` 12 h para el catálogo) + snapshot en IndexedDB vía `OfflineDatabaseService`. Va **por sucursal**, no por tipo de catálogo, y eso no es un detalle: `KpService` ya advertía que sin `sucursal` `kdii` trae una fila por plaza y **385 códigos tienen precio distinto entre tiendas** — un único registro "precios" serviría el de otra sucursal, en silencio y sin ser determinista.

**Tres estados, no dos, y con prueba negativa cada uno.** El híbrido va al ODS primero y cae al respaldo sólo ante fallo de red o >2.5 s; el origen viaja *con* el precio ("Precio de respaldo · confirma en caja antes de cobrar"). Un **"no encontrado" del servidor es autoritativo y NO consulta el respaldo** — el snapshot es más viejo y resucitaría un precio ya cambiado. Y `sin_datos` (red caída sin respaldo descargado) es una pantalla **distinta** de `no_encontrado`: mostrar el vacío ante un fallo de red le afirma al mostrador que el producto no tiene precio, y es falso.

- **Permiso propio `STORE_PRICE_CHECK_VER`** (decisión de Edgar sobre reusar `STORE_LABELS_VER`) y **repartido**, no sólo declarado: la migración `20260909120000` **deriva** el alcance del estado vivo (quien hoy tiene `STORE_LIVE_VER` o `STORE_LABELS_VER`) en vez de inventar una lista de roles — la lección de LC.6.2, donde un módulo llevaba un día en prod con su permiso en cero roles. Simulada con ROLLBACK contra `.245`: **5 roles**. `etiquetas_anaquel` excluido a propósito (`20260908140000` lo recortó a una sola clave).
- **Hallazgo medido, no supuesto:** `datos_al` de `/api/sucursales` llega **null para las 7 sucursales** — `SucursalesService` lo deriva de `analytics.cron_runs` con `job_key = 'cdc_wal_' || sucursal` y **esa fila no existe** (el CDC por sucursal está muerto, Fase OBS). La `FreshnessPill` se ocultaba sola, y una píldora ausente se lee igual que "todo bien" — el silencio exacto que VP.0 vino a matar. La pantalla ahora **declara** "Frescura del ERP sin medir". ⚠️ Queda abierto de dónde sale esa frescura: no se cambió porque no se adivina una fuente de datos (regla #0).
- **Removed:** `tools/verificador-precios/` completo, y con él el directorio `tools/`.
- **Internal:** `apps/view/jest.config.ts` gana un `moduleNameMapper` para `@primeui/license-manager`. Un spec que importara **cualquier** módulo de PrimeNG reventaba antes de arrancar (`@noble/ed25519`/`hashes` publican ESM en archivos `.js`) — por eso este app no tenía **ni un** spec de componente. Con el doble no-op, una pantalla ya se puede probar.
- **Verificado:** `nx build view --skip-nx-cache` verde (sin pipe) · `tsc --noEmit` de api verde · `nx test view` **9/9 suites, 108 tests** (14 nuevos) · los 3 endpoints con `curl` y datos reales (`17083` → ALTOS CAM CHICA COLOR 1KG, KG $62.99 / BTO $1,159.91 ×20).
- ⚠️ **NO verificado: la validación visual en el navegador.** La sesión del Chrome de la máquina está expirada y no hay cuenta con la que entrar sin credenciales (no se pidieron ni se fabricaron). Faltan 3 pasos, documentados en [`FASE_CV`](docs/IMPLEMENTACION/FASES/FASE_CV_CATALOGO_TIENDA_MAYOREO.md): aplicar la migración, re-loguear, abrir la pantalla.
...
`fitPrice()` encoge el precio midiendo su ancho. **Si mide antes de que la tipografía esté usable, mide con la fallback** —que tiene otro ancho— y deja el tamaño mal en la dirección de esa fallback. Medido, mismo precio y misma caja, cambiando sólo la fuente de la medición:
...
**Por qué era intermitente:** ya existía `document.fonts.ready.then(() => layout())` y no alcanza. Las familias llegan por un `@import` a fonts.googleapis.com **dentro del CSS del componente**, así que mientras esa hoja no baja **no existe ningún `@font-face`**: no hay carga pendiente, `fonts.ready` resuelve al instante y se mide con la fallback. Dependía de si la CSS de Google estaba en caché. Y el re-layout vivía **sólo en `ngAfterViewInit`**, mientras que las etiquetas de la cola nacen de un cambio de input → esas se quedaban con la medida mala para siempre. **Segunda mitad:** con `clientWidth` 0 el espacio disponible salía negativo y el bucle llegaba al piso, dejando el precio en 4.5 mm.

Arreglado con `FUENTES_USABLES` (promesa memoizada que espera `fonts.load` + `fonts.check` con tope de 3 s, y re-ajusta desde los dos hooks) más la guarda "sin medida no se encoge". Sin internet se sigue midiendo con la fallback, que ahí es lo correcto: es la que imprime.

### Changed — El apartado de mayoreo, legible: de 3.7 a 5.4 mm (2026-09-07)

Al reducir la etiqueta a 82×35 bajé la celda del monto de 20 a 16 mm y el auto-encogido dejaba el precio de mayoreo en **~3.7 mm**. Ahora el precio unitario cede **4.4 mm** de ancho a la columna de tiers (34 / 43.4, ya no el 54:55 original) y su arranque baja de 11.5 a 10 mm; la celda del monto sube a 22 mm y el monto a **5.4 mm**.

**El bold no se podía hacer con `font-weight`:** Bebas Neue no tiene bold real y el navegador no la sintetiza — medido, `font-weight:700` daba **el mismo ancho al píxel**. El peso va por trazo óptico, más grueso en las filas de mayoreo, que además llevan chip amarillo de marca. Los rótulos se acortaron ("Mayoreo 3+ cajas" en vez de "Mayoreo desde 3 cajas:") porque el rótulo era lo que se comía el ancho.

**Y de paso, un recorte silencioso que ya existía:** el bloque de tiers no tenía ajuste vertical y con 4 renglones el contenido medía **100 px contra 92 de caja ya en la etiqueta de 115×40** — el 4º tier se perdía tapado por el `overflow:hidden`. Nuevo `fitTiers()` baja todos los montos por igual hasta que quepan. Dimensionado con el dato: de los **9,013 productos** con precios de etiqueta, **76.1% tiene 2 renglones** y **sólo 2.0% cuatro** → 1, 2 y 3 tiers imprimen a 5.4 mm (94.5% del catálogo) y los de 4 bajan a 4.0, sin recortes ni rótulos partidos.

⚠️ **Sexta vez:** un acento grave en un comentario CSS parte el template literal y el compilador tira `Failed to resolve styles at position 1 to a string`. **ts-jest no lo detecta** (no hace el análisis estático de Angular): los 81 tests salían verdes con el build roto. El spec ahora lo prohíbe. Candado del módulo extendido a **16 casos**.
...
### Changed — El alcance dejó de ser un cheque en blanco, población por población (IDG.9, 2026-09-08)

`[ID.3]` había materializado como `all` explícito el acceso amplio **implícito** que la convención vieja daba («sin sucursal = ve todas»), con la nota «candidato a recortar». Esto recorta, midiendo cada población antes de tocarla.

- **Ruta (batches 297, 309, 310).** `persona → ruta` **no es una función**: sobre `daily_assignments` vigentes hay gente con 2, 4 y 5 rutas, y en `daily_captures` (90d) hasta 6 — poblar `users.route_id` y girar el rol a `own` le daría a esa gente **una de sus N rutas**. Y las fuentes se contradicen: de 22 personas con asignación **y** captura, **8 (36%)** no coinciden. Se pobló con la forma que la realidad tiene: 19 `route_id` (17 con evidencia operativa, 2 por convención de username) + 4 en `user_scopes` con `mode='listed'`, que es exactamente lo que los multi-ruta piden. ⚠️ **Mi propio gate era un falso verde:** filtraba `positions.scope_axis='ruta'`, que resuelve a **cero** usuarios activos (116 de 120 tienen puesto sin eje) — devolvía 0 vacuamente y habría dicho "ya se puede girar". Se cambió a medir por rol.
- **Sucursal: 35 overrides → 30 `listed` / 7 `all` (batch 319).** ⚠️ Se advirtió que quitarlos «los deja ciegos» y **estaba mal**: medido, **0 de los 35** tenía `STORE_LIVE_VER`, `STORE_ANALYTICS_VER` ni `STORE_ARQUEO_VER`, o sea que el override no concedía nada y recortarlo no quitaba nada. Se hizo ahora justamente porque es inerte. Los 7 que quedan tienen la zona sin ninguna sucursal asociada y **se declara que falta el dato** en vez de inventarlo: sus rutas tienen 0–1 tiendas cargadas, así que la plaza madre no se sostiene. Gate permanente: un `all` sin motivo escrito hace fallar la migración.
- **Cajeras (batch 320).** La anomalía era exacta: **las 3 de sucursal `04` eran las únicas sin `zona_id`, y son exactamente las 3 que tenían `zone = all`** — faltaba la zona, el default `own` resolvía a «sin valor» = no ve nada, y el parche fue abrir todas las zonas. Se arregló la herida: `zona_id` derivado de la sucursal (`sucursal → zona` **sí** es función: las 8 con zona apuntan a 1 cada una) y el override fuera. Cajeras **25/25 con sucursal y zona**.
- **Etiqueteras: una por tienda y sólo etiquetas (batches 336 y 338).** La etiquetera es **ciega a la sucursal** —no consume ninguna dimensión del `ScopeService` y `product_label_prices` no tiene columna de almacén—, así que una cuenta por tienda no cambia lo que nadie ve: da credencial propia, trazabilidad y revocación por tienda. Las 8 tiendas se **derivan** de `commercial.warehouses` con zona y se contrastan contra `store_live_tickets` (tickets de hoy en esas 8 y sólo en esas 8; Morelia `30`/`32` van por Wincaja, de ahí que no aparezcan en el feed Kepler). Las 2 que existían corrían en `piso_tienda` = **7 permisos**, incluido contar el efectivo de la caja —enforzado en el backend— y **jamás capturaron un arqueo**. ⚠️ **El rol correcto ya existía y no servía de nada:** `etiquetas_anaquel` figuraba con 0 usuarios pero las 2 ya lo cargaban como **complemento**, y el JWT lleva la **UNIÓN** de los roles, así que agregarlo nunca quitó nada — sólo sumó. Recortado a la sola clave `STORE_LABELS_VER`. ⚠️ **Cambiar `users.role_name` no alcanza:** el trigger `sync_primary_role_from_user` **degrada** el rol viejo en vez de borrarlo (a propósito), y degradado sigue sumando al JWT: hay que borrar la fila de `user_roles`. ⚠️ **`kind='servicio'` habría creado 6 cuentas inusables** — `[ID.17]` bloquea el login interactivo para ese `kind`.
- **Y el permiso que no servía de nada (batch 309 + front).** `promotor_ruta` recibió las claves de caducidades, pero la tarjeta del lanzador —**el único camino** al proyecto Tienda— pedía un `STORE_*`: acceso concedido en el backend y sin puerta en la UI. Se agregó el `anyOf`. Es lo mismo que hizo falta verificar para las etiqueteras: con un solo permiso, la cadena login → `/projects` → `/tienda` → `etiquetas` tiene que cerrar en los cuatro puntos (tarjeta, redirect, ruta, sidebar).

**Contraseñas de las 6 altas:** el script genera una distinta por tienda, aleatoria y sin caracteres ambiguos porque se teclean en un kiosco; no se imprimen ni entran a git, y las verifica con **prueba negativa** — el hash acepta la generada y rechaza otra. `must_change_password = false` con motivo: es credencial de puesto, y forzar el cambio deja al resto del turno afuera. ⚠️ **Por decisión del lead (2026-09-09) las 6 quedaron en `123456`**, con el riesgo declarado antes de aplicar y reafirmado: **un token válido no es lo mismo que un permiso**, porque el guard global es no-op en una ruta sin `@RequirePermissions`. En la primera pasada el alcance se derivó de `last_login_at IS NULL` (las 6 sin usar); con autorización posterior del lead, **`etiquetas.02` también se igualó** —hay que avisarle a esa tienda, que usaba otra— y `etiquetas.04` ya la tenía desde el 3-sep: **8 de 8 con la misma clave**. ⚠️ El `UPDATE` en lote reusó **un solo** `bcrypt.hash` para las 6, así que **6 de 8 compartían hash idéntico**; corregido con salt propio por cuenta (8 de 8 distintos, misma contraseña). Inmaterial con `123456`, pero es la clase de artefacto que después se lee como intencional.

**Abierto:** los 7 `warehouse = all` esperan que alguien diga de qué sucursal carga esa gente (la migración es idempotente y derivada, así que una segunda pasada los recorta sola) · 2 `zone = all` de telemarketing, sin `warehouse_code` no hay de dónde derivar · el giro de `route: own` sigue bloqueado por su propio gate · la fila de `daily_assignments` de `rvph01` en `RUTA 21` es la que está mal.
...
- **La pantalla vieja no se borra: se re-rotula a «Ajustes de stock» y lo declara.** Es lo único que tiene la escritura y el apartado, así que sigue viva con un aviso en línea de qué es y cuánto se desvía, más el enlace al censo. Heredar el Ajuste habría roto la pantalla nueva (escribe la tabla, muestra el ODS → ajustás y el número no se mueve).
- **La regla del dinero, con candado propio:** una celda cuyo peldaño el costo contradice muestra la cantidad **nativa con el rótulo del ERP** (`2,679 kg ⚠`) y su dinero va en raya, nunca `$0`. El test verifica que hay celdas marcadas que inspeccionar (no pasa en vacío) y que **cero** publican cajas o dinero.
- **Frescura por rama**, y no es cosmético: en una misma sesión Kepler pasó de **796 min a 0** y Wincaja de **371 a 761**. Una píldora sola habría promediado un feed de minutos con uno de horas. La pantalla tampoco promete "en vivo": publica la edad real de cada rama, y marca la columna cuyo feed se atrasó.
- **Perf medida, no estimada:** 4,435 ms el primer prototipo → **1,748 ms** (piso de 3 corridas). Los joins no eran el problema (332 ms los cuatro): era el pivot `jsonb` sobre 9,860 productos. Receta: **una** CTE `MATERIALIZED` + agregado sin `jsonb` + `LIMIT` + **el pivot al final, sólo sobre la página**.
- ⛔ **`customer_b2b` excluido del reparto del permiso**: verificado, tiene `COMMERCIAL_INVENTORY_VER = true`, así que el ancla obvia le habría dado la existencia de toda la red **valuada a costo**.
- **También:** `/compras/pedido` no leía query-params en absoluto, así que "navegable a su arreglo con el filtro puesto" era humo — ahora hidrata `search` y `warehouse_ids`.
- ⚠️ **Sin verificar:** ninguna otra pantalla del repo combina `#footer` con `pFrozenColumn` en PrimeNG 22, y O.2 exige la fila de totales congelada. Queda para validación visual.

### Fixed — El carril del ODS tenía tres dueños, y el muerto pasaba el healthcheck con el pulso del vivo (OBS.8, 2026-09-04)
- Disparador: **`/comercial/documentos` "no se actualizaba al momento"**. La pantalla no tenía nada: lee vistas en vivo sobre `kepler_ods`, así que su frescura *es* la del ODS. Abajo, el contenedor `ods-live-hot` llevaba **15 h colgado** (proceso vivo, **CPU 0.00 %**, cero salida desde `latido (end) falló: Connection terminated unexpectedly`) mientras Docker informaba `Up 2 days (healthy)`, y **cinco de las siete sucursales llevaban 14 h 56 min sin una sola pasada** — ventas las tapaba a medias el reconciliador cada 15 min, pero catálogo, precio, costo y existencia de esas cinco no las tapaba nadie.
- **La causa no era el cuelgue, era que nadie podía verlo.** El mismo shipper corría en Docker, en una tarea de Windows y dormía en el `dump.pm2` listo para revivir una tercera copia en el próximo reboot. `analytics.cron_runs` tiene `PRIMARY KEY (tenant_id, job_key)` y **no guarda host**: los tres escribían el mismo renglón, y el healthcheck del contenedor muerto leía el pulso que la tarea de Windows escribía desde otra máquina. Ahora el healthcheck compara `cron_runs.host` contra su propio `os.hostname()`.
- **Y el guardián mataba la pasada sana.** `FeedGuardian` decide "trabado" por *mtime del log*; una pasada real del ODS pasa más de 10 min sin imprimir, así que mataba el proceso a mitad de camino cada ~20 min y el `.cmd` re-arrancaba **desde la primera sucursal**: las otras cinco eran inalcanzables por construcción. Cuatro veces en una hora, con nombre y PID en `guardian.log`.
- **El gatillo del cuelgue: `pg` no trae timeout de conexión por default.** El latido —lo único que sale de la LAN, y lo primero de cada ciclo— abría su cliente sin `connectionTimeoutMillis` mientras el resto del script sí lo tenía. Un `connect()` contra un peer que no contesta ni resetea espera para siempre y clava el bucle del `--watch`.
- **El `HEALTHCHECK` de Docker no reinicia nada** — `restart: unless-stopped` reacciona a que el proceso *muera*, y este murió vivo. El `Dockerfile` afirmaba lo contrario y esa creencia costó las 15 h. Se agregó `autoheal`, acotado por etiqueta para que no toque la base de datos ni el API. Verificado con un contenedor canario.
- **Bajas:** 6 tareas programadas del ODS desregistradas, `dump.pm2` de 12 apps a 2, `watchtower` (en crash-loop) eliminado, y el **carril CDC-WAL retirado** (6 slots `lost` dropeados, su ecosystem convertido en lápida que falla si alguien lo levanta, 10 latidos muertos borrados del tablero).
- **Lo que el WAL cubría en exclusiva** —propagar DELETE— pasa a ser una señal: `reconcile-ods-window` cuenta ahora los **sobrantes** (llaves que siguen en el ODS y ya no están en la réplica). Reporta, no borra.
- **Medido después: las 7 sucursales a 9-30 s de rezago.** Deuda que quedó anotada: **5,021 pedidos fantasma** en el ODS, medidos en dry-run con un reconciliador de DELETEs que existía y nunca se había agendado.

### Fixed — Movimientos no asociados · la fecha salía con el día equivocado, y el rojo significaba cuatro cosas (LC.16, 2026-09-03)
- **`String(fecha).slice(0,10)` no es un formateador.** Sobre un `date` de pg daba **`"Mon Aug 31"` para una factura del `2026-09-01`**: pg devuelve la columna como `Date` en UTC-midnight y `String()` la renderiza en hora de México (−06:00), o sea **el día anterior**. Salía por cuatro lados, todos hacia la contadora: la tabla, el **respaldo XLSX** (columna declarada `type: 'date'` recibiendo el texto), el **CSV del Asociador de CFDI**, y el **orden de los renglones del TXT entregado** — `construirMovimientos` ordena por `a.fecha < b.fecha`, o sea alfabéticamente por día de la semana (`Fri < Mon < Sat < Tue < Wed`). Arreglado en el origen con `to_char(f.fecha,'YYYY-MM-DD')` en los tres SELECT.
- **El color había perdido su significado.** Medido antes de tocarlo: en jul-2026, **258 de 446 filas (58%) estaban en rojo, y 159 de esas decían "Ya en el libro"** — el camino feliz gritando como si fuera un error. Cuatro estados semánticamente opuestos compartían `severity="danger"`. Ahora el rojo es sólo lo que **impide generar el TXT** (jul: 258 → 99); "Ya en el libro" recede a texto apagado con check; fuera el verde del KPI de IEPS, que estaba puesto por `ieps > 0` (color por ser distinto de cero).
- **La primera factura aparecía al 63% del alto de la pantalla**, tras nueve bloques apilados, con tres barras de scroll. Cobertura + contexto + avisos van a un renglón que se abre (los **bloqueantes quedan fuera, siempre visibles**: son la razón por la que el botón está apagado); el rail se agrupa por año con el pendiente en el encabezado.
- **Chips de filtro + buscador**, default "Entran al TXT": en ago-2026 la tabla trae 722 filas y **240 son intocables**; en jul-2026, 262 de 440. Los cuatro grupos particionan la tabla y su `entran` coincide **exacto** con lo que el rail ya mostraba (82 / 482 / 178 / 108).
- **La tira de 5 mosaicos baja al pie de la tabla**, alineada bajo su columna, donde se lee como el asiento que es (`0% + c/IVA + IEPS + IVA = total`) y donde sirve — antes, en el renglón 400 de agosto no había ningún total a la vista. Suma lo filtrado y rotula cuál filtro. **`$0.00` → guion tenue**: 51-55% de las celdas de impuesto iban en cero, o sea la mitad de la tinta numérica de la tabla.
- La columna `Estado`, cuyo valor más común era un número de cuenta (52% en sept, 70% en ago), se parte en `Cuenta` + `Estado`. Y el breadcrumb decía **"Contabilidad / Página Actual"** — placeholder en prod para toda página que vive en pestañas.
- Builds api+view verdes; smokes `test-newdb-libro-compras-txt` (33/33) y `-caratula` (57/57). **Pendiente prod: redeploy api+view.** Sin migraciones ni permisos nuevos → sin re-login.

### Fixed — Compras deja de valuar y de PEDIR sobre un divisor que el costo contradice (U.2b, 2026-09-03)
- **El pedido era el hueco grande.** U.2 retuvo el *valor* de la existencia en `/compras/pedido` pero no la columna **Pedido**, que es la que se gasta y sale de restar esa misma existencia. Partido por dirección: **386 celdas piden $257,985 donde el árbitro pediría $130,711** (compran lo que ya está en piso) y **165 piden $60,360 donde pediría $150,769** (no compran lo que falta → quiebre). No es un ahorro: son **$218k del pedido apuntando mal en los dos sentidos** sobre $12.56M.
- **La regla se extiende a las 6 superficies de dinero de Compras**: Existencia Crítica (7 KPIs + fila, 465 políticas / 231 SKUs fuera del total), Sobrestock ($22.9M → **$14.6M** publicado), A traspasar (156 filas), A comprar (64 SKUs) y la bandeja de reabasto, que **persistía** el importe inflado.
- **El lado que ninguna retención puede mostrar:** con el divisor inflado la sucursal se lee abastecida, el déficit da 0 y la fila **no existe** en el plan de traspaso — **19 traspasos invisibles** contra 156 marcados. Se cuentan con un window *antes* del filtro y se declaran aparte.
- **Dos trampas de "cero" cerradas antes de poder retener nada:** los tres `money()` de Compras hacían `Number(v ?? 0) || 0`, así que un importe retenido salía **$0** ("no cuesta nada"); y `replenishment_findings.suggested_cost` era `NOT NULL`, o sea el scanner habría tenido que escribir 0. Mig `20260903160000` lo ablanda con el contrato en el `COMMENT`. También: `ORDER BY ... DESC` pone los NULL **primero** en Postgres → las filas retenidas encabezaban la lista del comprador (4 sitios).
- **Perf:** joinear la vista auditable (8.2 s marcados / 25 s tenant) llevó el plan de traspaso de 4.5 s a **29 s**. El veredicto baja al fact (mig `20260903170000`, lo escribe el nocturno) y los seis consumidores —que ya joineaban ese fact— **no agregan ni un join**: traspaso **29,362 → 288 ms**, workbook **443 ms**, sobrestock **106 ms**. La vista sigue siendo la fuente auditable; fact 552 = vista 552, cero discrepancias.
- **Candado** (`test-newdb-unit-rung-audit`, 40/40): si el importer deja de copiar el veredicto, todo pasa a "medible" y la mentira vuelve **en silencio** — un no-op se lee igual que "no hay nada marcado". El test exige que el fact tenga veredictos y coincida con la vista.
- **Documentado** en [`UNIDADES_DE_MEDIDA.md`](docs/UNIDADES_DE_MEDIDA.md) §8quinquies: *por qué la venta sí se convierte a cajas y la existencia no.* No cambia el divisor, cambia la unidad del numerador — la venta se normaliza sola porque **el ingreso delata la unidad**, la existencia es un conteo pelón. Corolario: la venta **no está verificada, está sin árbitro**.

### Security — Auditoría a fondo de permisos + endurecimiento (Fase AUTHZ-HARD, 2026-09-03)
- **El secreto de firma del JWT era el default público del repo.** `process.env.JWT_SECRET || 'super_secret_dev_key_change_in_prod'` en 16 archivos, y el string está commiteado en `docs/` → con él cualquiera forja `{role_name:'superadmin'}` y obtiene god-mode cross-tenant. Medido: prod usaba ese default. `requireJwtSecret()` (nuevo, en `libs/platform-core`) **aborta el arranque** si falta o es el default en producción; `algorithms:['HS256']` pinneado en todos los `JwtModule`. El webhook de WhatsApp y el ingest de tienda dejan de **fallar abierto** sin su secreto. **Pendiente: Edgar rota el `JWT_SECRET` de prod.**
- **Toda la autenticación colgaba de `ENABLE_MULTITENANT`.** Los guards globales sólo se registran si esa env es `'true'`, y el runbook de rollback dice literalmente "quitarla". `assertAuthWiring()` en `main.ts` aborta el boot en prod si no está.
- **~10 IDOR cerrados.** `logistics/analytics|reports` (nómina de choferes, márgenes, ROI) estaban **sin permiso** — cualquier autenticado, incluido un cliente B2B, los leía; ahora gateados por `LOGISTICS_*_VER`. `customer_b2b` (con `CUSTOMERS_VER`/`ORDERS_VER`) leía el 360, historial, canasta y pagos de **otros clientes** por `:id` → ownership enforcado. `visits/:id`, `daily-captures/:id`, `reports/:id` eran cross-tenant (superusuario sin filtro) → filtro por tenant. `push/unsubscribe` (borraba la de cualquiera) y `telemetry/portal/summary` (cross-tenant por `?tenant_id`) acotados. Inyección SQL en `fleet-utilization` parametrizada. Tableros internos de Thot separados en `COMMERCIAL_INTELLIGENCE_VER` (mig `20260904120000`).
- **Escalada de privilegios.** `PUT /users/:id/roles` y `/scope` no tenían los frenos de sus hermanos: cualquiera con `USUARIOS_GESTIONAR` se añadía `superadmin` como complemento y heredaba las 164 claves. Frenos añadidos (radio actual = 0, pero el candado va en código).
- **Desactivar/degradar = revocar.** El JWT (12h, no revocable) llevaba estado stale: un despedido/degradado seguía entrando. `JwtAuthGuard` relee `identity.users` (cache 30s) y da 401 si inactivo; el god-mode se decide con el rol fresco de DB, no del token.
- **Login:** rate-limit propio 5/min/IP (el global dejaba ~288k/día) + cerrado el oráculo de enumeración (los chequeos de estado corrían antes del bcrypt).
- **Fail-open de alcance cerrados (AUTHZ-HARD.3).** El patrón `user?.warehouse_code || query` / `zona_id NULL = ve todo` daba la red entera a quien no tenía alcance configurado. Migrados a `ScopeService` (fail-closed) tras **medir en prod** que ningún usuario real se ciega (los `own` tienen su columna; sólo smoke/servicio/god-mode quedan fuera) y que **0 usuarios** con `REPORTES_VER_GLOBAL` se restringen: store `snapshot`/`ticket-lookup` (warehouse), consola `reconciliation` overview/focos/cash-cuts/movements (un `encargado_tienda` veía los faltantes de caja de TODA la red), `stores.service` y `commercial-map.service` (dimensión `zone`, además desmezcla el eje de REPORTES). Aclaración: **`getDataScope` NO se retira** — es otro eje (jerarquía de reportes por `supervisor_id`) y ya es fail-closed.
- **Saneo de `user_scopes='all'` (prod).** Descubrimiento: las 116 filas son artefactos del materializador `[ID.3]` (mismo `nota`), no grants deliberados. Script `sanitize-scope-overrides.js` (backup + auditoría `scope_changed` + idempotente) quitó **72 overrides** seguros → vuelven al default del rol: redundantes (rol ya `all`), own-poblado (cae a su sucursal/zona real) y `customer_b2b` (portal deja de ver interno). Retenidos ~42 de usuarios sin sucursal/zona asignada (quitarlos los cegaría → asignar padrón primero).
- **Diferido con motivo** (en el tracker): asignar sucursal/zona a los ~42 retenidos (padrón, desde `/admin`), paridad front/back (UX, no seguridad), resto de higiene del padrón, re-lectura de rol en gateways WS.
- Builds api+view verdes. Smokes `test-authz-route-coverage` (18/18) y nuevo `test-authz-boot` (7/7). **Pendiente prod:** rotar secretos + aplicar mig + redeploy + re-login.

### Fixed — Arqueo · el sistema acusaba a las cajeras de un faltante que no existe (SM.23, 2026-09-03)
- `diff_real = esperado − total_contado` **estaba mal desde el principio**: el `esperado` de Kepler es de TODO el turno e incluye el efectivo que ya salió en sangrías, mientras que el conteo del cierre es solo del cajón. Con los promedios reales —esperado **$27,564** contra un cajón de **$8,977**— el sistema le habría reclamado **$18,587** a una cajera que contó bien. Pasó desapercibido porque los dos únicos arqueos capturados eran de datos sembrados con retiro en cero.
- La identidad correcta cierra el turno entero: **`Σ retiros contados + cajón contado = esperado`**. Y lo que no se contó **se declara aparte** (`retiros_sin_verificar`, `cobertura`) en vez de mezclarse con el faltante: un faltante real y "no lo contamos" son cosas distintas y no pueden sumar al mismo número. Verificado con el escenario real: `diff_real` pasa de **$15,000 inventados a $0**, con cobertura 25% sin contar los retiros y 100% contándolos.
- **El turno abierto que cruzó la medianoche desaparecía de la pantalla** — regresión de la ventana "solo hoy". La cajera lo sigue trabajando; es la "caja arrastrada" que el tablero ya vigila aparte. En staging era la caja 1 de suc 01 con **$15,000 retirados sin contar** y sin nadie a quien pedírselo. Ahora el turno abierto entra siempre; la ventana solo acota los cerrados.

### Added — Arqueo · "haz tu arqueo" y el retiro, donde está el 63-81% del efectivo (SM.23, 2026-09-03)
- **El programa no avisaba.** El turno esperaba en `/tienda/arqueo` y si la cajera no abría esa pantalla no se enteraba; la única alerta que salía sola era al supervisor y a los 45 minutos. Eso explica buena parte del 1% de cumplimiento: no es que la gente se niegue a contar, es que nada se lo pedía. Ahora llega esté donde esté — barra fija en la raíz (no toast: con clientes enfrente, algo que se va en 5 segundos no se ve) + notificación del navegador con la pestaña oculta, por un **room personal** del gateway `/store`. Soporta varios cortes en el día.
- **Decode nuevo — el disparador de los $15,000:** `kdpv_folio_caja.c46` es el límite de efectivo en caja (en suc 01 vale exactamente **$15,000**, el valor más común: 206 de ~318 cortes) y `c47` el tope duro. Al llegar al límite Kepler pide sacar el dinero y **sube `c48` en el turno ABIERTO** — verificado en vivo. Eso **corrige a SM.17**, que predecía el corte por HORA: el disparador no es el reloj, es el monto (por eso algunas cajas daban ±210 min de dispersión).
...
- **La cadena:** el rol `almacenista` da exactamente `COMMERCIAL_INVENTORY_RECIBIR` + `_SUPERVISAR`. Login → `/projects` → la tarjeta **Almacén** pedía `anyOf: [INVENTORY_VER, WAREHOUSES_VER, DEADSTOCK_VER, INVHEALTH_VER, RECONCILIATION_VER]`, **ninguno de los suyos** → cero tarjetas → `projects.component` hace `navigate(['/dashboard/captures'])`. Ésa era la pantalla por default.
- **Y un segundo rebote detrás:** entrando a `/almacen` a mano, `almacenHomeGuard` tampoco tenía sus permisos entre los candidatos → caía al fallback fijo `/almacen/inventory`, que exige el permiso que no tiene. **Sus pantallas existían y estaban bien gateadas**; lo único roto era la puerta de entrada.
- **Arreglo:** la tarjeta Almacén suma los permisos operativos del piso (5 → 13) y `almacenHomeGuard` suma 7 candidatos, cada uno apuntando a una ruta cuyo guard es ese mismo permiso (verificado ruta por ruta). Y `landingRedirectGuard` deja de mandar al fallback fijo cuando nadie empató: ahora **dice qué permiso falta** en vez de rebotar mudo.
- **Medido en prod:** antes caían **3 usuarios** (`luis_espino`, `brian_zavala`, `luis_navarro`); después, **cero** — el único sin tarjeta es `hacker`, que tiene 0 permisos. **Sólo requiere redeploy de `view`**: sin backend, sin migración, sin re-login.
- ⚠️ **Aparte, sin tocar:** el mapa del rol `almacenista` tiene 164 claves con **2 en `true` y 162 en `false` explícito**, escrito el 2026-09-02 por `superoot` — la firma de *"se guardó el mapa entero desde `/admin/roles`"* (mismo residuo que `[LC.6.2]`). Si el rol antes tenía más permisos se perdieron ahí; no hay bitácora de cambios de rol para saberlo, así que **no se adivinó**: si el almacenista debe hacer más, hay que volver a marcárselo.
- **Queda abierto (la clase, no el caso):** 35 permisos gatean pantallas y no abren ningún proyecto. Hoy no rompen a nadie, pero el patrón vuelve con el próximo rol acotado. El arreglo de fondo es derivar el gating de las tarjetas y los landings de `AUTHZ_TREE` en vez de mantener tres listas a mano.

### Fixed — Compras: **la cantidad se muestra en la unidad más grande, con el divisor del ERP dueño del almacén** (ADR-055, 2026-09-03)
- **De dónde salió:** Edgar pidió *"a diferencia de Kepler, usemos la unidad más grande de medida para mostrar las cantidades de venta y existencia; usar la tabla de existencias de Kepler para las que ya lo tienen, y la de Wincaja la tabla de Wincaja."* No era una preferencia visual: era el nombre correcto de un defecto en el camino del dinero.
- **El defecto:** los dos ERPs guardan la existencia en unidades distintas — Kepler en su unidad base, **Wincaja en su unidad de venta**, que en los multipack es el **PAQUETE**. `/compras/pedido` dividía la de **todos** los almacenes por un solo factor por producto (unidades base por caja), así que en MD-30/MD-32/CEDIS `00` dividía entre 140 lo que iba entre 14.
- **Y no era cosmético.** La capa cruda sí es auto-consistente, pero la derivada no: `analytics.product_demand.daily_pieces` **normaliza** la demanda a la unidad base (medido: 159 de 166 multipack de MD-30 con venta) mientras `replenishment_plan.stock_pz` se queda en paquetes — **el nombre miente**. El motor restaba *piezas de demanda menos paquetes de existencia*: **$866,805 de sobre-pedido** y **$2.68M de inventario que la pantalla no mostraba**. Son ~355 SKUs, no el catálogo entero; **las 6 sucursales Kepler no cambian ni un peso**.
- **Resolvedor único:** `analytics.v_warehouse_box_factor` (vista, `derive-no-copy`) da el divisor por **(almacén, producto)** en unidades nativas de ese almacén — Kepler por su resolvedor canónico, Wincaja por `wincaja.articulos.factor_venta`. Materializado en `replenishment_plan.display_bf` sólo por costo de lectura; la regla vive en la vista.
- **`transferPlan` y `overstockList` pasan a calcular en CAJAS:** restaban unidades de universos distintos, y `transferPlan` además comparaba el déficit del destino contra el stock del **CEDIS origen**, que puede tener otra unidad nativa. La caja es la única unidad que los dos ERPs declaran.
- **Se retira el gate hardcodeado** `w.code IN ('MD-30','MD-32')` (dejaba fuera el CEDIS `00`, también Wincaja) y la lectura de `analytics.wincaja_product_box_factor`, tabla alimentada por importer.
- ⚠️ **Es un divisor de PRESENTACIÓN: el dato base no se convierte.** Se intentó y llegó a prod el día anterior (revertido): `inventory_health`/`reorder_policy` salen de `sales_daily`, que está en la unidad nativa, así que convertir sólo la existencia **paró el pedido** (cobertura de 534–900 días).
- **Pendiente:** redeploy api + view. Migración ya en Railway (batch 260) e importer ya corrido. **Sin permisos nuevos → sin re-login.**

### Changed
- `docs/UNIDADES_DE_MEDIDA.md` §8ter documenta el hallazgo y la regla que sale de él: **la unidad de una columna no se hereda de su fuente**. Cada tabla derivada puede normalizar una columna y no la otra, y el nombre no avisa — hay que probar la unidad **en la tabla que el consumidor lee**, con el precio realizado contra `analytics.v_product_unit_ladder`. El §9 ("lo que NO se investigó") ya no lista Wincaja-vs-Kepler.
- ⚠️ **`ADR-052` está triple-ocupado** (*Contratos de tipos del boundary REST* + **Fase LC** + **BFF del Command Center**). Esta línea se renumeró a **ADR-055**; LC y Command Center siguen colisionando y falta decidirles número.

### Internal
- Candado `test-newdb-warehouse-box-factor.js` (**29/29** contra prod, en la regresión), con candados explícitos contra los **dos errores previos de esta misma línea**: convertir el dato base, y el `LEFT JOIN` a `wincaja.articulos` sin `source_dataset='actual'` (duplicaba la existencia, razón 2.000).

### Added — Arqueo · el corte de Kepler llega solo, y la cola que hace que el conteo ocurra (SM.20/SM.21, 2026-09-03)
- **Había cortes que Kepler generó y nunca jalamos.** `analytics.cash_cuts` se llenaba corriendo un CLI a mano: medido el 02/09, el ODS tenía 515 cortes cerrados con dinero y nuestra tabla 499. Los **20 que faltaban** eran todos de la sucursal 02, con **$300k+ de efectivo declarado** — y no se veían como pendientes, no se veían en absoluto, que es peor: un turno ausente no se persigue. Ahora `CashCutsSyncService` (@Cron 10 min) + un sync perezoso al abrir la pantalla lo traen solos; como el ODS vive en la misma base, es un UPSERT de una sentencia. Smoke `test-newdb-cash-cuts-sync` afirma **cero cortes sin espejo** y se verificó que falla cuando debe.
- **"Sin arquear" era una etiqueta que mentía.** La pantalla decía *"nadie contó el efectivo"* en turnos donde Kepler **sí** traía su cifra, mostrándola justo debajo del cartel. Kepler genera SU arqueo; lo que falta es NUESTRO conteo físico, y llamarlos igual borra el hallazgo que sostiene la fase (el 74.6% de los cortes cierra al centavo exacto: `c25` es declarado, no verificado). Pasa a **"solo Kepler"**, con el monto declarado a la vista y el detalle diciendo qué es. A la cajera se le sigue ocultando el monto (SM.10).
- **El corte sin contar deja de ser invisible (SM.21).** 76 de 78 cortes sin conteo no es un dato que falte jalar: es trabajo que no se hizo, y sin dueño ni reloj no se hace — contar el cajón compite contra atender clientes y siempre pierde. El turno cerrado se vuelve **exigible**: regla nueva `arqueo_no_realizado` a la bandeja que el encargado ya abre. **45 min** = aviso; **12 h** = crítico y `arqueo_no_verificable`, porque pasado eso el efectivo ya se depositó o se fue en sangrías y el corte queda sin verificar de forma permanente — el hallazgo no se apaga, sube. El score ordena por dinero además de por demora.
- **El número que hace que la cola sirva:** `GET /cumplimiento` + franja en pantalla. En staging dice lo que hay que oír — **1% de los cortes con conteo físico, $5.8M sin verificar** desde el 1 de agosto.
- **Dónde está el efectivo, medido:** entre el **63% y el 81%** del contado sale del cajón como **sangrías** durante el turno (Padre Hidalgo $7.6M de $12.1M en 90 días). Contar solo al cierre verifica una minoría del dinero. Kepler **no itemiza los retiros** (solo el total `c48`), así que el arqueo de la sangría queda como el siguiente paso de mayor cobertura.
- **UI:** tarjetas por cajera **agrupadas por sucursal**, con la sucursal desplegable al hacerle clic (acordeón WAI-ARIA) y su propio conteo. El ticket de 80 mm se imprime **en la captura**, no en el historial — antes había que salir a buscar el corte para llevarse el respaldo, o sea firmarlo media hora después del conteo.

### Fixed — Arqueo · tres bugs que solo se veían con datos reales (2026-09-03)
- **El sync fallaba callado.** `trx.raw(SQL, [a, b])` con placeholders `$1/$2`: knex espera `?` o nombres, respondía *"Expected 2 bindings, saw 0"* y el servicio lo tragaba como best-effort. El UPSERT nunca corría — exactamente el fallo que ese código venía a arreglar, sin un solo error visible.
- **El turno que "toca" lo decidía la posición en el arreglo.** Salía primero y accionable el corte del 02/09 con el del 01/09 bloqueado detrás, al revés de SM.16: el template usaba `i > 0` sobre el orden de llegada y el backend desempataba dos cortes del mismo día por número de caja, no por hora. Ahora se ordena por el instante real de cierre y cada tarjeta lleva su ordinal.
- **SM.16 impedía CORREGIR un conteo ya hecho.** La regla existe para que no te saltes un pendiente, pero también bloqueaba recontar un turno ya arqueado cuando quedaba uno más viejo — dejando congelada una cifra que la cajera sabe equivocada, justo lo contrario de lo que busca. Ahora la corrección está exenta: el turno viejo sigue igual de pendiente después de ella.
- **El header le decía a la cajera una sucursal que no es la suya:** mostraba `rol · zona` siempre, así que a una cajera de Padre Hidalgo le decía "CAJERO · LA PIEDAD RD" (su zona de trade marketing). Poner el nombre equivocado de la sucursal arriba de la pantalla donde se sella efectivo es peor que no poner nada.
- **Rectificado el decode de `c43`/`c44`** (billetes/monedas): el análisis viejo los daba por no confiables porque metía `c45` en la suma y exigía tolerancia de centavos. La identidad real es `c43 + c44 + c48 (retirado) = c25`, y cuando no cierra el hueco suele ser un retiro sin registrar — ahora sale con nombre en pantalla y en el ticket.
- **Cerrado el decode del desglose por denominación:** no existe en ningún ERP. Barridas las **1,275 columnas numéricas** de Kepler por forma del dato (3 candidatas, ninguna es arqueo) y perseguido el total del corte, que aparece en **una sola columna de toda la base**. Wincaja sí tiene denominaciones, pero de cada **retiro**, no del corte (`Arqueos.Folio` ata a `Retiros`, verificado: folio 88036 → $11,300), y sus sucursales ya migraron a Kepler.
...
### Fixed — Autorización: **61 escrituras estaban abiertas a cualquier usuario con sesión** (AUTHZ.5, 2026-09-02)
- **De dónde salió:** con CASL retirado quedó un solo mecanismo, y eso volvió medible **quién no lo usa**. De **1,115 rutas** (641 lectura / 474 escritura), **61 escrituras** no declaraban `@RequirePermissions` ni eran `@Public`. `RolesGuard` es global pero **no-op** sin el decorador —correcto para `@Public` y rutas sólo-auth—, así que **olvidarlo no rompe nada visible: deja la puerta abierta**.
- **Logística: 8 controllers sin una sola línea de autorización** (45 de las 61) — flota, embarques, guías, **nómina**, config, checklists, fotos y gastos. Cualquier autenticado podía crear, editar y **borrar** unidades, choferes, embarques, guías, periodos de nómina, bonos y multas. La contraparte: **15 claves del enum que ninguna ruta exigía** (`LOGISTICS_GUIDES/EXPENSES/PAYROLL/CONFIG_*`), o sea casillas en `/admin/roles` que no otorgaban nada mientras las rutas que debían proteger estaban abiertas. Las pantallas `/logistica/*` sí estaban gateadas: **la autorización de Logística vivía sólo en el frontend.**
- **El mapeo se midió contra prod antes de cerrar**, para no dejar a nadie sin trabajar: `GET fleet/vehicles` acepta también `REPARTO_DESPACHAR` (los 9 de `/reparto` eligen la moto ahí y no tienen `FLEET_VER`), y las 5 transiciones de ruta del embarque aceptan `REPARTO_ENTREGAR` (los 2 `repartidor` tienen `SHIPMENTS_VER` pero no `_GESTIONAR`). Guías no necesitó excepción. Borrar evidencia y lo administrativo quedan en `_GESTIONAR` solo.
- **`tenants-admin`: el TODO del primer sprint seguía abierto.** Cualquier autenticado podía crear un tenant, listarlos todos y **desactivar** el de Mega Dulces. Nuevo `PlatformAdminGuard` con lo que ese TODO ya proponía: secreto de despliegue (`PLATFORM_ADMIN_KEY` por header) —los roles son **por tenant** y no pueden autorizar algo cross-tenant— más sesión de admin. **Fail-closed sin el secreto.**
- **Otras 5 cerradas:** `cron/cleanup` (borrado masivo de imágenes), `alerts/test` y `alerts/scan-now` (su propio resumen decía *"admin only — escanea TODOS los tenants"* y nada lo hacía cumplir), `reports/optimize-stops` (pega contra Mapbox, se paga por llamada) y `planograms/match-skus`. Las 6 que quedan sólo-auth son self-scoped y ahora **lo dicen en el código**.
- **Cero permisos invisibles.** 4 claves estaban en el enum y fuera de `authz-tree`: el guard las exigía y `/admin/roles` no las podía otorgar (`COMPRAS_360_VER`, `WHATSAPP_BOT_VER/_GESTIONAR`, `FINANCE_RECON_RECIBIR`). Catálogo ahora **164 = 164 = 164 = 164**.
- **Resultado: 61 → 11 escrituras abiertas**, y las 11 justificadas. **No requiere migración ni re-login**: los permisos ya estaban repartidos en prod, lo que faltaba era exigirlos.

### Internal
- Smoke **estático** `test-authz-route-coverage.js` (18/18, en la regresión): toda escritura declara permiso **o está en una lista blanca con el motivo escrito**, los 8 controllers de Logística ruta por ruta, el catálogo cuadra sin invisibles, y CASL sigue retirado. Un test de runtime no servía (474 escrituras × N roles): lo que se vigila es la **forma del código**, que es donde vive el olvido.
- ⚠️ Al desplegar, los 2 `repartidor` y los 9 de `/reparto` son el caso de prueba. Si algo se cerró de más el síntoma es un **403 en una pantalla que antes funcionaba**, y el arreglo es el permiso que falta — no quitar el guard.

### Removed — Autorización: **CASL se retira; el permiso vuelve a ser una clave** (Fase AUTHZ / ADR-054, 2026-09-02)
- **De dónde salió:** revisando `/admin/users`, Edgar preguntó *"¿qué tan bien se está usando CASL?"*. La respuesta medida fue: no se estaba usando. El backend ya gateaba por **clave exacta** desde meses antes (`RolesGuard`), porque el modelo `(action, subject)` colapsaba permisos que el negocio distingue — `ORDERS_VER` abría `ORDERS_FULFILL/CANCELAR`. Lo que quedaba de CASL alrededor sólo aportaba modos de falla.
- **Cuatro compuertas estaban muertas.** De los 32 permisos `*_GESTIONAR/CONFIGURAR`, **sólo `ROLES_CONFIGURAR`** concedía la acción `'manage'`; los otros 31 concedían `['read','create','update','delete']`. Y en CASL `manage` es comodín del lado de la **regla**, no de la consulta (verificado corriendo el CASL del repo: `can('manage','users')` = `false`). Escondía los controles de gestión de **usuarios, catálogos, parámetros de scoring y planogramas** para todo rol no-admin.
- **Con víctimas reales en prod: 5 usuarios activos** tenían un permiso que el sistema no honraba — `jefe_marketing` (2) y `supervisor_ventas` (3). En catálogos no era un botón escondido sino **403** (`catalogs.controller` decidía con `can('manage','catalogs')`). `USUARIOS_GESTIONAR` se salvó por casualidad: hoy sólo lo lleva `superadmin`.
- **La traducción era incompleta por construcción:** **51 de las 164** claves del enum no tenían subject y `buildAbility` las salteaba en silencio. Eran las de las fases nuevas (FISCAL 17, FINANCE 15, COMMERCIAL 7, STORE 5); por rol, `marketing` tenía 38 claves sin regla, `contabilidad` 29. Esos módulos funcionaban **porque** el guard ya no usaba CASL.
- **Cero `conditions`/`fields`** en todo el repo — o sea cero uso de lo único que justifica la librería. El alcance por fila ya vive en `ScopeService` + `identity.user_scopes` (ADR-050).
- **Qué se borró:** `buildAbility`, `ability.factory.ts`, `ability.types.ts`, `can-do.directive.ts` (cero usos), los mapas `permissionToSubject`/`permissionToAction` (~113 entradas cada uno) y `@casl/ability` del `package.json` y del lockfile. Sobrevive `isPlatformAdminRole` en `ability/platform-admin.ts`.
- **Qué lo reemplaza:** `PermissionsService.has()` / `hasAny()` / `has$()` / `isAdmin()` sobre el mapa de permisos — la misma verdad que enforcea el backend. **`has()` corta por rol de plataforma antes de mirar el mapa**, indispensable porque el rol `superadmin` tiene el mapa **vacío** en prod (11 cuentas): su acceso siempre vino del nombre del rol.
- **El JWT adelgaza ~3.5 KB.** Llevaba `permissions` **y** `rules`, la misma verdad dos veces: `marketing` = 4,901 B + 3,554 B → ~11.6 KB de token (de ahí el `large_client_header_buffers 4 32k` de los 3 nginx), y de sus 65 reglas el front consultaba 3 pares.
- ⚠️ **Cambia lo que se ve.** Aparecen los controles que `can('manage', X)` escondía y desaparece el nav que se mostraba por colapso de subject (la acción rápida *Registrar visita* se le ofrecía a quien sólo tenía `VISITAS_VER`). **No requiere re-login** (`permissions` y `role_name` ya viajaban en el JWT). Pendiente: validación en navegador + redeploy de las 4 apps.

### Fixed — `getDataScope` lee permisos frescos, no reglas congeladas (2026-09-02)
- Dejó de reconstruir una ability desde `user.rules` para preguntarle `can('read','reports_global')`. Ahora lee `req.user.permissions`, que **el guard relee del cache en cada request**; `rules` viajaba en el token y quedaba congelada hasta el próximo login. Efecto colateral bueno: **un permiso de reportes revocado se respeta al instante** — antes no. Equivalencias verificadas 1:1 contra el `ability.factory` que se borró, y el god-mode de plataforma pasó a ser explícito en vez de depender de que la ability se hubiera construido.
- `RequesterContext`/`AuthUser` del módulo de usuarios declaraban `rules?: unknown[]` (campo muerto) y **no** `permissions`/`role_name`, de los que ese alcance depende. Andaba sólo porque en runtime llega el `req.user` completo: nada impedía que un caller armara `{ sub, username }` y el padrón cayera **en silencio a `own`**.

### Internal
- El retiro fue en **3 fases** (backend → UI → borrado) a propósito. Al dejar de emitir `rules`, el `if (payload.rules)` que envolvía el `perms.load()` de `vendor` y `portal` se volvió falso y las dos apps quedaban con **cero permisos** — un bug de runtime que el type-check no ve. Retirar el productor antes que sus consumidores es la forma de encontrarlo en producción.
- Verificado: `tsc` api limpio · `nx build` **verde en view, vendor y portal** · cero `.can(` en los 3 frontends · cero referencias a `@casl` en el repo · los 3 `permissions.service.ts` idénticos (md5).
- Queda abierto de la misma revisión de `/admin/users`: el padrón se acota por permisos de **reportes** y no de usuarios (un rol de RH con `USUARIOS_GESTIONAR` y sin reportes vería sólo su propia fila), y `authz-tree` declara `view: USUARIOS_VER` mientras la ruta y el sidebar exigen `USUARIOS_GESTIONAR` para *ver*.

### Fixed — Ventas por ruta · **el incentivo pagaba sobre una cantidad que mezclaba piezas, paquetes y cajas** (RR-PROMO.1, 2026-09-02)
- **Qué estaba mal:** el panel "Incentivo por enunciado (AI)" de `/comercial/ventas-por-ruta` sumaba `qty` a ciegas. La venta de ruta registra cada línea en el peldaño en que se vendió (pieza / paquete / caja) y **el rótulo no distingue**: el mismo `'PZA'` del SKU `70031` trae 361 líneas a $6.12 (pieza) **y 45 líneas a $90.96 (paquete de 16)**. Es la cantidad con la que se le paga a la gente.
- **Tamaño medido en prod (ago-2026, venta de ruta):** la suma cruda **subcontaba 9.1%** (223,394 contra 243,626 reales) y en **140 SKUs que son $1,110,809 = 19.0% de la venta de ruta** el error pasaba de 5% — `97245` al 42%, `97244` al 43%, `88045` al **84%**.
- **Cómo se arregla:** nueva vista `analytics.v_product_unit_ladder` (mig `20260902180000`) — la escalera de unidades por SKU (`kdii` `c11`/`c80`+`c81`/`c83`+`c84` + precios `c90`/`c91`/`c92`). El motor identifica el peldaño de cada línea por el **precio realmente cobrado**, no por la etiqueta: funciona porque los peldaños distan ≥ el factor (≥2×), mucho más que cualquier descuento. Cobertura **100.0% de la venta de ruta** (19 SKUs / $1,877 sin SKU en el maestro).
- **Lo que no se puede resolver se declara, no se inventa:** fuera de la banda 0.5×–2× la línea queda sin peldaño y **no se suma** (medido: 0.17% de las líneas / 0.11% del importe). La pantalla, el XLSX y el PDF traen la nota de unidad; los clientes que no califican por líneas sin resolver se reportan aparte como *indeterminados* en vez de contarse como que no compraron.
- **La cantidad ya no se llama "Piezas".** Viaja con su rótulo real (`PAQ`, `PZA`, `CJA`…), porque para buena parte del catálogo la unidad base **es el paquete**: en `97192` la base es `PAQ` y `c84=24` son *paquetes* por caja, no piezas. Granel marcado: en producto de peso la cuenta por unidades no se publica.
- **Cumple la regla principal del proyecto** (cero importers · del ODS · una tabla principal · documentada y verificada): la vista es `derive-no-copy` **sólo sobre `kepler_ods.kdii`** — no toca `catalog.products`, ni la etiquetera, ni `analytics.product_box_factor` (que es tabla alimentada por importer). Candado en el smoke que verifica justamente eso.
- La misma normalización se aplicó a `evaluateIncentive` (la tool `thot_incentivo`), que tenía una copia del SQL: si las dos rutas de código dieran cantidades distintas para la misma promo, no serviría ninguna.

### Internal
- Nuevo smoke `test-newdb-route-promo-units.js` (21 aserciones, en la regresión): la vista es VISTA y sólo depende de `kepler_ods`, la basura del campo unidad (`'500'`, `'250'`, `'2KG'`) nunca se publica como unidad, un factor de 1 no es peldaño, el peldaño elegido es el más cercano en precio (blinda el `ORDER BY`), lo no resuelto no se suma, granel marcado. Los 4 candados de cantidad se ejercieron contra prod (59,055 líneas, corrección +7.6%); en local se saltan porque no hay venta de ruta.
- Detectado de paso: hay líneas de ruta con **fecha futura** en la fuente (`max(business_date)` = 2026-12-05 con el reloj en septiembre). El motor ya las corta con `business_date <= CURRENT_DATE`; el smoke ancla su ventana igual.

### Added — Contabilidad · **Movimientos no asociados**: lo que falta por contabilizar, en TXT para ContPAQi (LC.6.1, 2026-09-02)
- **El propósito del módulo, dicho claro:** sacar los movimientos que ContPAQi **no tiene atados a ninguna póliza**, en TXT, para que contabilidad los suba y cierre el trámite. Lo que estaba construido era "la póliza completa del mes"; esto es "lo que falta por asociar" — un conjunto mucho más chico, accionable, y que trae su propio criterio.
- **Cierra LC.2, que llevaba desde el 2026-09-01 bloqueado.** La pregunta estaba mal planteada: no había que *predecir* qué factura entra al libro (el mejor clasificador daba 42.9% de precisión y todos los discriminantes del CFDI quedaron descartados uno por uno) — había que **preguntarle a ContPAQi qué no está asociado**. No hizo falta la contadora.
- **Cuál señal sirve, verificado con las tres a la vez:** el flag `Documento.IsAsoContabilidad` del ADD y la evidencia real (`AsocCFDIs`, el SoR de la asociación) **coinciden dentro del 1%** (jun-2026 175 vs 181 · jul 441 vs 448 · ago 724 vs 722). La que **NO sirve** es `analytics.gl_poliza_lines.cfdi_uuid`: nuestro espejo tiene **18,979 de los 504,365 UUID asociados (3.8%)**, así que habría reportado miles de "no asociados" falsos. Por eso el flag se persiste en `fiscal.cfdis.aso_contabilidad` en vez de derivarse del espejo; `NULL` es "no se sabe", no "no asociado".
- **La trampa que evita duplicar $32.6M:** **271 CFDIs de 2026 no tienen marca de asociación pero su importe YA está en la póliza del mes** (jul solo: 150 por $17.9M). La causa es nuestra — el TXT no lleva UUID, así que ContPAQi contabiliza la factura y nadie la asocia. Con el criterio ingenuo (`IsAsoContabilidad = 0` a secas) se habrían vuelto a mandar. El criterio final tiene **dos condiciones**: sin marca **y** su importe no aparece ya como abono a proveedor (`212*`) en el Diario folio 1 del mes. Es heurístico por importe, así que se marca y **excluye por default**, se muestra con su razón a la vista, y el generador **bloquea** (no avisa) si alguna quedó incluida.
- **Lo que falta, medido en prod:** 4,166 CFDIs sin asociar; en 2026, **1,772 por $61.5M**. Hasta junio es goteo ($0.6–1.7M/mes); **jul-2026 son 291 por $2.5M** y **ago-2026 son 724 por $48.2M** — ese mes no tiene póliza de compras en absoluto. El complemento **cuadra al centavo** en los dos meses grandes (Δ 0.00, validado DB-direct): jul 199 facturas/$1,265,797.22 · ago 515/$46,133,945.25.
- **Cómo quedó:** un sub-módulo, no otro módulo — mismo motor, mismos permisos (`FISCAL_PURCHASE_BOOK_*`), otro alcance. `TipoCorrida = 'libro' | 'complemento'`, `finance.purchase_book_runs.tipo`, folio 2 para el complemento (el 1 es siempre el registro de compras y el 2 está libre en todos los meses medidos). Pantalla `/contabilidad/movimientos-no-asociados`, primera en el tab bar. Endpoints bajo `/finance/purchase-book/no-asociados` — declarados **antes** de `@Get(':mes')` o Nest se los come como si fueran un mes.
- **Perf:** el tablero pasó de **4,335 ms → 376 ms** al pre-agregar las patas de póliza por `(mes, importe)` (en vez de un `EXISTS` correlacionado por CFDI) y acotar el rango a los meses que muestra. Índices nuevos: `fiscal.cfdis_no_asociados_idx` y `analytics.gl_poliza_lines (tenant_id, upper(cfdi_uuid))`.
- **En prod:** migs `20260902120000` (batch 247) y `20260902130000` (batch 248) aplicadas una por una con `migrate:up`, y el flag backfilleado con el rango completo 2018→hoy (167,224 CFDIs). Builds api+view verdes. **Falta: desplegar el código.** Sin permisos nuevos → sin re-login para esta pantalla.

### Fixed — Contabilidad · el Libro de Compras estaba en prod y **nadie podía abrirlo**: faltaba repartir su permiso (LC.6.2, 2026-09-02)
- El par `FISCAL_PURCHASE_BOOK_VER` / `_GESTIONAR` nació con el módulo pero **nunca lo repartió nadie** — sólo existía en el enum de `permissions.ts`, sin seed ni migración. En prod la única fila de `role_permissions` con esas claves era `almacenista`… **en `false`**: residuo de guardar el mapa completo desde `/admin/roles`, que escribe todo el JSONB y deja las claves nuevas del enum en `false`. Resultado: **cero roles con acceso**; sólo `superadmin`/`admin` entraban por `ALL_PERMS`, y quien copiara la URL recibía 403 (el `permissionGuard` de la ruta y los `@RequirePermissions` del controller exigen lo mismo).
- **Mig `20260902140000` (batch 250)**, con el alcance calcado del hermano del mismo proyecto (`FISCAL_CONTAB_*`, que gatea Pólizas y Contabilidad electrónica): quien ya **ve** pólizas ve el libro; quien las **gestiona** genera el TXT. El criterio se lee del estado vivo de prod, no se inventa. `_VER` → 8 roles (auditor_externo, contabilidad, credito_cobranza, direccion, finanzas, gerente_compras, marketing, superadmin) · `_GESTIONAR` → 6 (los mismos sin auditor_externo ni direccion, que quedan de lectura). Los `retirado_*` se excluyen a propósito.
- Aditiva y no destructiva (`permissions -> 'KEY' IS NULL`, nunca el operador `?` de JSONB): el `false` explícito de `almacenista` **no se pisa**. Simulada primero en una transacción con `ROLLBACK` contra prod para ver a quién tocaba antes de aplicarla. **Los usuarios afectados tienen que volver a entrar** — los permisos viajan en el JWT.

### Internal — prod al día en migraciones (batch 249) + se desmintió el "backlog de 91" (2026-09-02)
- **El aviso de "91 migraciones pendientes en prod" era falso y lo corrijo.** Salió de un `migrate:list` que corrió con `DATABASE_URL_NEW` vacío: **`$FLEET_DB_URL` no existe en el shell** —vive dentro de `.env` y sólo aparece después de que dotenv corre *dentro de node*—, así que el prefijo `DATABASE_URL_NEW="$FLEET_DB_URL"` pasaba cadena vacía y el CLI midió `localhost:5433`. El mismo footgun mandó un backfill de 29,756 CFDIs a la base equivocada sin un solo error visible (el script dice "✅ escritos"). Documentado en la memoria de topología de DBs; para apuntar a prod hay que resolver la URL dentro de node y relanzar con `spawn`.
- **Prod tenía 2 pendientes, no 91. Aplicadas (batch 249) → 0 de 541.** Las dos revisadas antes de correr: ninguna borra tabla ni columna.
  - `20260901170000_mv_wincaja_enriched.js` — **no-op**, como debía: su guard detectó que el matview ya está enriquecido y no lo dropeó. Verificado antes y después: `analytics.mv_wincaja_sales_daily` sigue con sus **4,009,787 filas / 1,354 MB**, 23 columnas incl. `branch_name`, y responde ($23.96M de venta wincaja en ago-2026 por sucursal). Sólo quedó registrada, que era el objetivo (así el deploy no la re-corre).
  - `20260902120000_grant_inventory_recibir_to_supervisor.js` — otorgó `COMMERCIAL_INVENTORY_RECIBIR` al rol `supervisor` (1 fila). Aditiva: sólo donde la clave era `NULL`, sin pisar nada; `SUPERVISAR` ya lo tenía, así que no hubo escalación. **Los supervisores tienen que volver a entrar** — los permisos viajan en el JWT.

### Fixed — `/comercial/sell-out` tardaba 2.6 min
...
### Added — Caducidades de tienda: se registra por producto y cada uno genera su hoja de expediente (P2.6.9, 2026-09-08)

- **El alta por "hoja" se fue de `/tienda/caducidades`.** El colaborador de sucursal no maneja ese concepto: escanea un producto, teclea fecha y cantidad, guarda, y la pantalla queda limpia para el siguiente. Cada alta entra **ya cerrada** y se refleja en el control de caducidad (FEFO) al guardarse — antes quedaba un borrador esperando que alguien lo cerrara, y si nadie lo hacía el dato no llegaba a inventario nunca.
- **La sucursal no se elige: se hereda** de la ficha del usuario. El server la resuelve y devuelve su nombre (el token sólo trae el código de 2 dígitos, y el catálogo de almacenes pide un permiso que el colaborador no tiene). Sin sucursal en la ficha, la pantalla **lo dice al abrir** en lugar de fallar con un 403 cuando ya se tecleó toda la captura.
- **Dos oficios, partidos por permiso:** `COMMERCIAL_EXPIRY_CAPTURAR` da de alta y ve *lo que capturó hoy*, con corregir y borrar; `COMMERCIAL_EXPIRY_VER` ve además el expediente de su sucursal.
- ⭐ **Cada producto genera su hoja de expediente, con folio propio** — `CAD-<sucursal>-<año>-<NNNNN>`, consecutivo por sucursal y año. `/tienda/caducidades/expediente` archiva por sucursal (8 Esquinas, Padre Hidalgo, Zamora Centro, Canindo…) con una card por cada una — hojas, vencidos, por vencer y última captura, incluidas las que no tienen nada, porque un expediente vacío también es información. `/tienda/caducidades/hoja/<folio>` es el **formato imprimible**, con líneas de firma de quien levantó y del encargado; el diálogo del navegador ya ofrece "Guardar como PDF", así que no depende del almacenamiento de archivos (los `S3_*` siguen vacíos). Migración con **backfill**: lo ya capturado recibe folio en orden de captura, y el contador arranca donde termina.
- **La fecha se teclea en dígitos pelados** (`0327` = fin de marzo 2027) reusando el intérprete del Andén, en vez de pelear con un datepicker: se captura de pie, con el teléfono en una mano.
- **Corregir es reversible de verdad:** borrar o editar una alta devuelve al lote sin fecha lo que había fechado, y **se niega si ya salió mercancía de ese lote** — devolver algo que ya se despachó inventaría producto.

**Tres cosas que estaban mal y se vieron al partir los permisos:** escanear y buscar por nombre exigían el permiso de *ver*, o sea **el colaborador no podía hacer su trabajo**; el buscador general pedía un permiso de catálogo entero para teclear un nombre (ahora el módulo trae el suyo); y el historial **no filtraba por sucursal** — cualquiera con *ver* veía las 7, y filtrarlo en la pantalla habría sido un adorno sobre una API abierta. Además el colaborador aterrizaba en *Etiquetas*, cuyo guard lo rebotaba: el mismo síntoma que ya había tenido la cajera.

⚠️ **Smoke y validación visual pendientes** (sin base ni API arriba en esa sesión). **Prod:** falta la migración del folio, redeploy, asignarle el permiso **y la sucursal en la ficha** al rol del colaborador, y re-login.

### Added — Caducidades: se escanea o se le habla (P2.7, 2026-09-08)
- **El formulario dejó de pedir 8 campos en blanco.** De esos 8, **6 no son datos del anaquel**: son ficha del producto (presentación, unidad, ubicación) u opcionales (observación, acción, foto). Lo único que agrega el anaquel es **cuánto hay** y **qué fecha marca el empaque**. Ahora son dos pasos: *¿qué producto es?* (escaneo, buscador o voz) → **Cantidad y Fecha** grandes, con el resto ya prellenado y plegado en *Más detalles*.
- **Asistente por voz.** Se toca el micrófono y se dice *"tengo 3 cajas de mazapán que caducan en octubre"*; el asistente pregunta lo que falte y va llenando el renglón. Cadena: `MediaRecorder` → **Groq Whisper** (el endpoint de dictado de Thot, reusado tal cual) → **Claude Haiku 4.5** con *forced tool* → campos. Contesta en voz alta con la voz del navegador (gratis, silenciable). Tres chips (Producto / Cantidad / Caducidad) muestran lo que ya entendió: el operador sabe qué le falta decir sin escuchar toda la respuesta.
- **El producto lo resuelve el catálogo, nunca el modelo.** El LLM solo dice el NOMBRE que escuchó; la búsqueda contra `public.products` es determinista y si hay varios, elige la persona. Un LLM inventando UUIDs metería mercancía equivocada al sub-ledger FEFO. Tampoco escribe el renglón: prellena y el humano confirma (co-piloto, ADR-020).
- **Se valida lo que dice el modelo:** unidad contra el enum, fecha contra calendario real (mata el "31 de febrero") y contra ±5 años ("dos mil sesenta y dos" es error de dictado, no un dato). Lo que no pasa, se descarta y se vuelve a preguntar.
- **Cazado al cablear: el dictado le habría dado 403 justo a quien lo va a usar.** El proxy de voz que ya existía (el de Thot) está gateado con `COMMERCIAL_ORDERS_VER`, y el colaborador de caducidades lleva `COMMERCIAL_EXPIRY_CAPTURAR` y nada de ventas. La transcripción se extrajo a `SpeechToTextService` en `platform-core` (infra leaf) y **cada dominio la expone con SU gate** — en vez de repartir un permiso de ventas para poder hablar.
- **El asistente quedó en la pantalla nueva de tienda** (`/tienda/caducidades`, captura directa un-producto-a-la-vez): escribe los mismos 3 pasos y la fecha la sigue interpretando `parseExpiryShort`, así que no hay una segunda forma de decidir qué fecha se guarda.
- **Sin llaves configuradas no se rompe nada:** el panel dice el motivo real (`GROQ_API_KEY` / `ANTHROPIC_API_KEY`) y la captura sigue por escaneo o a mano. Cámara y micrófono **exigen HTTPS**: por `http://IP` de LAN el navegador no los da, y ambos lo explican.

### Added — Caducidades: escanear para dar con el producto (P2.6.8, 2026-09-07)
- **La hoja de anaquel ahora se captura escaneando.** Tres caminos, un solo campo: la **pistola de la cajera** (lector HID — teclea el código y manda `Enter`), la **cámara del teléfono** (`@zxing/browser`, formatos de retail) y el **código tecleado** para etiqueta rota o código de anaquel. Un campo y no tres a propósito: dos inputs peleando por el foco es lo que rompe una pistola en modo wedge. El foco vuelve solo tras cada lectura y tras guardar renglón → captura en ráfaga.
- **Endpoint nuevo `GET /commercial/expiry-reviews/resolve?code=`**, que lee **`catalog.product_barcodes` (1→N)** y no `products.barcode`: esa columna escalar guarda sólo el EAN de la **pieza**; el de la **caja** — el que más se escanea en bodega — no está ahí. Normaliza además **UPC-A 12 ↔ EAN-13 con cero al frente** (según cómo esté configurada la pistola entrega una u otra).
- **Efecto de paso: la unidad deja de adivinarse.** Como la tabla trae `unit`+`factor`, el código leído dice si se escaneó caja o pieza; eso reemplaza a la heurística "código numérico = caja", **que se equivocaba** (un EAN de pieza también es numérico) — y esa cantidad alimenta FEFO al enviar la hoja.
- Sin match **no es un error**: la hoja lo guarda como renglón raw (no alimenta FEFO) y lo dice. Código ambiguo tampoco corta: ofrece los candidatos.

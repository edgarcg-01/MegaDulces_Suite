/**
 * Fase CXC — Cartera: DEDUP defensivo en la vista `analytics.customer_receivables`.
 *
 * Hallazgo (2026-08-22, verificado en prod): `kepler_ods.kdue` trae **filas duplicadas
 * exactas** (2 ctid para el mismo documento) — artefacto de la replicación per-branch→ODS
 * (ver [[feedback_per_branch_to_ods_single_db]]). 2,365 grupos duplicados. La tabla vieja
 * los colapsaba por su PK (suc,doc,folio); la VISTA no → doble-conteo. Como la mayoría de
 * duplicados son abonos (Cobro CFDI), se **doble-restan** y **subvalúan el saldo** en las
 * sucursales afectadas (la 01 no tenía dups → cuadró; la 02 sí). Mismo riesgo en `kdm5`
 * (sum(c13) se dobla si hay filas repetidas → over-reduce saldo_documento).
 *
 * Fix contenido en la vista (NO tocar el pipeline ODS): dedupe por identidad de documento
 * en kdue (DISTINCT ON suc+género+doctype+serie+folio) y dedupe exacto de filas en kdm5.
 * Misma firma de columnas → CREATE OR REPLACE VIEW. PROD-ONLY (kepler_ods vacío local).
 *
...
WITH src AS (
  ...
  WHERE (c29='C' AND btrim(c4::text) IN ('8','12','13'))
     OR (c29='A' AND btrim(c4::text) IN ('7','21','25'))
...
m0 AS (
  ...
  WHERE c2='U' AND btrim(c4::text) IN ('7','21','25')
),
ap AS (
...
exports.down = async function () {
  // No revierte: la vista deduplicada es estrictamente más correcta que la anterior.
};

/**
 * VECINAL (RV) como canal propio `preventa` en la cadena del sell-out —
 *   mv_kepler_sales_daily → v_sellout_daily → mv_sellout_monthly.
 *
 * Por qué: la venta VECINAL (reparto tiendita-a-tiendita, `kduv.c3`='RUTA VECINAL …', código
 * `1V0NN`/`3V001` en `kdm1.c12`) estaba clasificada como canal `ruta` (RD), MEZCLADA con las
 * camionetas. NO faltaba ni se doble-contaba: estaba mal etiquetada. Medido prod (agosto-2026):
 * mv_kepler `ruta` = 100% vecinal (PH 1V001 $901k + 1V002 $428k = $1.33M; Zamora 3V001 $263k;
 * Yurécuaro 1V004 $201k; La Piedad Abastos 1V003 $224k). Las camionetas (RUTA 21-28, almacenes
 * `RUTA-%`) NO viven en mv_kepler — vienen de la pierna de rutas de v_sellout → quedan intactas.
 *
 * Cambio de fondo: SÓLO `CHANNEL_EXPR` en mv_kepler — detectar vecinal ANTES del `ruta` genérico y
 * asignarle `preventa` (RV). Es un RELABEL: ningún total de sucursal cambia (se redistribuye
 * ruta→preventa). El resto (createView/createRollup) es VERBATIM de 20260905120000 (la definición
 * vigente con monto_neto). Sin window ni join nuevo → el refresh de mv_kepler se queda en ~35s.
 *
 * Idempotente: guard por la presencia de 'preventa' en la definición de la matview. Recrea los 3 en
 * cadena (mv_kepler DROP CASCADE tira vista+rollup) + refresh mv_kepler. NO editar una vez aplicada.
 * @param { import("knex").Knex } knex
 */

const T = `'00000000-0000-0000-0000-00000000d01c'::uuid`;

// ── VERBATIM de 20260905120000 (si tocás allá, tocá acá) — salvo CHANNEL_EXPR ──
...
// Camioneta (RD) — nombre 'RUTA NN' o código legacy 1V (queda como fallback; hoy 0 filas no-vecinal).
const IS_ROUTE = `(btrim(v.c3) ILIKE 'RUTA %' OR btrim(v.c3) ILIKE 'RUTA VECINAL%' OR btrim(h.c12) ~ '^1V')`;
// VECINAL (RV) — nombre 'RUTA VECINAL …' o código [digito]V[digito] (1V0NN/3V001/2V0NN/10V0N).
const IS_VECINAL = `(btrim(v.c3) ILIKE 'RUTA VECINAL%' OR btrim(h.c12) ~ '^[0-9]+V[0-9]')`;
const CHANNEL_EXPR = `CASE
    WHEN ${IS_VECINAL} THEN 'preventa'
...
async function createKepler(knex) {
  ...
  await knex.raw(`COMMENT ON MATERIALIZED VIEW analytics.mv_kepler_sales_daily IS
    'REGLA #1/ODS: venta KEPLER a grano dia DERIVADA de kepler_ods (kdm1 kdm2 kduv). monto=Sum kdm2.c13 (bruto c/IVA); monto_neto=Sum imp*c16/(c16+desc). Canal: VECINAL (RUTA VECINAL/[0-9]V) -> preventa (RV); resto ruta/mayoreo/credito/mostrador. Refresh nightly + ANALYZE.'`);
}
...
exports.up = async function (knex) {
  // Idempotente: si la definicion de la matview ya asigna 'preventa' (vecinal), ya esta aplicada.
  const has = (await knex.raw(
    `SELECT 1 FROM pg_matviews WHERE schemaname='analytics' AND matviewname='mv_kepler_sales_daily'
       AND definition ILIKE '%''preventa''%'`,
  )).rows.length;
  if (has) return;
...
exports.down = async function () {
  throw new Error('irreversible: forma parte de la cadena sell-out; revertir = re-aplicar 20260905120000.');
};

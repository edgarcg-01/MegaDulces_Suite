 ...
 * (`.stock-live-snapshot.json`) se desincroniza y deja valores FANTASMA para siempre
 * (`88009` en `01`: POS 2485 / ODS 2487 / esta tabla 3547).
 *
 ...
 * a las ventas del intervalo (casi nada fuera de horario). Con FEEDS_SINK=http el
 * delta va por el feeds-ingest (ingress gratis).
 *
 * Modo --watch: cicla cada N segundos (default 15) con la conexión destino persistente
 * → stock "al momento" sin re-arrancar el proceso. El snapshot en disco persiste entre
 * ciclos (sube solo lo que cambió). Reconecta las sucursales por ciclo (robusto a caídas).
 *
 ...
 *   node database/importers/kepler/import-branch-stock-live.js --apply --watch=15  # loop al-momento
 */
...
const WATCH_SEC = WATCH_ARG ? Math.max(5, Number(WATCH_ARG.split('=')[1] || 15)) : 0;
...
/** Un ciclo: lee todas las sucursales → estado deseado → diff vs snapshot → ship delta.
 *  Devuelve stat {status, rows} para el heartbeat. Conexión destino `db` persistente. */
async function runCycle(db, { apply, full }) {
  ...
  if (!syncedCodes.size) { console.log('  ninguna sucursal respondió — nada que hacer.'); return { status: 'error', error: 'ninguna sucursal respondió' }; }
  ...
  if (!changed.length) { console.log('  sin cambios — cero tráfico.'); return { status: 'ok', rows: 0 }; }
  ...
  return { status: 'ok', rows: r.rowCount };
}

(async () => {
  const db = new Client({ connectionString: DST });
  await db.connect();
  if (WATCH_SEC) console.log(`\n=== stock live WATCH ${WATCH_SEC}s · sink: ${sink.sinkMode()} · Ctrl+C para salir ===`);
  try {
    let cycle = 0;
    do {
      cycle++;
      ...
      if (APPLY) await hb.begin('kepler_stock', 'Kepler stock vivo (multi-sucursal)').catch(() => {});
      try {
        stat = await runCycle(db, { apply: APPLY, full: FULL && cycle === 1 });
      } catch (e) {
      ...
      if (APPLY) await hb.end('kepler_stock', stat).catch(() => {});

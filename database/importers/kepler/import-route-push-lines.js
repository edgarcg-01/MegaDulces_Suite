...
(async () => {
  ...
  try {
    ...
    // forma_pago = kdm1.c10 = CLIENTE (columna mal nombrada en el push); 'CONTADO'/vacío = público → NULL.
    const { rows } = await src.query(
      `SELECT substring(sucursal from 'ruta_(.*)') AS route_no,
              fecha::date AS business_date, folio, sku, max(producto) AS producto,
              max(NULLIF(NULLIF(btrim(forma_pago), ''), 'CONTADO')) AS cliente,
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      // DO UPDATE: corrige filas ya cargadas (ej. backfill del cliente) e idempotente en el nightly.

...
(async () => {
  ...
  try {
    ...
    const upc = await db.query(`
      WITH upd_src AS (SELECT DISTINCT ON (btrim(upper(nombre))) sku, btrim(nombre) nombre FROM stg2 ORDER BY btrim(upper(nombre)), sku)
      SELECT s.sku, pt.nombre AS destino_nombre, ph.nombre AS ya_lo_tiene
      FROM upd_src s
      JOIN catalog.products pt ON pt.tenant_id=$1 AND btrim(upper(pt.nombre))=btrim(upper(s.nombre))
        AND (SELECT count(*) FROM catalog.products p2 WHERE p2.tenant_id=$1 AND btrim(upper(p2.nombre))=btrim(upper(pt.nombre)))=1
      JOIN catalog.products ph ON ph.tenant_id=$1 AND ph.sku=s.sku AND btrim(upper(ph.nombre))<>btrim(upper(s.nombre))
      LIMIT 40`, [M]);
    console.log(`
UPDATE crearia sku DUPLICADO en ${upc.rows.length} casos (sku que ya tiene otro producto):`);
    for (const r of upc.rows.slice(0,30)) console.log(`  sku=${r.sku} → "${r.destino_nombre}" pero ya lo tiene "${r.ya_lo_tiene}"`);

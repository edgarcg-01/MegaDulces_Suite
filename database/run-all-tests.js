...
const TESTS = [
  ...
  { file: 'test-newdb-role-rename.js', label: 'ID.32 renombrar un rol es un UPDATE (los 14 roles `retirado_*` de prod no son roles: son NOMBRES LIBERADOS, porque las FK compuestas estaban en ON UPDATE NO ACTION y renombrar reventaba) — son CUATRO FK y no tres: la 4a es positions_default_role_fk, el rol que un PUESTO propone por default + el ON DELETE NO se uniformo a proposito (RESTRICT en users es lo que impide borrar un rol con gente adentro) + los triggers de esas FK ENCENDIDOS por tgenabled y no por convalidated, que ya mintio una vez ([IDG.6]: 144 de 195 apagados en prod) + LC.6.2 cada clave la concede alguien: 174 de 175, con la que falta en lista con MOTIVO escrito, y DECLARA las que concede un rol sin una sola persona activa', needsApi: false },

...
const TESTS = [
  ...
  { file: 'test-mirror-readonly-negative.js', label: 'REP.1.2 el espejo PROD→LOCAL no puede escribirle a prod, comprobado mandando escrituras de verdad contra prod y exigiendo el 25006 del motor. Carga las funciones REALES de pull-prod-to-local.js, no una copia (una copia se desincroniza y el test se queda verde midiendo código que ya nadie corre). Medido el 2026-09-08 sobre PG 18.6: de las tres sondas candidatas, `pg_current_xact_id()` PASA —devolvió un XID— y habría dado verde siempre; `CREATE TEMP TABLE` y el `UPDATE` de 0 filas sí se rechazan. Va el UPDATE porque GOTCHAS §33: un SELECT que funciona no prueba que un UPDATE funcione (ahí un rol "de solo lectura" tumbó prod porque el login SÍ escribía). Si no se llega a prod: NO MEDIDO, nunca verde', needsApi: false },
  ...
  { file: 'test-newdb-receipt-roles.js', label: 'RE.28 el rol de cada hoja, que es lo que hace posible la hoja interna. La worklist que RE.13 volvió el camino principal clavaba `role: factura` porque las reglas vivían DENTRO del otro componente → desde el 27-ago ninguna evidencia pudo declarar la hoja interna y los dos controles de RE.25/RE.26 (`paquete_ok`, `folio_interno`) quedaron muertos en origen 11 días: 93 de 129 comprobantes con hoja antes del corte, 0 de 56 después. Fija que el payload no vuelva a clavar el rol, que las reglas vivan en `receipt-roles.ts` y no duplicadas en un componente, y que todo rol de los datos lo conozca el módulo (uno que no conoce deja de contar para `paquete_ok` en silencio)', needsApi: false },
  ...
  { file: 'test-newdb-role-rename.js', label: 'ID.32+ID.33 renombrar un rol es un UPDATE (los 14 roles `retirado_*` de prod no son roles: son NOMBRES LIBERADOS, porque las FK compuestas estaban en ON UPDATE NO ACTION y renombrar reventaba) — son CUATRO FK y no tres: la 4a es positions_default_role_fk, el rol que un PUESTO propone por default + el ON DELETE NO se uniformo a proposito (RESTRICT en users es lo que impide borrar un rol con gente adentro) + los triggers de esas FK ENCENDIDOS por tgenabled y no por convalidated, que ya mintio una vez ([IDG.6]: 144 de 195 apagados en prod) + LC.6.2 cada clave la concede alguien: 174 de 175, con la que falta en lista con MOTIVO escrito, y DECLARA las que concede un rol sin una sola persona activa + ID.33 el god-mode: las CINCO copias del literal new Set([superadmin,admin]) (platform-core, users.service como ELEVATED_ROLES, y los 3 frontends) tienen que decir lo mismo -- hoy solo las ata un comentario que dice "si cambia aca, cambia alla" -- y coincidir con la columna is_platform_admin cuando este aplicada', needsApi: false },
  { file: 'test-newdb-kind-dispositivo.js', label: 'ID.31 el sujeto se DECLARA (kind gana `dispositivo`: metia 109 personas y 14 credenciales de puesto en la misma bolsa `interno`, y token_ttl_days -la senal que se suponia que las distinguia- esta en NULL en el 100% de las filas) + los 14 se derivan de EVIDENCIA (el nombre es una tienda o el propio username), no del prefijo: las 28 cuentas con username numerico son cajeras con nombre de persona y NO se tocan + password_hash nullable con CHECK -> status=invited deja de ser inrepresentable, que es por lo que no habia flujo de invitacion + PRUEBA NEGATIVA read-only: se saca del catalogo el predicado REAL del CHECK y se evalua con valores sinteticos (interno+active sin hash RECHAZA, invited ACEPTA) sin escribir una fila en prod + el login corta antes de bcrypt.compare, que LANZA con hash nulo', needsApi: false },
  { file: 'test-authz-jwt-size.js', label: 'ID.29 al token solo viajan las claves CONCEDIDAS (el mapa de /admin/roles trae las 175 del enum, casi todas en false, y viaja en el header Authorization de CADA request: almacenista con DOS permisos cargaba 7,746 B de los 8,192 que nginx acepta por default) + el filtro esta en los DOS caminos de login y el primitivo vive en libs/ + PRUEBA NEGATIVA sobre el .ts REAL via ts-node: un mapa todo-en-false colapsa a {} (o sea que el filtro no es la identidad) + mide el header por rol contra prod y DECLARA el techo que el filtro NO baja (superadmin concede 166 de 175: solo sacar el mapa del token lo mueve)', needsApi: false },
  { file: 'test-newdb-padron-limpieza.js', label: 'ID.27 el padron queda limpio y lo declarado sigue declarado (nombre de credencial de ruta coherente con su username + cliente B2B en none DECLARADO en vez de own irresoluble + la 1a baja de la historia del padron con status+terminated_at escritos, no un activo=false mudo + superoot sin rutas + el fixture test_tenant_b declarado y en 0 usuarios) + PRUEBA NEGATIVA x2: fingir la ficha vacia sube el conteo, y vaciar los roles del fixture dejaria a test-authz-tenant-failclosed en NO MEDIDO + DECLARA los que ven 1 sola fila del padron por tener USUARIOS_VER sin reportes', needsApi: false },
  { file: 'test-newdb-scope-resolvable.js', label: 'ID.26 el alcance declara cuando NO se puede resolver (own con la ficha vacia emite el MISMO WHERE false que none: indistinguibles) + PRUEBA NEGATIVA: fingir la ficha vacia sube el conteo, y sin universo se DECLARA en vez de dibujar 0 + publica el riesgo latente de girar a own por dimension', needsApi: false },
  ...
  { file: 'test-newdb-permission-delivery.js', label: 'ID.29 el REPARTO del permiso, no su declaración. `[LC.6.2]` estaba escrito en el encabezado del enum y no lo medía nadie: ya cobró en `FISCAL_PURCHASE_BOOK_*` (un día en prod, cero roles) y en `STORE_PRICE_CHECK_VER` (el verificador de mostrador, cero roles hasta la mig 20260909120000 → 7). Midiendo para escribirlo apareció la tercera, viva 23 días: `COMMERCIAL_PREVENTION_GESTIONAR` gatea 9 escrituras, dirección lo VE y nadie lo OPERA. Mide contra PROD a propósito y SOLO LEE (verde sobre la copia local vieja es justo la mentira que ataja); si no llega, NO MEDIDO nunca verde. Declara la base y la fecha del role_permissions más nuevo (poblado != fresco). Separa "reservado que no bloquea nada" de "gatea escrituras que nadie alcanza" leyendo los @RequirePermissions reales, y la lista de excepciones se poda sola: si una declarada ya se repartió, exige sacarla. Cierra con 4 PRUEBAS NEGATIVAS, incluida la que exige que la lista TAPE algo real en vez de estar de adorno', needsApi: false },
  ...
  { file: 'test-db-health-tz-bias.js', label: 'W4.3 el tablero de salud no publica edades con el sesgo de 6 h: ningún sensor castea last_update a timestamp salvo los 6 declarados con su tipo medido (el cast tira la zona y deja el reloj de la sesión pg en Etc/UTC, que node-pg reinterpreta como MX — la edad sale 6.00 h más joven; tapaba un warn real de 35.4 h en stock_cedis_00 y dejaba a fleet_positions con warnH 3 incapaz de disparar antes de las 9 h) + los 15 sensores por-columna leen columnas timestamptz + PRUEBA NEGATIVA del predicado y del sesgo medido en vivo + guarda anti-no-op: si el parseo deja de reconocer los sensores FALLA en vez de pasar en vacío (ya pasó: la primera versión saltaba en silencio los 4 sensores recién editados)', needsApi: false },
  { file: 'test-wincaja-replica-fidelidad.js', label: 'WR.7+WR.8 la replica cruda de Wincaja es un ESPEJO: ninguna carga escribio menos filas de las que leyo (perdia 19,321 de 146,289,530 por colapso de filas byte-identicas contra la identidad (_dataset,_row_hash)) + INVARIANTE del watermark: la columna de marca es la PK COMPLETA o esta declarada con su motivo (Cortes/Retiros leian WHERE Folio > marca con PK (Folio,Caja) y el folio reinicia POR CAJA -> 5 de 6 cajas ciegas para siempre, 2,939 de 4,193 retiros) + cero marcas huerfanas + paridad exacta de tablas y columnas entre los 33 schemas + PRUEBA NEGATIVA del predicado. Vive on-prem (:5433/wincaja): si no lo alcanza declara NO MEDIDO, no verde', needsApi: false },
  ...
  { file: 'test-newdb-sales-line-units.js', label: 'U.9 el RENGLÓN de venta trae su propia conversión y la vista la LEE sin inventarla (v_erp_sales_line_units). Respuesta a "¿por qué Kepler UI saca bien las unidades y nosotros no?": Kepler NO resuelve la unidad, nunca la pierde — cada renglón de kdm2 trae c11/c9/c12 (la base) y c55/c56/c57/c58 (qué compró el cliente, cuántos, a qué precio, con qué factor), y la pantalla del ERP sólo lo imprime. Invariantes medidos en prod: c9 = c56×c58 al 99.99% de 899,646 renglones ($6,702 no cierra), c13 = c56×c57 al 98.89%, c57/c12 = c58 al 95.58% con mediana 1.0000. El caso 70031 desmiente al propio doc: los 45 renglones "PZA a precio de paquete" que se citaban como prueba de que c11 miente son 3 paquetes de 16 DECLARADOS — leíamos media línea. Candados: NO CIRCULARIDAD (la vista no puede consultar la etiquetera ni el resolvedor, o "el renglón confirma al catálogo" sería tautología), que sin_declarar EXISTA y viaje con NULL y nunca con 1 de relleno, que el anti-réplica deje FUERA la réplica (suc 03 con almacén 02, $7.5M) pero DENTRO los sub-almacenes de ruta, y el hueco declarado con su tamaño: Wincaja no tiene conversión por renglón y su importer no escribe el peldaño, así que rung_factor IS NULL hoy significa TRES cosas y barre el 83% de la venta', needsApi: false },
  { file: 'test-newdb-stock-truth.js', label: 'LA VERDAD DE LA EXISTENCIA DE KEPLER. La CANTIDAD ya cerraba y este candado la protege: la identidad entradas-salidas=qty deja CERO sin explicar (92.22% directa + 1,748 saldos negativos que la vista recorta por diseno), kdil.c4 es 0 en el 100% de las filas (asi que el baseline=0 del dictamen es correcto, no un bug) y la sucursal 00 de Kepler, que deriva 122,096,465 unidades fantasma, ya esta excluida. Lo que NO cerraba es el VALOR: la existencia se valuaba con catalog.products (un costo por PRODUCTO, global, en la unidad que el catalogo tenga) cuando Kepler trae SU costo por sucursal x SKU en kdik.c16, al mismo grano que la cantidad. Medido: cost_base/c16 da mediana 1.0000 (pega +-2% en 72.46%) y cost_with_tax/c16 da 1.0800, o sea cost_base ES el costo de Kepler y la pantalla publica CON IMPUESTO. Brecha total $5,642,385 = 13.21% de lo publicado, y se parte en dos causas separables: $2,795,095 de impuesto (razon 1.0983 en las filas donde el costo YA coincide) y $2,025,427 de FACTOR DE CAJA en 273 filas de granel (razones 16.2, 21.6, 20.0, 14.0, 32.0, 31.4 — ROLLO GUAYABA GRANEL, CHOC HERSHEY BARRA GRANEL 14KG, TURIN CONF SEMIAMARGO 16KG: cost_base viene por bulto, c16 por pieza). ANTI-REPLICA obligatorio en kdik (3,667 de 31,084 filas traen c1 distinto de sucursal); probado en negativo: sin el filtro el valor arbitrado se mueve $864,270 y 592 filas se caen de confirmado. sin_testigo viaja con valor NULL, jamas con 0 de relleno, y las DOS ausencias (falta el testigo de Kepler / falta el costo del catalogo) son etiquetas distintas. La vista NO elige el costo bueno: devuelve los dos y el veredicto', needsApi: false },
  { file: 'test-newdb-kepler-parity.js', label: 'P.0 PARIDAD con Kepler: lo que entregamos tiene que ser lo que entrega Kepler. Compara analytics.sales_daily contra la suma CRUDA de kdm2 por (SKU, sucursal, fecha) y parte el delta por CAUSA. Medido 2026-09-07 (90d, suc 01-06): contra el ticket U-D-10 la cantidad coincide en 98.02% y el importe en 99.74% (delta $727,484) — ahi ya somos casi identicos. La brecha es NUESTRA y tiene cinco causas: falta U-D-8 Factura Telemarketing ($14.5M) y U-D-12 no fiscal ($1.5M) porque mart.ventas filtra c4=10 (y mv_kepler_sales_daily SI las cuenta, asi que el fact y el sell-out se contradicen entre si); 5,136 celdas que Kepler tiene y el fact no; 1,101 celdas multiplicadas x12 por el factor que adivina pickPriceTier; 3,833 divididas /2 y 749 multiplicadas x2 al convertir 500g y 2KG a kilos. Tres trampas que este candado evita porque ya produjeron una medicion falsa: aislar los canales de Kepler (los almacenes 01-06 traen ademas venta historica de Wincaja remapeada, y mezclarla infla el delta de 1.6% a 27%), el anti-replica btrim(c1)=sucursal, y contar los DOS lados del FULL OUTER JOIN', needsApi: false },
  { file: 'test-newdb-wincaja-unit-audit.js', label: 'P.1 Wincaja DESCRIBE su unidad (v_wincaja_unit_audit) — y la RETRACTACION de un defecto que yo invente. La vista publicaba caja_sin_capturar para 1,286 SKUs ($1.5M) con una regla apoyada en unidad_compra (constante CJA en 94.53%) y factor_compra (1 en 15,535 de 15,535): dos campos que Wincaja NO mantiene. El dinero lo desmiente — el precio de esos articulos es precio de CAJA en 95.5% de los casos, igual que el grupo que Wincaja rotula CJA; factor_venta=1 es CORRECTO porque su unidad de venta ES la caja. El candado viejo EXIGIA que ese numero falso fuera > 500. Ahora afirma lo contrario: que caja_sin_capturar no vuelva, que los fv<=1 esten precificados como CAJA (>=80%) y los fv>1 por UNIDAD (>=80%) — los dos grupos se separan solos —, que divisor_wincaja nunca sea NULL, y que el par de compra siga sin mantenerse. Premisa verificada: Wincaja no tiene unidades incorrectas (0 de 46,577 con mas de una unidad, factor_venta consistente entre ramas en 99.92%). Limite declarado: sin rotulo PAQ, pieza-vs-paquete no es decidible dentro de Wincaja', needsApi: false },
  ...
  { file: 'test-newdb-goods-receipts-aging.js', label: 'RE.3 el calendario de pago y lo que NO debe publicar: NO existe la liga recepcion->pago (erp_supplier_payments no trae folio de entrada, expense_doc_chain vacia) asi que no se sabe que ya se pago; 10,940 recepciones tienen vencimiento pasado por $507M y casi todo esta pagado -un aging ingenuo publicaria esa deuda falsa-, por eso solo se lista lo que AUN NO vence y lo vencido va como numero declarado sin lista; ademas las gemelas (dup_of_folio) quedan fuera o se pagaria dos veces la misma compra', needsApi: false },
  { file: 'test-newdb-goods-receipts-vencimiento.js', label: 'RE.1 el ancla sabe CUANDO se paga (fecha_vence c18 + condicion_pago c30 + dias_credito): el vencimiento viene siempre en Kepler (12200/12200) y casa con el plazo declarado en 99.92% dentro de +-3 dias -la tolerancia ES el dato: "30 dias" en Kepler es UN MES DE CALENDARIO, da 31 en 711 docs y 28 en 111, por eso c18 se guarda cruda y NO se deriva del texto-; ninguna "Pago de contado" (el 68%) figura a plazo o RE.3 inventaria deuda; el dias_credito negativo se conserva crudo (calidad de dato del ERP, no se clampa); y agregar columnas no hizo fanout', needsApi: false },
  ...
  { file: 'test-newdb-bulk-gate-ajuste.js', label: 'RE.21.2 la puerta nueva del lote "cuadra con ajuste" (trx+ROLLBACK sembrando ajustes): abre solo con ajuste LIGADO a esta entrada y del tamaño del hueco -compara magnitud y no signo, porque la direccion contable no es estable-, y NO abre si esta ligado a otra entrada, si el monto no casa, si el hueco es 0 ni con un ajuste de $0; con varios gana el mas cercano', needsApi: false },
  { file: 'test-newdb-adjustment-lines.js', label: 'RE.22.1 desglose de un ajuste (vista viva erp_purchase_adjustment_lines sobre kdm2): las devoluciones X-D-40 SI traen renglones (766 en 235 docs) y las notas X-D-55 NO (0 sobre 1,256 encabezados) porque una nota es dinero y no mercancia -por eso el contrato tiene 3 valores: `no_aplica` (correcto) vs `sin_dato` (falta info), y confundirlos declararia faltante el 90% del dinero de los ajustes-; anti-replica c1=sucursal sin claves repetidas; el n de linea ordena por su valor numerico y no como texto; un documento abre en <1s', needsApi: false },
  { file: 'test-newdb-adjustment-explain.js', label: 'RE.21 premisas del match por MONTO: la liga por entrada_folio (c39) cubre el 4% y en las X-D-55 es CERO de 1,256 -las notas son el 90% del dinero-; factura_ref (c11) NO es llave pese al nombre (el 29% trae el codigo de proveedor); el ajuste llega DESPUES el 63% de las veces (el cuadre al capturar es una foto); y va del 0% al 100% de la entrada, o sea que no hay regla de signo y hay que comparar magnitudes', needsApi: false },
  { file: 'test-newdb-goods-receipts-write-scope.js', label: 'RE.20.6 el alcance manda al ESCRIBIR: nadie con COMPRAS_ENTRADAS_VALIDAR queda sin regla de alcance (el default de mode_write es `none` = fail-closed, así que uno sin regla se queda sin poder validar NADA en cuanto entra el guard), los acotados por `own` tienen warehouse_code al que apuntar, y se listan los que quedan limitados a su sucursal', needsApi: false },
  ...
  { file: 'test-newdb-libro-compras-caratula.js', label: 'LC.10 carátula editable + anti-duplicado en los DOS modos: UNIQUE (mes, folio) un folio una corrida + los hechos que consulta la guarda (jul-2026 folio 1 OCUPADO con 597 renglones/$33,804,766.23 → se rechaza; ago-2026 folio 1 LIBRE y cero abonos a 212 → ahí entra el mes; y ago SÍ tiene 224 patas por $7.2M en 501/502 desde otras pólizas, o sea la 2ª puerta no es teórica) + ninguna ruta /no-asociados declarada después de @Get(:mes) + el bloqueo dejó de estar condicionado por tipo + invalidar() tira contenido Y hash; skip-graceful sin pólizas', needsApi: false },
  { file: 'test-newdb-libro-compras-txt.js', label: 'LC.9 el TXT que se le entrega a ContPAQi (carga poliza-txt.ts real vía ts-node): largos P=147/M=211 + round-trip byte construir→parsear→construir + la referencia VACÍA sobrevive (rompe un split por espacios) + el parser NO adivina (tipo de movimiento "2" se rechaza en vez de asumir cargo, que invertiría el asiento) + invariante ningún renglón se serializa sin cuenta (padR(null,30) daba 30 espacios y tumbaba el import) + las corridas guardadas re-parsean y cuadran vs su total y vs la póliza real + escáner de ?/¿ dentro del SQL (el 500 de prod del 2026-09-02); skip-graceful sin fixture de ContPAQi', needsApi: false },
  ...
  { file: 'test-newdb-me-context.js', label: 'SN.2 GET /users/me/context (forma del contrato MeContext con position/department objeto o null DECLARADO + 401 sin token + gate estático: declarado antes de :id y sin @RequirePermissions)', needsApi: true },
  ...
  { file: 'test-newdb-rd-route-expenses.js', label: 'RD.4 el GASTO DE FLOTA de Ruta Directa (logistics.route_expenses) — dato PROPIO que no existe en ningun ERP, solo en la hoja CONTROL DE GASTOS RD: si se pierde o se duplica no hay de donde reconstruirlo, y las tres tablas que el proyecto ya tenia para esto estaban VACIAS (fuel_transactions 0, vehicle_usage_logs 0, route_tickets 5). El arbitro NO es el total del Excel: ese esta roto (Z7 suma bloques rotulados con rutas 24/25/300/301 que no existen en los datos y subdeclara el combustible en ~$332,000), asi que el cuadre es contra la COLUMNA CRUDA J3:J2057 — 782 filas, $848,610.04, 34,718.24 lts, al centavo. Candados: la llave natural incluye el IMPORTE porque el folio se repite con montos distintos (medido: folio 38438 con $105 y $859.73) y sin eso el re-import perderia uno · cero duplicados · las 5 filas SIN TIPO siguen marcadas y no adivinadas por su descripcion · el tipo 0 SIN CLASIFICAR existe, porque sin el una fila sin categoria obliga a adivinarla o a perderla · litros solo en COMBUSTIBLES salvo UN caso incoherente del origen que se DECLARA y tolera para que la compuerta atrape el segundo · RLS forzado · y el permiso repartido', needsApi: false },
  { file: 'test-newdb-rd-commissions.js', label: 'RD.6 el MOTOR DE COMISIONES de Ruta Directa — reemplaza las hojas COMISIONES / FORMATO DE PAGO / FORMATO DE SUPERVISOR, que es con lo que hoy se paga a 13 choferes y 3 supervisores cada quincena. Alimentado con el SUBTOTAL/VENTA del propio Excel reproduce 163 de 163 celdas periodo×ruta al centavo en las cinco columnas, asi que la aritmetica no esta en duda y este candado vigila la ESCALA y las CORRECCIONES: cada defecto del Excel que la migracion arregla tiene aca su prueba negativa. Una venta de $400,000 paga 5% (el Excel cerraba con IF(venta<400000) sin rama else y pagaba CERO en la venta mas alta) · $175,000 no paga (el Excel usaba un umbral de 169,999.99 solo en las rutas 22 y 23) · escalones contiguos, sin huecos ni traslapes · la nomina de banco tiene UN valor por zona y no los SEIS que el Excel repartia entre hojas · las rutas 28 y 322 tienen supervisor (se caian de la suma y del factor) · la 505 queda sin chofer DECLARADO y no inventado · los bonos guardan su comparador porque el Excel usa >= en los del chofer y > en los del supervisor · las 7 tablas con RLS forzado · y el permiso REPARTIDO y acotado, no solo declarado en el enum (leccion LC.6.2: es nomina, no un reporte de ventas)', needsApi: false },
  { file: 'test-newdb-wincaja-business-date.js', label: 'RD.1 la FECHA de negocio de Wincaja — RS.12b cambió la expresión de fecha por una función IMMUTABLE para poder indexarla y declaró en su comentario que "business_date NO cambia"; el Postgres de prod corre en Etc/UTC y la vista se evalúa en la DB, así que TODA la venta Wincaja quedó corrida un día (542,684 docs de 2026, 21 sucursales, sin excepción). Nadie lo vio porque era una migración de performance que no midió el antes/después. Candados: la premisa (fecha date-only con offset fijo que no cruza el día UTC — el offset NO es el mismo en prod que en .245) + la expresión no depende del huso de la SESIÓN, que es el defecto de fondo + el ÁRBITRO DE NEGOCIO: las rutas descansan domingo, no sábado (con la fecha corrida daba domingo 60,439 vs sábado 105) + la vista dejó de usar fecha_mx_date + siguen los índices por expresión, porque un arreglo correcto y lento devuelve el Seq Scan de 1.44M filas + la matvista no quedó materializada con la fecha vieja, medida con el MISMO árbitro (comparar montos contra la vista da rojo siempre: la matvista filtra por JOIN products/warehouses y es un universo más chico por construcción)', needsApi: false },
  ...
  { file: 'test-newdb-device-token-ttl.js', label: 'CH.1 el token de una cuenta de DISPOSITIVO vive lo que dice su fila (identity.users.token_ttl_days) y sigue siendo revocable. Carga los .ts REALES via ts-node (token-ttl.ts + permissions-cache.service.ts) y firma con jsonwebtoken reproduciendo el merge de JwtModule, asi que no necesita el API arriba. Las negativas son el punto: (1) la cuenta normal SIGUE en 12h -- si alguien "arreglara" esto subiendo JWT_EXPIRES_IN el caso positivo pasaria igual y los admin tendrian un token de un anio; (2) ningun TTL invalido (0/-5/texto/NaN/undefined) emite un token SIN exp -- `{}` y `{expiresIn: undefined}` se ven igual en el codigo y hacen lo contrario, porque Nest mergea y la clave en undefined BORRA la expiracion; (3) la DB rechaza un TTL de 0, que seria un login que "funciona" y entrega un token ya expirado; (4) desactivar la cuenta la rebota igual con token de 365d (isUserActive real, el que consulta jwt-auth.guard) y apagar un kiosco no toca al vecino -- que es la razon de una cuenta por dispositivo. CH.1.10 agrego la COMPUERTA: `assertDeviceCredential` es pura, asi que se la llama sobre el prototipo del servicio REAL (sin Nest ni API) y se prueban sus 3 permitidos y sus 5 negativas -- la que importa es que la regla se evalua sobre el CAMBIO y no sobre la fila resultante, porque 125 de las 126 cuentas de prod son must_change_password=false con TTL nulo (era el default de la columna antes de [ID.8]) y mirar la fila habria devuelto 400 al editarle el nombre a casi todo el padron. Mas el TECHO del CHECK (antes solo se probaba el piso 0), que el hardcode `must_change_password: true` dejo de ser constante --era la causa de que el alta viviera en un script suelto--, que la compuerta se llama desde el alta Y la edicion, que el script de alta ya no existe, y un CENSO declarado (no un check) de las cuentas que la regla nueva no dejaria crear', needsApi: false },
  { file: 'test-newdb-label-price-notify.js', label: 'TDA.1 un cambio de precio en Kepler LLEGA a la pantalla, y el aviso no puede romper la ingesta. El hop-2 recomputa product_label_prices DENTRO del POST del carril @15s: si el aviso lanza, bloquea o tarda, el dano no es "no llego el aviso" sino que se cae el carril que alimenta precio/costo/margen/reorden de toda la plataforma. Las negativas son el punto: sin configurar, con el API en 500 y con el API COLGADO devuelve false y NO lanza (y se rinde por timeout en <2s, si no bloquearia el carril). Mas: el aviso va DESPUES del COMMIT y sin await; lleva los ids REALMENTE cambiados (el UPSERT es churn-free, con RETURNING) y no los que llegaron; una lista vacia no manda nada; el recorte se DECLARA con truncated en vez de pasar una lista parcial por completa; y el contrato de upsertLabels sigue devolviendo un NUMERO -- el out-param es el 5o y opcional porque su otro llamador es un importer on-prem que corre desde el working tree, donde cambiar el tipo de retorno es un cambio en produccion al guardar el archivo', needsApi: false },
...
(async () => {
  ...
  // throttle desactivado —eso es un segundo paso— pero las DB-direct sólo necesitan un Postgres
  // migrado, que un `services:` de GitHub Actions sí puede dar.
  //
  // No es un modo "rápido" ni un atajo: es la mitad del gate que YA se puede automatizar. La otra
  // mitad sigue siendo local hasta que CI pueda levantar el API, y eso queda declarado, no
  // disimulado — el resumen dice cuántas se omitieron y por qué.
  const soloDb = process.argv.includes('--solo-db');
  const seleccion = soloDb ? TESTS.filter((t) => !t.needsApi) : TESTS;
  if (soloDb) {
    console.log(`--solo-db: ${seleccion.length} suites DB-direct · ${TESTS.length - seleccion.length} HTTP OMITIDAS (necesitan el API arriba).`);
  }

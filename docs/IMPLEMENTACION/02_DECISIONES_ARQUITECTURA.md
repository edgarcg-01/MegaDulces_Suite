...
**Decisión:** Adoptar el patrón **monolito modular + worker-tier + broker de colas**, sin romper el dominio en servicios de red. Se aplica el top-5 en orden: (1) **secretos → bóveda** (Infisical/Doppler), (2) **observabilidad** (nestjs-pino JSON + OpenTelemetry + Grafana/Loki/Prometheus), (3) **worker-tier con `pg-boss`** (cola sobre el propio Postgres, cero Redis nuevo; proceso `WORKER=true` con el mismo código Nx y toggle estilo `ENABLE_MULTITENANT`), (4) **media → bucket propio de Railway** (Tigris, S3-compatible; el `ObjectStorageService` ya lo usa — no Cloudflare R2), (5) **CI extendido** (`nx affected` + `run-all-tests` en el gate). Cada pieza entra por toggle, default OFF, prod intacto hasta validar.
...
## ADR-053 — **Reclamo de recepción** (WMS-REC.8): el faltante tiene responsable, no ajusta stock, y le pega al fill rate que ya existe

**Estado:** Aceptado

**Fecha:** 2026-09-08

**Contexto:** el Andén (`/almacen/anden`) ya distinguía de dónde viene la mercancía —`receiving-origin.ts` clasifica `supplier` vs `transfer` desde el código de "proveedor" de Kepler— pero **el chip era decorativo: ahí terminaba**. Si faltaba media tarima, `discrepancyFor()` marcaba el renglón `faltante`, la pantalla lo pintaba… y al cerrar el vale no quedaba registro de que se reclamó, ni a quién, ni si se resolvió. Peor: el toast decía literalmente *"El proveedor lo va a ver en su scorecard"* cuando **no existía registro alguno**.

Y el fill rate que sí existe (RA.14 / RA-PRO.27, `/compras/proveedores`) se alimentaba **sólo** de `commercial.purchase_orders` (`received_qty/ordered_qty`, `source_type='supplier'`), o sea de las OCs app-nativas. La compra mayoritaria nace en Kepler, así que para casi todo proveedor ese denominador está vacío y el **1.0 significaba "no sé", no "surtió completo"**.

**Decisión:**

1. **El reclamo se levanta al CERRAR el vale, en la misma transacción, y el faltante NO bloquea el cierre.** El cierre es la luz verde que mete la mercancía a inventario; bloquearlo dejaría existencia física invisible — la "mercancía fantasma" que la fase pelea desde WMS-REC.7.2. El 409 que ya existe es por otra cosa (mercancía **retenida** por un rojo, que *no entró*); un faltante nunca llegó, no hay nada que retener. Se levanta **después** de que `pending → faltante` queda firme, y **nunca en la Puerta 1**: el cotejo corre contra el chofer y el andén ocupado es el recurso caro. Si el reclamo no se puede escribir, el cierre falla entero.
2. **Grano = renglón, no vale** (`commercial.receiving_claims`, una fila por SKU, `dedup_key = 'recv-line:'||receiving_line_id`). Se reclama por SKU y el cierre es reintentable sin duplicar.
3. **`dañado` y `producto_incorrecto` también generan reclamo** — y no por completitud: `discrepancyFor()` deja que el override manual **reemplace** a `faltante`, así que marcar un renglón "dañado" borraba la etiqueta del hueco y el reclamo se habría perdido justo en el caso más caro. Como no existe columna `damaged_qty`, esos dos nacen con la cantidad del hueco y, si no hay hueco, con `qty_claimed = NULL`: la cantidad **se teclea en la bandeja**, que es la superficie tranquila.
4. **El traspaso NO se traduce a una sucursal.** `TI###` no mapea limpio (el ERP se contradice: `TI001` con dos nombres, `TI005` como "ZAMORA CANINDO" y como "ABASTOS LP" mientras `analytics.transfer_dest_map` dice que Canindo es `TI006`). El reclamo guarda el **código** y el **nombre que trae el documento** —los dos son hechos— y el dueño concreto sale de un crosswalk **capturado a mano** (`commercial.erp_transfer_origin`, mismo patrón que `erp_sucursal_warehouse`). Mientras esté vacío, el traspaso se **mide sin dueño** y la bandeja lo dice. Mostrar el nombre del documento es un hecho; deducir la sucursal sería una invención.
5. **No ajusta stock ni dinero.** El faltante **nunca entró** al inventario (`close()` da de alta sólo `received_qty`), así que ajustarlo lo contaría dos veces; y la nota de crédito real vive en Kepler (`X-D-55`/`X-D-40`, decodificadas en Fase RE), que es **read-only**. El `amount` es **estimación para priorizar**, etiquetada como tal: `importe ÷ cantidad` del renglón del ERP, **en la unidad del documento** (`qty_unit`) — cero factor de caja inventado (sólo 3.1% de los SKU lo tienen). Sin renglón del ERP el monto es **NULL** con `amount_source='sin_dato'`, nunca `$0`.
6. **Alimenta el fill rate que ya existe; no crea un scorecard paralelo.** Las dos evidencias se **suman al mismo grano** (`ord`/`recv`/`n`) en `purchaseSuggestion` y en `listSuppliers`: OCs app-nativas **+** vales cerrados con su reclamo. No es un peldaño nuevo de precedencia — la fórmula no cambia, sólo tiene más evidencia, y sin reclamos ni vales el número es idéntico al de antes. `fill_evidence` (`po` · `recv` · `po+recv` · `none`) declara la procedencia: un número sin procedencia no se discute con un proveedor.
7. **Un reclamo `discarded` (era error de conteo nuestro) NO penaliza; todo lo demás sí, incluido `open`.** Si sólo contaran los confirmados, desatender la bandeja protegería al proveedor — exactamente el efecto que este item viene a matar. Descartar **exige motivo** (400 sin nota): es lo único que saca al responsable del cumplimiento, así que queda por escrito quién lo dijo.
8. **Bandeja: se reusa el patrón de `commercial.replenishment_findings`** (RA.8) —schema `commercial.*`, UPSERT idempotente, RLS forzado, se trabaja desde Compras— en **tabla propia**, porque el ciclo de vida es **negociado**, no auto-resolutivo: un hallazgo de reorden desaparece cuando la condición se despeja, mientras la condición del reclamo ("faltó media tarima en un vale cerrado") es verdadera para siempre y lo que cambia es el resultado de una negociación con un tercero. Y **sin permiso nuevo**: `COMPRAS_HALLAZGOS_VER/GESTIONAR`, porque el que abre las dos bandejas todos los días es el mismo comprador — eso ahorra el backfill y el re-login.

### Consecuencias

- El andén dice la verdad: el cierre devuelve `claims.raised` + el responsable, y el toast informa *"Se levantaron N reclamos a X"* en vez de prometer un scorecard inexistente.
- **El reclamo hace visible lo que el vale ya decía.** Un vale de 50 renglones cerrado tras contar uno genera 49 reclamos — no es ruido nuevo: esos 49 renglones ya estaban marcados `faltante`, sólo que nadie los veía. Es la señal que faltaba de que un vale se cerró sin contar.
- El fill rate de un proveedor puede **cambiar de valor** al aplicar esto (gana evidencia que antes no tenía) y por lo tanto mover el sugerido de compra. Es el objetivo, y `fill_evidence` lo hace auditable.
- Diferido: cruzar el reclamo aceptado contra la nota de crédito del ERP (`X-D-55`/`X-D-40`) para verificar que el proveedor de verdad acreditó; capturar `damaged_qty` en el renglón del andén.

Hereda **ADR-044** (la app es dueña de la capa que Kepler no codifica, sin write-back) y **ADR-016** (el motor decide, el humano resuelve, el LLM fuera del camino del dinero). Candado: `database/tests/http-receiving-claims-test.js` (**74 aserciones por HTTP**, en la regresión) + `libs/commercial/.../receiving-claim.spec.ts` (Jest, lógica pura).
...
- ✅ **RESUELTO 2026-09-17 — el sistema NO aparta.** Decisión de Edgar. La preventa sigue sin reservar stock. La regla de distribución del §15 deja de aplicarse sobre una promesa y pasa a aplicarse sobre un **hecho**: se reparte lo que el surtidor **efectivamente levantó**, en la desconsolidación, no una reserva previa. Abarata SU.8 (sin expiración, sin liberar al cancelar/reabrir) y evita el peor escenario: apartar sobre `commercial.stock`, que **no es el SoR** (la existencia manda desde `analytics.v_erp_stock_on_hand`). **Costo aceptado:** dos olas pueden pedir lo mismo y la segunda encontrarlo agotado — eso no se previene, se resuelve por excepción (SU.5) + recuperación (Thot/RA). Corolario: la existencia mostrada al armar la ola es **informativa y la pantalla debe decirlo**.

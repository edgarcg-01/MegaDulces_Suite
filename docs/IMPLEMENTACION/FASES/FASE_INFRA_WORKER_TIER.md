...
- 🚀 **INFRA.2.3 EN PROD + VERIFICADO (2026-08-12)** Stack LGTM en Railway (servicio `observability` `d598a881`, imagen `grafana/otel-lgtm`, proyecto balanced-dream). API `MegaDulces` con `OTEL_EXPORTER_OTLP_ENDPOINT=http://observability.railway.internal:4318` + `OTEL_SERVICE_NAME=trade-api` + `LOG_JSON=true` (red privada, sin Tunnel). Grafana público (`observability-production-4823.up.railway.app`), password admin seteado. **Verificado en vivo**: `trade-api` emite → Prometheus (RSS 295MB, heap, CPU, HTTP) + Loki (logs). Alternativa self-host `.249` queda como fallback. **Pendiente menor**: Volume para persistir (hoy efímero); rotar el password de Grafana a Infisical.
...
- 🔨 **INFRA.3.4** 2º servicio Railway `worker` (id `1ba28e68`, balanced-dream) **CREADO + 48 vars cargadas** vía CLI (43 referencias `${{MegaDulces.*}}` + `WORKER=true`+`ENABLE_WORKER_QUEUE=true`+`OTEL_SERVICE_NAME=trade-worker`+`NODE_OPTIONS=--max-old-space-size=1024`). **Falta (tu dashboard, el CLI no tiene auth de GitHub)**: (1) conectar el servicio al repo `edgarcg-01/Trade_marketing` rama main (el MISMO del que build`ea MegaDulces — origin; NO `Megadulces-Logistica`, ése es el remote viejo); (2) Config Path = `railway.worker.json` (corre `node dist/apps/api/main.js`, sin nginx/healthcheck/migraciones); (3) deploy → **ver logs** (esperado: "Worker-tier arriba… crons + cola pg-boss activos"). Recién con el worker SANO: `ENABLE_WORKER_QUEUE=true` en MegaDulces (el gate 3.5 apaga los crons del API). **⚠️ boot no verificado en runtime** (createApplicationContext con 200+ módulos); un crash-loop NO afecta al API (que sigue corriendo los crons hasta que prendas el flag).

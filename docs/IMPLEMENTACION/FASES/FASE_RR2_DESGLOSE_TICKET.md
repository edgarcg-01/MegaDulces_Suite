...
| Item | Qué | Por qué |
|---|---|---|
| **RR3.0** ✅ | **El scope por defecto dejó de ser el año.** `openRoute(row, month?)` abre en el mes clickeado, o en el **último mes con venta** de esa fila; el año completo queda a un click en el chip de periodo. Un solo `detailBounds()` decide el alcance (mes > año explícito > periodo de la vista). | **Medido en prod tras el cambio: 221 ms contra 2,443 ms — 11×.** Era prerrequisito: sin esto, cada faceta nueva heredaba el barrido anual. |
| **RR3.1** ✅ | **El filtro es el estado.** Barra de chips (`Ruta 27 · Ago · SKU 17089 · KG`) con facetas `dSku/dClient/dUnit/dDay/dMonth`; cada una removible, más "Quitar filtros". Las facetas de producto/cliente se **siembran** del filtro de página al abrir y viven dentro del panel. La faceta de unidad acota **todo** el desglose (backend: `opts.unit`), no sólo la lista de tickets — si no, las pestañas contarían una población distinta de la que los chips declaran. | Sin esto "todo clickeable" es un laberinto. |
| **RR3.2** ✅ | **Los drills**: celda de mes de la matriz → ruta acotada a ese mes · Productos → faceta SKU · Unidades → faceta unidad · Por día → faceta día · Clientes → faceta cliente · SKU dentro de un ticket → faceta SKU. Cada uno agrega faceta y salta a Tickets. El **público NO es clickeable**: es el mostrador a bordo, no un cliente. | El grueso del pedido; el endpoint ya aceptaba todos los filtros. |
| **RR3.3** ✅ | **Promedios por renglón**: `lines` en `totals` y `units_per_line` por producto (mismo barrido, cero costo). KPIs "Unid./renglón" y "Renglones/ticket", columna `U/ren` en Tickets y en Productos. | El pedido explícito. |

...
> **CXC.20 ✅ 2026-08-31 — el saldo cuadra con Kepler + doctypes que faltaban + días de pago (mig `20260831140000`, PROD PENDIENTE).** (1) **Saldo inflado $3.72M (+8.1%) corregido:** la página sumaba `saldo_documento` (importe − aplicado kdm5); el saldo verdadero es `Σ signed_amount` de `kdue`. Descomposición exacta del gap: $2.04M que el `greatest(0,…)` recortaba en silencio + $1.56M de abonos sin linaje en kdm5 + $0.11M aplicado fuera de la vista. Fix = `saldo_ajustado`: el total lo manda kdue y el **remanente se reparte FIFO sobre las partidas más viejas**, así `Σ saldo_ajustado = max(Σ signed,0)` exacto y el aging conserva detalle. Residuo declarado, no escondido: 5 clientes / $41,246 donde kdm5 aplicó más de lo que kdue justifica → campo `sin_documento` visible en el drill. (2) **Doctypes que la cartera ignoraba** (censo completo de kdue): C-41 Embarque Sucursal **$605,397** (en la suc 02 el embarque ES el cargo), A-40 Anticipo **−$509,844**, C-9 Ticket Crédito $18,139, A-30/A-35 −$22,571, C-3 $1.16. Siguen fuera por ser contado: C-10 ($89.9M) y C-5 ($2.37M). (3) **Saldo a favor visible**: 149 clientes, **$2,063,836** que la pantalla nunca mostró (KPI + campo por cliente). (4) **Días de pago** (`dias_pago` por documento saldado): 8,659 docs, promedio 14.9 / mediana 2 → columna "Paga a" en la tabla, en el drill y en el resumen. (5) `kdue.c16` **no es referencia, es el estatus** → columna `estatus`; `referencia` queda deprecada con el mismo dato. Va `CREATE OR REPLACE` con las 25 columnas originales en su orden + 4 nuevas al final: un DROP se habría llevado la matview `analytics.mv_customer_receivables` que existe sólo en el `.245` (cluster compartido, de otro dev, no está en el repo ni en prod). Aplicada a `.245`; **falta Railway**.
> **CXC.19 ✅ 2026-08-31 — control "lo embarcado contra lo facturado" (mig `20260831130000`, PROD PENDIENTE):** vista viva `analytics.erp_shipment_billing` (embarque `U-D-41` → factura `U-D-8` por documento padre `c37`/`c39`, 100% de las UD08 nacen de un embarque) + 3 reglas en el scanner CxC (`cxc_embarcado_sin_facturar`, `cxc_factura_duplicada`, `cxc_embarque_descuadrado`) → bandeja Maat. `kdue` dice qué se debe; esto dice **qué debería deberse y no se está cobrando**. **Simulación contra prod: 7 hallazgos por $52,950, cero ruido** (5 embarcado sin cargo $35,663 + 2 facturado dos veces $17,288). **3 falsos positivos que la vista evita, cada uno verificado con datos:** (1) el **descuento NO se resta** — el embarque ya viene con él aplicado (FONSECA 29-ago: emb 163,766.70 vs fac 163,764.63 con `c13`=4,689.71); restarlo inventaba 307 hallazgos por ~$86k; (2) **un embarque → N facturas** (79 casos, hasta 11): se agrega antes de comparar; (3) **`cargo_directo`** — en la suc 02 el embarque YA es el cargo en `kdue` (`c29='C' c4=41`, 1,095 docs), ahí "sin factura" es lo normal → sin esa columna serían 1,094 hallazgos falsos. Tolerancia $1 (el máximo descuadre real medido es $189, redondeo por renglón). Serie 02 = traspaso a sucursal → `no_aplica`. Aplicada a `.245`; **falta Railway**.
> **CXC.18 ✅ 2026-08-31 — facturas pagadas + abrir el documento (mig `20260831120000`, PROD PENDIENTE):** el auxiliar devuelve **todas** las partidas con `saldada`/`pagada_el` y el drill las esconde tras un toggle «Ver N pagadas» (default sigue siendo partidas vivas); la lista trae `n_saldadas` para que `incluir_saldados` deje de pintar filas vacías. La **fecha de la aplicación** venía siempre en '—' porque **`kdm5` no tiene fecha** (verificado en prod: llega hasta `c14`='EMBARCADO'); la mig la trae del abono en `kdue.c7` uniendo por (suc, doc_code del aplicador, folio) sobre el mismo `src` deduplicado → 99.9% de cobertura en prod, saldo idéntico al peso, sólo cambia el contenido del jsonb (firma igual → `CREATE OR REPLACE`). **Clic en la partida → abre el documento** en `/comercial/documentos?doc=<folio_digital>` (deep-link nuevo, abre el side-peek sin depender de la ventana de fechas de la tabla), gateado por `COMMERCIAL_SALES_DOCS_VER`. **Cobertura medida en prod:** UD08 806/806 y UD12 1,604/1,604 resuelven; **UD13 (1,184 partidas, $34.1M = 69% del saldo) NO existe** en `erp_sales_invoices` — su renglón en kdm2 es contable (`VENTAS AL 0 %`), no hay producto que desglosar → la fila muestra un guion con explicación en vez de un botón muerto. Aplicada a `.245` (DDL directo); **falta Railway**.

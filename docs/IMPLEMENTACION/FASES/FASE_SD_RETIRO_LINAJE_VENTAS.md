...
- **SD.4c 🧪 EN CÓDIGO 2026-09-17 — Command Center + reportes SAL: los ~12 lectores de canal migrados al twin, y un BUG de $9.77M/30d corregido.** Las dos "dependencias declaradas" de SD.4 (tickets + canal) **ya estaban resueltas** en el twin al medirlo vivo (SD.4b + SD-CH). Se migraron los ~12 sitios que aún leían `sales_daily` a `mv_sales_blended` (constante `SALES_FACT`, column-safe verificado: sólo `channel/revenue/cost/units/tickets/...`, todas en el twin; el margen se deriva). **Hallazgo de fondo:** `NON_SALE_RAW_CHANNELS = ['mayoreo']` **excluía `mayoreo` del Command Center** con el comentario "mayoreo=TI% traspaso, no es venta" — suposición **refutada por K.3** (mayoreo = Factura Telemarketing U-D-8, venta real; TI001/TI002 no existen en kdud; el telemarketing aterriza en `mayoreo`, no en `credito`). Decisión de Edgar (2026-09-17): **incluir mayoreo**. Se vació `NON_SALE_RAW_CHANNELS` a `[]` (el twin sólo trae ventas reales; los no-venta se excluyen aguas arriba en `mv_kepler`). **Antes/después medido en prod (venta real 30d): $43.87M → $53.64M = +$9.77M (+22.27%)**, casi todo `mayoreo` ($10.16M). Desglose de canal (decisión: taxonomía fina): `credito` $6.40M→$3.59M al desagregar `preventa` $2.39M + `ruta` $0.37M + `contado_nf` $0.13M como cubos propios. `nx build api` verde. **Pendiente: redeploy + validación visual (Command Center + reportes SAL).** `me-zona` queda fuera (frescura-por-canal load-bearing). Con esto, ~4 de ~5 lectores reales migrados.

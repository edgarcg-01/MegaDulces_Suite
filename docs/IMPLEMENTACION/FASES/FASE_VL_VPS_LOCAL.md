...
| **VL.2b** | ✅ **2026-09-11** | ⭐ **EJECUTADO — ventana real de 6 min 34 s** (10:25:53 → 10:32:27), de los cuales **1 min 51 s fue la transferencia** de ~11 GB. Runbook y post-mortem: [`RUNBOOK-VL2b-corte.md`](../../../ops/vl/RUNBOOK-VL2b-corte.md).** Ventana **35–45 min**, de los cuales **~9 son la transferencia** (no 44): **`wincaja` no viaja** — OID 561593, **39.7 de los 50.8 GB** — se queda en `.249` con su carril hasta VL.5, se excluye del `tar` y se dropea en el destino arrancando el cluster aislado. ⚠️ **Sacar datos de `.249` está estrangulado, y no es culpa del plan** (medido, 4 GB por camino): API de Docker **5 MB/s** · volumen por 9P **5** · `tar`+`ssh` en contenedor **16** · **`tar`+`zstd -1`+`ssh` todo DENTRO del contenedor 21** ⭐ (ratio **4.6×**) · lectura sola **69** · Windows→LAN **68**. **Los bytes nunca deben cruzar la frontera Windows↔WSL.** **Mudar la fuente — en la ventana nocturna/fin de semana.** **Copia física del volumen**: `docker stop` → copiar `pgvector-md-data` (55 GB **en tránsito**) → levantar en el servidor nuevo con **la misma major (PG 18)** → **`DROP DATABASE wincaja` allá** (queda residente **~15 GB**; sus 40 GB los sigue usando el carril Wincaja **en `.249`** hasta VL.5 — §6.1). La copia física es lo que preserva `pg_replication_origin`, y por eso las 8 suscripciones **retoman desde su slot sin hueco**; `pg_dump` por base **no** lo preserva. ⚠️ `wincaja` **no** tiene orígenes que preservar (no la alimenta replicación lógica, la escribe el replicador Jet) → dropearla no pierde nada. ⚠️ Mientras la réplica está abajo **los publicadores retienen WAL** → **medir el disco libre de las 8 sucursales el día antes**, no suponerlo (si una está justa, se acorta la ventana o se hace esa rama por separado). Plan B (por rama, si alguna no retoma): `DROP`/`CREATE SUBSCRIPTION` con `copy_data=true` sólo de esa rama. | VL.2c |
| **VL.2c** | ✅ **2026-09-11** | **Desactivar las suscripciones viejas en `.249`.** Tras la copia física **los dos clusters tienen las 8 suscripciones idénticas apuntando a los mismos slots**, y un slot admite **una sola** conexión activa → se pelean (`replication slot is already active`). En `.249`: **`ALTER SUBSCRIPTION <cada una> DISABLE`** — **no `DROP`**: se conservan como rollback y su Postgres sigue arriba sólo por `wincaja`. ⛔ **Si alguna vez se dropean en `.249`, primero `ALTER SUBSCRIPTION … SET (slot_name = NONE)`**, o el `DROP` intenta borrar el slot **del publicador** y le corta la fuente al servidor nuevo. ⚠️ **El rollback a `.249` es limpio SÓLO de inmediato**: en cuanto el servidor nuevo confirma LSNs, los slots avanzan y volver deja un **hueco** — por eso VL.3 se corre antes de soltar la ventana. | VL.3 |
| **VL.3** | ✅ **2026-09-11** | **CERRADO: la pasada posterior al corte dio `huecos 49 · repuestas 49 · errores 0`** — dentro del baseline (12–167; el pico de 218 era el rezago de los 6.5 min y se reabsorbió en una pasada). **Reapuntar y cerrar.** `ODS_SOURCE_BASE` → la fuente local del servidor nuevo. **Candado de completitud:** `reconcile-ods-window` sobre la ventana que cubre el corte. ⚠️ **El criterio NO es "0 filas ausentes"** — eso era un error de este plan, corregido el 2026-09-10 al medir el régimen real: el reconciliador encuentra **48–167 huecos por ventana de 3 días y los repone todos**, de forma continua. Exigir cero es pedir un verde que no existe. **El criterio correcto es comparar contra el baseline medido ANTES del corte**: (a) `huecos` en el mismo rango, (b) `repuestas == huecos` (repone todo lo que encuentra), (c) `errores = 0`, y (d) ninguna clase de hueco nueva. Registrar el baseline es parte de VL.2b, no de VL.3. **Observado el 2026-09-10 en régimen normal, sin tocar nada: `huecos` 167 (disparó el umbral de alerta de 50) · 49 · 48 · 42, y en las cuatro `repuestas == huecos` con `errores 0`.** Ése es el rango contra el que se compara. | VL.4 |
| **VL.4** | ✅ **2026-09-11** | ⭐ **EJECUTADO — 10 de 13 carriles mudados.** La agenda dejó el Programador de Windows y vive en el repo: [`ops/vl/crontab.feeds`](../../../ops/vl/crontab.feeds) (9 carriles en el servicio `feeds-cron`) + `feeds-livefast` como servicio propio (@60 s, sub-minuto: el cron no llega). Cadencias **copiadas**, no "mejoradas": cambiar el cuándo y el dónde en el mismo movimiento hace imposible atribuir una falla. Las 10 tareas de `.249` quedaron **deshabilitadas, no borradas** (rollback). **Verificado a las 12:00 con los 6 carriles disparando juntos**, todos con `host` de `md`: `refresh-consolidado` 5 s · `contpaqi` 20 s · `contpaqi-slow` 59 s · `stock` 99 s · `receipts` 171 s · `live` 226 s. **⛔ Los 3 carriles MUDOS no se mudaron** (`prices`, `fleet-gps`, `store-poller`): medido, no escriben `analytics.cron_runs` → ADR-060 no permite declarar migrado un carril sin latido verde, y mudar un carril mudo es renunciar a saber si funciona. Se quedan en `.249` hasta **VL.4b**. ⚠️ Ironía medida: **el carril mudo de `prices` es justo el del incidente que fundó la Fase OBS** ("seis días publicando precios viejos, uno 54 % bajo costo"). **Las cuatro trampas que sólo aparecieron al correr de verdad, cada una escondiendo a la siguiente** — ninguna la atrapó la verificación previa: (1) **`target-guard.js` se requiere por path computado y sólo dentro del `if (APPLY)`** (`run-prod-feeds.js:349`) → un grep de `require('…')` literal no lo ve **y una prueba en seco no entra a esa rama**: los carriles corrían perfecto sin `--apply` y morían con `MODULE_NOT_FOUND` en cuanto se les ponía; (2) **Alpine no trae `tzdata`**, así que `TZ=America/Mexico_City` **se ignora en silencio** y el contenedor queda en UTC → el `nightly` habría corrido 21:00 MX y los importers calculan ventanas de fecha con hora local; (3) **`crond` lanza desde `/root`** y los pasos hijos usan rutas relativas: el mismo comando daba **2/2 OK por `docker exec`** (CWD `/app`) y **0/2 desde cron** — la prueba manual NO reproduce el entorno de cron, el CWD es parte de ese entorno; (4) **el wrapper disfrazaba fallos de éxito, dos veces**: leía `$?` después de un `if !` (todo fallo salía "código 0", que es imposible) y después trataba `rc=1` como "salteada por lock" cuando 1 es también el código de cualquier comando que falla (busybox `flock` no tiene `-E`) → el lock pasó a tomarse sobre un **descriptor**. Y un **CRLF**: el `feeds.env` generado desde Windows llegó con 25 retornos de carro, y el `\r` al final de `CRON_TENANT_ID` lo volvía un UUID inválido (`invalid input syntax for type uuid`) sobre un valor que a la vista estaba bien. Commit `44e94d0c`. | VL.4b |
| **VL.4b** | ✅ **2026-09-11** | ⭐ **LOS 13 CARRILES FUERA DEL PROGRAMADOR.** Los 3 mudos laten y están en `md`; sus tareas de Windows **deshabilitadas, no borradas**. El latido se cableó **antes** de mudar cada uno — mudar un carril mudo es renunciar a saber si funciona del otro lado, y esta fase lo pagó el mismo día. **`store_poller`**: latido propio + **detector DIRECTO de réplica congelada** sobre `pg_stat_subscription` (las ramas 06/07 se leen de réplica lógica y el poller se conecta a ese mismo cluster, así que le pregunta a la suscripción si sigue recibiendo). ⚠️ El criterio obvio —"hace mucho que no llega un ticket"— es **inservible**: a las 21:00 dispararía con cada tienda que cierra, y una alarma que grita en falso enseña a ignorar el tablero. **`fleet_gps`**: reporta **`error` con 0 objetos**, porque su falla real no es caerse — no hay API oficial (ADR-034) y cuando la sesión expira `fetchObjects()` devuelve `[]` (verificado en el código), el ciclo termina "bien" y no entrega nada; con ~50 devices, cero nunca es normal. **`feed_prices`**: entró como **modo del runner**, no como latido pegado a mano, así hereda timeout por paso y agregación de fallas; con `--source=ods` (el default) los dos importers leen `kepler_ods` **en la misma DB de prod**, sin tocar `.245` → el carril es autocontenido en el servidor. Los tres con **umbral registrado en `CRON_JOBS`**. Prueba negativa en los tres. **⛔ Y aparecieron TRES fallas nuevas, las tres de la misma familia: el sistema reportando éxito sin hacer nada.** (1) **La réplica congelada** — lo que disparó todo: VL.2c dejó las suscripciones de `.249` en `DISABLE` pero **su Postgres sigue arriba**, así que el poller conectaba a una base que existe y dejó de avanzar; **137 min sin un solo error en su log** (medido: `kepler_md_06` 137 tickets del día en `.249` contra 224 en `md`; `md_07` 58 contra 110). Arreglo: `KEPLER_REPLICA_BASE` → `md`; el backfill recuperó 176 tickets solo. ⚠️ **Regresión de VL.2b que la verificación del corte no vio**: se revisaron los carriles del ODS y los feeds, **no los que quedaron en `.249` leyendo la réplica local**. (2) **El CRLF hizo correr los 9 carriles EN SECO diciendo `✓ ok`** — `crontab.feeds` no termina en `.sh`, así que `*.sh eol=lf` no lo cubría; con `core.autocrlf=true` `git archive` lo exportó con CRLF y `argv.includes('--apply')`, que es comparación **exacta**, dejó de matchear `--apply\r`. El latido tampoco se escribía (también cuelga de `APPLY`), así que el tablero conservaba la marca vieja **en vez de ponerse rojo**. Se cazó comparando el log (`contpaqi ok` a las 13:16/17/18) contra su latido clavado en 13:14:58. Ventana ~6 min, sin pérdida (ventanas rodantes). Arreglado en **dos capas**: regla en `.gitattributes` (el origen) y `sed` + **aserción que rompe el build** en el Dockerfile (el borde). (3) **`client.end()` no tiene timeout y colgó el poller** 20 min después de mudarlo: un solo socket, `→192.168.44.44:5432 FIN_WAIT1` con 6 bytes en Send-Q; Yurécuaro se fue sin completar el handshake y el `await` no volvió nunca — ciclo trabado, 05/06/07 sin leer, guard `running` bloqueando todo, contenedor `healthy`. El archivo ya tenía la lección **a medias** (se protege del mismo cuelgue en `fetch` desde 2026-08-04, no en el cierre). Los 3 cierres pasan ahora por `cerrar()`, acotado a 5 s + `destroy()`. ⭐ **Sin el latido puesto una hora antes, este cuelgue habría sido invisible otra vez.** Commits `96681e07` · `1482d56f` · `e231be81` · `3579db8e`. | VL.5 |
...
| **VL.6.1** | ✅ **2026-09-11** | ⭐ **Los 3 contenedores de VL.4 decían `healthy` con un chequeo que no medía nada.** Heredaban el `HEALTHCHECK` de la imagen **sin** `ODS_HB_URL`/`ODS_HB_KEY`, así que `health.js` salía por su primera guarda imprimiendo *"sin ODS_HB_URL/ODS_HB_KEY — no se evalúa"* y `exit 0`: **verde incondicional**. Y ninguno llevaba la etiqueta `autoheal`, o sea que aunque se pusieran `unhealthy` **no había brazo**. Veredicto ausente **y** brazo ausente — los dos pecados que ADR-053 y ADR-056 nombran, reproducidos en los contenedores más nuevos. **No es teórico:** 20 min después de mudar `store-poller` quedó colgado en un `client.end()` contra Yurécuaro (socket en `FIN_WAIT1`), proceso vivo, log mudo, y `docker inspect` diciendo `healthy`; **lo encontró un humano leyendo el latido**. Ahora **el latido ES el healthcheck** y `autoheal` es el brazo. Cada uno pregunta por lo suyo: `feeds-livefast`→`feed_livefast`, `store-poller`→`store_poller`, y `feeds-cron`→ **CANARIO de 3 claves de 1 minuto** (`fleet_gps,feed_contpaqi,feed_receipts`). ⭐ El canario es la parte con criterio: `feeds-cron` no tiene UN carril sino once — preguntar por uno solo lo ata a ese carril (el día que se retire o se renombre, **bucle de reinicio eterno**), y preguntar por TODOS es peor (reiniciaría los once porque la fuente de uno se cayó, y reiniciar **no repone una fila**). Lo que ese contenedor debe probar es que el camino `cron → run-feed.sh → node → prod` **entrega**; que un carril concreto esté mal lo grita `db-health`, que sí mira uno por uno con el umbral de cada cual. `ODS_HB_IGNORE_ERROR=1` en los tres: su `status='error'` es alarma de **dato** (ContPAQi caído, sesión del GPS vencida, un POS que no contesta) y reiniciar no arregla ninguna — lo que sí dispara el reinicio es que el ciclo **deje de completarse**. `health-lane.sh` resuelve el destino del latido, que llega por caminos distintos (`feeds-cron` monta `/secrets/feeds.env` porque `crond` no hereda entorno; los otros dos usan `env_file`), y **si no puede resolverlo sale 1, no 0** — salir 0 diciendo "no se evalúa" es exactamente el falso verde que esto cierra. **Verificado:** 4 pruebas negativas (sin clave · clave inexistente · canario con 3 claves muertas · canario real desde afuera, que da rojo por propiedad **mostrando los carriles vivos**) + las 3 positivas en vivo (`canario ok — 3/3 entregando`, `feed_livefast ok (0.2 min)`, `store_poller ok (0.1 min)`) + **el brazo probado con un contenedor cebo**: autoheal lo detectó y lo reinició 3 veces. De paso quedó medido que autoheal ya venía haciendo su trabajo: curó `ods-live-mirror` una vez hoy. | VL.6.2 |
| **VL.6.2** | ⬜ | **El watchdog a `md`.** Medido: `health-watchdog.js` **ya viaja dentro de `trade-ingest:latest`** y `pg` resuelve → el port es **una línea de crontab**, no un rewrite. Lo que falta es lo que está roto en él: (a) **no late** — es de los pocos artefactos invisibles para `db-health`; (b) **`WATCHDOG_WEBHOOK_URL` nunca estuvo configurada en ningún lado que él lea** (0 ocurrencias, verificado), así que su push externo jamás existió y su único canal efectivo es la campana del API — justo lo que él existe para cubrir cuando el API está caído; (c) su lanzador `.vbs` **se traga el código de salida**. ⚠️ Su lista de 6 jobs **NO está desactualizada**: es corta a propósito (es el dead-man's switch del scanner del API, no la superficie de monitoreo — ésa es `CRON_JOBS`). | VL.6.3 |
| **VL.6.2** | ✅ **2026-09-11** | **El vigilante en `md`, latiendo, y declarando lo que NO puede hacer.** El port fue **una línea de crontab**: `health-watchdog.js` ya viajaba dentro de la imagen y `pg` resolvía (medido) — no hubo que reescribir nada, contra lo que el plan suponía. Lo que sí estaba roto era el watchdog: (a) **no latía** — era el único de su familia invisible para `db-health`, o sea que el proceso cuyo trabajo es notar que el scanner murió podía morirse primero y en silencio; ahora escribe `health_watchdog` con umbral registrado en `CRON_JOBS`; (b) **`WATCHDOG_WEBHOOK_URL` nunca estuvo configurada en ningún lado que él lea** (0 ocurrencias, verificado), así que su push externo jamás existió y su único canal efectivo es la campana del API — **exactamente lo que él existe para cubrir cuando el API está caído**. Eso no se arregla desde el código (hace falta una URL), pero **se dejó de ocultar**: el `note` del latido declara siempre el estado del canal y hoy dice `canal externo: NINGUNO`. ⚠️ Su lista de 6 jobs **NO estaba desactualizada**, como parecía: es corta a propósito — es el dead-man's switch del scanner, no la superficie de monitoreo (ésa es `CRON_JOBS`). ⛔ **Y quedó una lección de la mudanza**: mover el carril sin **deshabilitar la tarea de Windows** dejó DOS procesos escribiendo el mismo `job_key` — un carril, dos dueños, que es justo contra lo que advierte el chequeo de propiedad de `health.js`. Se vio en el `host` del latido y se corrigió. | VL.6.3 |
| **VL.6.R** | ✅ **2026-09-11** | ⭐⭐ **LA PRUEBA QUE LA FASE NO PODÍA SEGUIR POSPONIENDO: `md` se reinició y volvió solo.** El supuesto valía toda la fase —la fase nació de que `.249` no levanta la ingesta sin que alguien inicie sesión— y estaba **sin verificar**: el host había booteado el 10-sep y el stack se creó 3 h después, así que el daemon había arrancado **una sola vez en su vida**. **Preparación, no fe:** antes del reinicio se midió que `docker`/`docker.socket`/`containerd` están `enabled` y que los 8 contenedores tienen `restart: unless-stopped`; se agregó **`stop_grace_period: 60s`** a `pg-ods` (sin él son 10 s y después `SIGKILL`: un Postgres al que se le corta el checkpoint de cierre arranca en **recuperación de caída** y convierte un reinicio de 2 min en uno de veinte); y se **ensayó** recreando sólo Postgres — cierre limpio, listo en 1 s, las 8 suscripciones retomando al instante sin que los carriles se enteraran. **Resultado medido del reinicio real:** los **8 contenedores arriba y `healthy` 39 s después del boot**, sin que nadie los levantara · Postgres `database system is shut down` → `database system was shut down at 22:37:31` → `ready to accept connections` **41 ms** después, o sea **cero recuperación de caída** · las **8 suscripciones activas recibiendo a 0.0–0.2 min** · ventana real de Postgres abajo: **45 s** · y el candado de completitud: `cdc_reconcile` dio **huecos 61 · repuestas 61 · errores 0**, dentro del régimen medido (42–167) y con **todo repuesto** — el `status='error'` es sólo el umbral de alerta de 50, no un hueco sin reponer. ⚠️ Observación que deja la prueba: justo después de un reinicio los healthchecks pasan sobre latidos **previos** al corte (miden antigüedad, y la tolerancia es 10–45 min), así que un contenedor que vuelve pero **no** entrega recién se delata al vencer su tolerancia. Es correcto para un chequeo de entrega, pero conviene saberlo al leer un tablero recién arrancado. | — |
| **VL.6.3** | ⬜ | **El respaldo.** ⭐ **Lo urgente ya se corrigió el 2026-09-11**: la tarea tenía `ExecutionTimeLimit = PT1H` y las corridas duran **74 / 90 / 69 min** (medido), así que PowerShell moría a los 60 min — pero `pg_dump` es otro proceso, sobrevive huérfano y **termina de escribir**, así que el archivo queda con buen tamaño y nadie sospecha. Lo que **no** corría desde el 2026-09-08 —el día exacto en que se escribieron— eran los pasos 5 (`pg_restore --list`), 5b (piso de 400 tablas) y 6 (retención GFS): **la compuerta construida para el incidente de "respaldaba la base equivocada" estaba muerta**, y la retención no podaba. Subido a `PT4H`. Falta: portarlo a `md` (ya tiene **`pg_dump` 18.6, que empata EXACTO con prod**; `.249` tiene 18.4), **darle latido** (hoy no tiene) y agendar la restauración — se ejercitó **una sola vez, a mano, el 2026-09-08**, y el runbook promete en presente un "consumidor diario" que **nadie agenda**. ⚠️ Mudarlo **no lo acelera**: el cuello son ~40 Mbit de salida compartidos, mismo gateway. | VL.7 |
| **VL.6.4** | ✅ **2026-09-11** | ⭐ **Un latido para 53 pasos no es una alarma, es un adorno — y el problema de fondo no es la alarma.** Nace de una pregunta del usuario: *"¿por qué tener 52 nightly? ¿son necesarios?"*. **Medido en `analytics.cron_run_log` de prod:** las cuatro últimas corridas del nocturno reportaron **`ok`** — `47/53`, `52/53`, **`36/52`**, `51/52`. O sea que una noche fallaron **16 de 52 pasos** y el tablero quedó verde, porque `run-prod-feeds.js` sólo marca `error` si falla **el total** (`failed === total`). ⛔ **Lo obvio —invertirlo para que falle con cualquier paso— se rechazó, y el autor ya lo había previsto**: está comentado en el código que la falla parcial queda en `ok` *"sin disparar alarma crítica por ruido"*; con 6 pasos fallando en una noche normal el carril viviría rojo, y una alarma que grita todos los días enseña a ignorar el tablero (ya pasó: 488 alertas, cero reconocidas en cinco semanas). **El defecto real es que un lote de 53 pasos no se puede resumir en un booleano** — y su consecuencia no es que no avise, sino que **un paso que hace meses no escribe una fila se ve idéntico a uno crítico**, y por eso son 53 y no 20: agregar al nocturno es gratis e invisible, sacar exige una evidencia que nadie tenía con qué producir. **Lo entregado:** `cron-heartbeat.stepLog()` escribe **una fila por PASO** reusando `analytics.cron_run_log` (VP.3.3), que ya tenía las columnas y los índices exactos — **no se inventó una segunda bitácora**, que es el pecado que este repo ya se cobró ocho veces; se distinguen por la barra en `job_key` (`feed_nightly` = carril, `feed_nightly/import-margin.js` = paso) y se verificó antes que en prod hay **cero `job_key` con barra** y que la tabla **no tiene un solo lector en producción**. El runner cronometra cada paso y captura la **última línea** de su salida (stdout por tubo, reescrito byte a byte: el log del contenedor no cambia). La vista `analytics.v_feed_step_health` (ventana 30 d) da corridas, fallas, `horas_sin_ok`, p50 y ⭐ **`resumenes_distintos`** — *1 sobre 30 corridas = su propio resumen nunca cambió*, la señal de poda más filosa computable sin tocar los 53 scripts (no es prueba: un paso idempotente también repite). ⚠️ **`rows_affected` va NULL a propósito**: el orquestador corre subprocesos, **no conoce sus filas**, y sacarle un entero al texto con un regex daría números equivocados (*"0 best-sellers — tabla intacta"*); un número equivocado en una columna que se llama «filas» es peor que un hueco (ADR-056). Lo que hay es la **cita textual** del importer. ⚠️ **Una fila ausente NO es un paso sano**: el padrón vive en `run-prod-feeds.js`, no en la BD. **Fuera, con criterio medible:** los carriles sub-minuto (`livefast`/`receipts`/`contpaqi`) tienen 1-2 pasos —su latido ya es por paso— y costarían **7,200 filas/día** contra las **~940** de todos los demás juntos. **Candado `test-newdb-feed-step-health.js` 15/15 contra prod**, con **prueba negativa**: con la BD caída la bitácora avisa y **no lanza**, ni en la descarga a mitad de lote ni en la final. ⚠️ De paso, **quinta vez** que un acento grave dentro de un template literal rompe el archivo. | VL.6.3 |
| **VL.7** | ⬜ | **Apagar `.249` como servidor** (menos lo de VL.5). Deshabilitar las tareas migradas — **no borrarlas**: quedan deshabilitadas 2 semanas como rollback —, retirar el residuo `PM2 Resurrect ODS`, y **declarar en el repo** qué corre en el servidor nuevo. ✅ **[`ops/README.md`](../../../ops/README.md) existe desde el 2026-09-11** y sus afirmaciones estructurales se verificaron contra `md` ese mismo día (8 contenedores `healthy` con los nombres que lista · secretos en `600` · 12 líneas de crontab · sin repo git allá · `pgvector-md` arranca en el mismo minuto que el boot). ⛔ **Lo que la verificación SÍ encontró y sigue abierto:** el criterio de `cdc_reconcile` que el README enunciaba (`repuestas == huecos`) **no es el que usa el código** —`huecos > ODS_RECONCILE_ALERT` (default **50**) ∥ `errores > 0` ∥ `sobrantes > ODS_SOBRANTES_ALERT`— y con el baseline medido en **42–167** esa alarma vive en rojo (17:14: `error · huecos 191 · repuestas 191 · errores 0`). Además `sobrantes` nace con el umbral en `0` = **apagado**, y había **16,157 filas de más** (el síntoma del `DELETE` que no se propaga). Subir el umbral o aceptar el rojo es **decisión de Edgar**, no se tocó. Las 3 tareas Wincaja siguen habilitadas hasta que cierre VL.5. | VL.8 |
...
| **VL.10** | ✅ **2026-09-11** | **Lectura de desarrollo sobre las réplicas de `md`.** Se abre el cluster nuevo (`kepler_md_00..07` + `kepler_consolidado`, 11 GB) y **no prod**: es dato derivado y reconstruible, está en la LAN, y leerlo no toca ni producción ni los POS. **Un rol POR PERSONA** (`edgar`, `david`, `francisco`, `sistemas`), miembros de un grupo `dev_ro` NOLOGIN que carga los permisos — con rol compartido, `pg_stat_activity` dice `dev_ro` y no se sabe quién corre la consulta que se come el servidor, ni se puede revocar a uno solo sin cambiarle la credencial al equipo (pasó con `app_runtime`, GOTCHAS §24). **⛔ Lo que NO se otorga, y es el hallazgo que cambió el diseño:** `kepler_consolidado` tiene **30 tablas FORÁNEAS** (postgres_fdw `srv_md00..05`) que apuntan a **los POS de las sucursales**, y las vistas `dic.*` leen de ellas — otorgarlas no sería dar lectura, sería que cada `SELECT` de un dev **golpee la máquina donde se cobra** (y ni funcionaría: el único *user mapping* es el de `postgres`). El dato equivalente está local y más fresco en las réplicas. ⭐ **La exclusión se calcula POR PROPIEDAD, NO POR NOMBRE**: un `WITH RECURSIVE` marca las foráneas y todo lo que derive de ellas; una foránea nueva, o una vista que la lea, queda excluida sola. **Guardas en el rol nominal, no en el grupo** — `ALTER ROLE … SET` **no se hereda por membresía** y ponerlo en el grupo no da error, simplemente no hace nada: `default_transaction_read_only=on`, `statement_timeout` 120 s, `idle_in_transaction_session_timeout` 60 s, `lock_timeout` 5 s, `CONNECTION LIMIT 5`. No es paranoia: **estas réplicas son la fuente VIVA del pipeline que publica la venta**, y la replicación lógica replica `TRUNCATE` (ACCESS EXCLUSIVE), así que un `SELECT` largo de un dev **bloquea al worker de apply** y frena la suscripción de esa sucursal — lo que acota eso es `statement_timeout`, no `lock_timeout`. ⭐ **La compuerta probó lo único que había que probar:** todos los rechazos llegaban como *"read-only transaction"*, o sea del cinturón — que es un GUC de sesión que cualquiera apaga con un `SET`. Con el cinturón **apagado** el rechazo pasa a ser **`permission denied for table kdii`**: el permiso aguanta solo. ⚠️ Y el primer intento de probarlo se auto-engañó (un solo `-c "SET …; DELETE …"` va en **una** transacción implícita, cuyo modo lectura se fija al abrirla → fallaba por el cinturón sin haber probado nada; hacen falta `-c` separados). **4 personas × 19/19 verificado.** Scripts: [`dev-ro-setup.sh`](../../../ops/vl/dev-ro-setup.sh) · [`dev-ro-verify.sh`](../../../ops/vl/dev-ro-verify.sh) · [`sql/`](../../../ops/vl/sql/). ⚠️ **Abierto:** el `pg_hba` del cluster es `host all all all scram-sha-256` — o sea que **`postgres` (superusuario) es alcanzable desde cualquier IP con contraseña**. Es previo a esta fase y no se tocó (cambiarlo en caliente puede cortar la ingesta), pero ahora que hay más gente conectándose conviene acotarlo a la LAN + la red de Docker. | — |

### VL.2b/2c ✅ EJECUTADO 2026-09-11 — ventana real de 6 min 34 s

Edgar pidió *"necesito que el ODS corra ya"* y se hizo en horario hábil, sin esperar la ventana
nocturna. El costo declarado antes de tocar nada: **rezago, no pérdida** — los slots retienen WAL y el
reconciliador barre 3 días; esta misma semana hubo 9.5 h de rezago por Windows Update sin que nadie lo
notara, así que 40 min con dos personas mirando era más seguro que a las 2 AM.

| | |
|---|---|
| Ventana | **6 min 34 s** (estimada 35–45) |
| Transferencia | **1 min 51 s** para ~11 GB (`wincaja` excluida: 39.7 de los 50.8 GB) |
| Bases que llegaron | las 9, con **tamaños idénticos** al origen |
| `DROP DATABASE wincaja` | OK, con sus archivos ausentes y el cluster aislado |
| Suscripciones | las **8 retomaron desde su slot**, sin `copy_data` |
| `.249` | **0 de 8 habilitadas** (`DISABLE`, conservadas como rollback) |
| Latidos en prod | los 3 con **host nuevo** y **8/8 ramas** (`bac958e87e06` · `a8449b7271c4` · `d74999b67919`) |

⚠️ **Un error propio dentro de la ventana, y estaba escrito en mi propio runbook.** Excluí `pg_wal`
diciendo que *"el cluster lo regenera"*. **Es falso.** El arranque murió con
`invalid checkpoint record · PANIC: could not locate a valid checkpoint record at FD/15850760`: aun
tras un apagado **limpio**, Postgres necesita el segmento WAL que contiene el checkpoint de cierre
para localizarlo. Se arregló mandándolo aparte — 256 MB en **6 segundos** — pero costó un reintento.
Runbook corregido.

⚠️ **El ruido de las 7 tablas de `md_00` viajó con la copia**, como el runbook advertía: `ods_repl` no
tiene SELECT sobre 7 tablas de RH/nómina propiedad de `sa`, reintenta en bucle y ocupa 7 de 10 slots
en el POS. **No bloquea** pero sigue pendiente el grant (§1.1 del runbook).

...
| **VL.4b** | ⬜ | **Ponerle latido a los 3 carriles mudos y mudarlos.** `prices` (@30 min, `import-label-data.js` + `repoint-catalog-prices.js --sync`), `fleet-gps` (@1 min, `database/scripts/fleet-poll-onprem.js`), `store-poller` (@25 s, `live-tickets-poller.js`). El mecanismo **ya existe** y lo usan los otros 10 (`database/importers/lib/cron-heartbeat.js`); lo que falta es cablearlo + registrar el umbral en `CRON_JOBS`. ⚠️ **Editar un importer es deploy a prod YA** ([[feedback_editing_importer_deploys_to_prod_instantly]]) → un carril a la vez, con su prueba negativa. El servicio `store-poller` ya está **definido en el compose y NO arrancado**, a propósito. | VL.6 |

...
| Riesgo | Mitigación |
|---|---|
| ~~**Dependencia dura de Fase ID**~~ — **medido y descartado para este flujo** (2026-08-27, `audit-entradas-access.js`): los 29 usuarios con permiso de entradas tienen alcance con ≥1 sucursal, porque son justo los que tienen `warehouse_code` o `user_scopes=all`. Sigue valiendo para otros dominios que migren a `ScopeService`. | El empty-state explicativo de la vista A queda igual: cuesta poco y cubre al usuario nuevo que entre sin sucursal. |
| ⚠️ **3 sucursales sin capturista propio**, con **862 de las 1,096 entradas**: `00` CEDIS (815), `06` (31), `04` (16). Sólo 3 usuarios son capturistas puros. La pantalla existe y no tiene dueño. | Es asignación, no código: alguien de CEDIS necesita `COMPRAS_ENTRADAS_GESTIONAR` + alcance sobre `00` antes de anunciar el proceso. Lo mide el script del 13.6. |

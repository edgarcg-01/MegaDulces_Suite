...
| **FIQ.6** apartado con TTL | ✅ **HECHO** (local) | `commercial.stock_reservations`/`_lines`/`reservation_sequences` (mig 20260727140000, RLS forzado). `StockReservationService.apartar/activeByPhone/release` (reusa `OrderStockService.reserve/release` con guard anti-freeze + `resolvePriceForQty`). Folio `AP-YYYY-NNNNN`. Cron `@Cron('30 */5')` (admin knex, keyed por tenant) libera vencidos y devuelve stock. Tools `apartar_pedido`/`consultar_apartado`/`cancelar_apartado`. Apartar vacía el carrito (no doble-reserva). Smoke `test-newdb-fiq6-reservations` 15/15. Build api OK. Deferred: conversión apartado→orden (humano). |
| **FIQ.7** trust-score + gate | ✅ **HECHO** (local) | `commercial.contact_trust_features` (feature store por E.164) + `commercial.trust_thresholds` (config por tenant, seed MD) — mig 20260727150000, RLS forzado. `ContactTrustEngineService` (CERO LLM): agrega señales reales (pedidos/cancelaciones/entregas fallidas=no-show/llamadas/conversaciones-sin-pedido="solo juega"/apartados vencidos/deuda) → tier `neutral/allow/require_deposit/block` con guard `min_observations` (cold-start neutro). Gate en `confirmar_pedido`: `block`→handoff, `require_deposit`→pide anticipo/transferencia (no cobro online, ADR-037), el LLM NUNCA acusa. Cron nocturno `@Cron('0 30 8')` multi-tenant refresca. Smoke `test-newdb-fiq7-contact-trust` 16/16. Build api OK. **Deferred: emisión a `commercial_findings` (bandeja ops) — evita conflicto resolve-stale cross-source; el feature store ya es queryable.** |
| **FIQ.1** cerebro + safety | ✅ **HECHO** (local) | Opt-out (BAJA/STOP) YA se procesaba antes del orquestador (WhatsAppOptinService). Nuevo: **throttle/budget-guard** (`WHATSAPP_DAILY_TURN_CAP` def 50 turnos/24h por teléfono → handoff + template, sin LLM — anti-DoS/costo canal público), **`whatsapp.bot_chat_log`** (audit por turno: modelo/tools/iteraciones/latencia + CHECK feedback±1) — mig 20260727160000, RLS forzado. **Model tiering** Haiku→Sonnet (`pickModel` heurístico: mensaje largo/hilo extenso/ambigüedad/mayoreo/factura → Sonnet; `WHATSAPP_SONNET_MODEL` def claude-sonnet-5). Smoke `test-newdb-fiq1-brain` 12/12. Build api OK. **Deferred: few-shot por similitud (embeddings), captura 👍/👎 (columna lista), extended thinking Sonnet.** |
| **FIQ.8** análisis de mercado | ✅ **HECHO** (local) | Tools customer-facing `top_productos` (más pedidos, 365d) + `tendencias_mercado` (30d, "de temporada") sobre `analytics.product_sales_stats` (demanda REAL agregada), **tenant_id explícito** (analytics.* sin RLS — must-fix), enriquecidos con precio+existencia (el bot los ofrece y agrega). Customer-safe: rank + etiqueta, SIN revenue/unidades. Sin migración (reúso). Smoke `test-newdb-fiq8-market` 10/10. Build api OK. **Deferred: MV `analytics.product_seasonality` (necesita 12m+), tool por zona (contacto sin zona).** |
| **FIQ.5** geolocalización (core) | ✅ **HECHO** (local) | Captura de ubicación (pin de Meta): `InboundMessage.location`, el adapter extrae lat/lng, el ingest acepta `type=location` y lo pasa al orquestador, que mete las coords en `delivery_address` (`{lat,lng,street}`) — el geofence de última milla (`parseCoords`) ya las lee. Prompt guía a confirmar + pedir calle/referencia. Sin migración (reúso). Smoke `test-newdb-fiq5-location` 11/11 (contrato pin→adapter→orquestador→geofence). Build api OK. **Deferred: ETA (`route_eta_min`) + geocode de texto → GEO_PORT/Mapbox (`MAPBOX_TOKEN`); auto-asignación de repartidor; STT de notas de voz (ADR).** |
| **FIQ.10** outbound reorden | ✅ **HECHO** (local, envío gated) | Motor de reorden NBA: puerto `listDueForReorder` (atrasados vs cadencia `customer_360` + producto habitual `recommended_baskets`, tenant explícito, solo contactables), `WhatsAppReorderService` (compose determinista + filtro opt-in + cooldown anti-spam `whatsapp.reorder_nudges` mig 20260727170000 RLS + createTargeted+send gated) + endpoints `/whatsapp/reorder/preview|run`. Smoke `test-newdb-fiq10-reorder` 6/6. Build OK. **⚠️ Dependencia de datos (no bug): en local solo 2/2935 clientes tienen `cadence_days` (resto 'new' sin historial de pedidos app) y ~4 contactables → el motor rinde ~0 hasta que se acumule historial de pedidos O `customer_360` derive cadencia del sell-out ERP (analytics.*). El ENVÍO exige plantilla Meta aprobada (`WHATSAPP_REORDER_TEMPLATE`); sin ella queda `skipped:no_template`. No auto-envía por cron (disparo admin explícito).** Deferred: atribución oferta→pedido, cadencia desde ERP, Haiku rewrite del mensaje. |
| **FIQ.11** cierre | ✅ **HECHO** (local) | ADRs **035-039** en `02_DECISIONES_ARQUITECTURA.md` (bot avanzado / identidad E.164 / trust-score / apartado / geoloc). Entry en `CHANGELOG.md`. 6 smokes nuevos integrados a `run-all-tests` (fiq1/5/6/7/8/10). Regresión completa exit 0 (DB-direct verdes; 19 rojos = `needsApi` sin API). |
| FIQ.9 visión | ⬜ TODO (gated) | Necesita descarga de media de Meta (token Graph vivo) + imagen real para validar. Único sprint pendiente. |

**Despliegue:** los commits FIQ.0/2/3/4 **YA están en `origin/main`** (auto-push del entorno). Railway aplica las migs (20260727120000/120500/130000) al boot en el redeploy. **⚠️ FIQ.3 (precio por CANTIDAD/tiers) toca `OrdersService.addLine/replaceLines` → cambia el pricing de TODOS los canales (portal/vendedor/televenta/whatsapp), no solo el bot.** Pendiente: setear `WHATSAPP_PRICE_LIST_CODE=MAYOREO`, corregir `WHATSAPP_APP_SECRET`/`PHONE_NUMBER_ID` de Meta, y validación runtime en vivo (FIQ.2/3/4 no tienen smoke DB del flujo del bot, solo del motor).

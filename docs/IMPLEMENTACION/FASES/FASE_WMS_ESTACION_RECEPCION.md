...
>
> **[WMS-REC.8] 🧪 2026-09-08 — reclamo diferenciado de faltantes: el chip de origen deja de ser decorativo (ADR-053).**
> - **El agujero que cierra.** El Andén ya distinguía proveedor / CEDIS / traspaso (`receiving-origin.ts`), pero **ahí terminaba**: `discrepancyFor()` marcaba el renglón `faltante`, la pantalla lo pintaba, y al cerrar el vale **no quedaba registro de que se reclamó, ni a quién, ni si se resolvió**. El toast incluso decía *"El proveedor lo va a ver en su scorecard"* cuando **no existía registro alguno**.
> - **Cuándo se levanta: al CERRAR, en la MISMA transacción**, y **después** de las dos sentencias que pasan `pending → faltante` — ese es el instante exacto en que el faltante queda firme. Y **no en la Puerta 1**: el cotejo corre contra el chofer y el andén ocupado es el recurso caro; esto **no agrega un solo toque** al camión esperando. Si el reclamo no se puede escribir, el cierre falla entero.
> - **El faltante NO bloquea el cierre** (decisión de Edgar, 2026-09-08). El cierre es la luz verde que mete la mercancía a inventario; bloquearlo dejaría existencia física invisible — la mercancía fantasma que WMS-REC.7.2 vino a matar. El 409 que ya existe es por otra cosa: mercancía **retenida** por un rojo, que *no entró*. Un faltante nunca llegó: no hay nada que retener.
> - **Grano = renglón, no vale** (`commercial.receiving_claims`, mig `20260908150000`): se reclama por SKU. `dedup_key = 'recv-line:'||receiving_line_id` con UNIQUE `(tenant, dedup_key)` → un reintento del cierre no duplica reclamos, y el `DO NOTHING` protege el trabajo humano ya hecho.
> - **`dañado` y `producto_incorrecto` también generan reclamo, y no por completitud:** `discrepancyFor()` deja que el override manual **REEMPLACE** a `faltante`, así que marcar un renglón "dañado" borraba la etiqueta del hueco y el reclamo se habría perdido **justo en el caso más caro**. Como no existe columna `damaged_qty`, nacen con la cantidad del hueco y, si no hay hueco, con `qty_claimed = NULL` → la cantidad se teclea **en la bandeja**, que es la superficie tranquila.
> - **⛔ El traspaso NO se traduce a una sucursal.** `TI###` no mapea limpio (`TI001` con dos nombres; `TI005` como "ZAMORA CANINDO" y como "ABASTOS LP" mientras `analytics.transfer_dest_map` dice que Canindo es `TI006`). El reclamo guarda **código + nombre del documento** —los dos son hechos— y el dueño concreto sale de un crosswalk **capturado a mano**, `commercial.erp_transfer_origin` (mig `20260908150100`, mismo patrón que `erp_sucursal_warehouse`). Vacío = el traspaso se **mide sin dueño** y la bandeja lo dice, con el botón para capturarlo donde duele. Capturar un código **reasigna de una vez todos sus reclamos huérfanos**: primero duele, después se configura.
> - **No ajusta stock ni dinero.** El faltante **nunca entró** (`close()` da de alta sólo `received_qty`), así que ajustarlo lo contaría dos veces; y la nota de crédito real vive en Kepler (`X-D-55`/`X-D-40`), read-only. El monto es **estimación para priorizar**: `importe ÷ cantidad` del renglón del ERP, **en la unidad del documento** (`qty_unit`: PZA/PAQ/CJA) — **cero factor de caja**, que falta en 97 de cada 100 SKU. Sin renglón del ERP el monto es **NULL** con `amount_source='sin_dato'`: un `$0` se leería como "no cuesta nada". Verificado en `postgres_platform` que `importe/cantidad == costo_unitario` exacto en las 51 líneas disponibles; **falta comprobarlo a escala en `platform_test`**.
> - **Le pega al fill rate que YA existe** (RA.14 / RA-PRO.27), sin scorecard paralelo: las dos evidencias se **suman al mismo grano** `ord`/`recv`/`n` —OCs app-nativas **+** vales cerrados con su reclamo— en `purchaseSuggestion` y en `listSuppliers`. La fórmula no cambia, sólo tiene más evidencia; sin reclamos ni vales el número es **idéntico** al de antes. `fill_evidence` (`po`·`recv`·`po+recv`·`none`) declara la procedencia, porque un número sin procedencia no se discute con un proveedor. **Por qué hacía falta: el denominador viejo era `commercial.purchase_orders` (sólo OCs app-nativas) — 0 filas en la DB local, y la compra mayoritaria nace en Kepler, así que el 1.0 significaba "no sé", no "surtió completo".**
> - **`discarded` no penaliza y exige motivo** (400 sin nota): es lo único que saca al responsable del cumplimiento, así que queda por escrito quién lo dijo. **`open` sí penaliza** — si sólo contaran los confirmados, desatender la bandeja protegería al proveedor, que es el efecto que este item viene a matar.
> - **Bandeja: se reusa el patrón de `commercial.replenishment_findings`** (RA.8: schema `commercial.*`, UPSERT idempotente, RLS forzado, se trabaja desde Compras) en **tabla propia**, porque el ciclo de vida es **negociado**, no auto-resolutivo: un hallazgo de reorden desaparece al despejarse la condición, y la del reclamo ("faltó media tarima en un vale cerrado") es verdadera para siempre. **Sin permiso nuevo:** `COMPRAS_HALLAZGOS_VER/GESTIONAR` — el que abre las dos bandejas todos los días es el mismo comprador → **sin backfill y sin re-login**.
> - **Frontend** `/compras/reclamos` (Operations): `MetricStrip` de header (abiertos · monto · antigüedad · proveedores · traspasos sin dueño), tabla densa con la columna Vale congelada, `app-side-peek` con el detalle y las acciones, `app-load-state` para la matriz de estados (vacío-por-filtro ≠ vacío real ≠ error de red), filtros en la URL. En `/compras/proveedores`: badge de procedencia del fill rate + columna **Reclamos** con link filtrado. Y el toast del Andén dice el hecho (*"Se levantaron N reclamos a X"*) en vez de una promesa.
> - **2 defectos PROPIOS que la verificación encontró** (y por eso se verifica): (1) la FK compuesta con `ON DELETE SET NULL` **pelado** intentaba anular también `tenant_id` (NOT NULL) → borrar un proveedor con reclamos reventaba con *"null value in column tenant_id"*; ahora es `SET NULL (columna)` (PG 15+). (2) tres estilos usaban **alias de token congelados en claro** (`--border`, `--surface-card`, y `--text-body`, que **no existe**) → la columna sticky salía **blanca** en modo oscuro. Medido en runtime y documentado como [`GOTCHAS §37`](../../GOTCHAS.md).
> - **El reclamo hace VISIBLE lo que el vale ya decía.** Un vale de 50 renglones cerrado tras contar uno genera 49 reclamos: no es ruido nuevo —esos renglones ya estaban en `faltante`— es la señal que faltaba de que un vale se cerró sin contar.
> - **Verificado:** smoke HTTP `http-receiving-claims-test` **74/74** (2 corridas, idempotente, registrado en `run-all-tests`) · Jest `receiving-claim.spec.ts` (**2 suites / 30 tests**) · vecinos verdes: sesión **18/18**, auditor **17/17**, lote-línea **38/38**, luz-verde **26/26**, RA service-level **18/18**, RA network **7/7** · `nx build api` y `nx build view --skip-nx-cache` verdes · **validación visual real en navegador: claro + oscuro + móvil 390 px, sin errores de consola propios.** (`test-newdb-replenishment` 13/14 y `test-newdb-purchase-chain` fallan por **falta de data** en la DB local — reproducido idéntico con el cambio stasheado.)
> - **Pendiente prod:** 2 migraciones a Railway (`20260908150000_receiving_claims`, `20260908150100_erp_transfer_origin`) + **redeploy api+view**. **Sin re-login** (no hay permiso nuevo). Ya en prod hay que **capturar el crosswalk `TI###`→almacén**, que arranca vacío a propósito.
> - **Diferido:** cruzar el reclamo aceptado contra la nota de crédito del ERP (`X-D-55`/`X-D-40`) para verificar que el proveedor acreditó de verdad · `damaged_qty` en el renglón del andén · tab de Reclamos dentro de Almacén (necesita decisión de permiso) · el fix global del `h1` en oscuro (GOTCHAS §37, decide Edgar).
...
>
> **[WMS-REC.7.1] 🧪 2026-09-01 — rediseño del Andén: tres secciones hermanas y UNA barra por sección.** Reemplaza el andén de 2 puertas encadenadas por **3 secciones** (Llegada · Caducidad · Ubicación) sobre el mismo control segmentado iOS, y sustituye el par "campo de escaneo + buscador" por **una sola caja de entrada**.
> - **Caducidad y Ubicación son colas HERMANAS, no pasos.** Encadenarlas obligaba a que quien fecha también acomode; en la bodega real las trabaja gente distinta en momentos distintos. Se salta entre las tres sin perder el vale: la sección **no vive en la ruta** (el back del navegador rompería el flujo a media captura), es estado de pantalla.
> - **La barra única.** Un solo input por sección hace las dos cosas: recibe el disparo de la pistola **y** filtra la lista mientras se teclea. **Dos inputs compitiendo por el foco es exactamente lo que rompe una pistola en modo wedge** — el disparo se va al elemento equivocado y el operario tiene que tocar la pantalla con guantes antes de cada lectura. Con una sola: si el código deja **una sola coincidencia, Enter abre ese renglón** (apuntar y disparar es el gesto completo); si hay varias no se adivina, se deja el filtro puesto. El mismo Enter sirve para los dos casos, así que **el operario no tiene que saber en qué modo está**.
> - **El foco vuelve solo** tras cada disparo, cada guardado y cada cierre de panel (`refocoTick`). Sin eso no hay ráfaga y la pistola deja de servir a la segunda caja.
> - **Filtrado sin acentos ni mayúsculas** (`filtro.util.ts`, puro y testeado): nadie teclea "Mazapán" con tilde en un handheld con guantes. Busca por nombre, SKU y código; **en Ubicación también por rack** — teclear `R04` deja a la vista todo lo que va a ese pasillo y el bodeguero camina una vez en vez de cuatro. Varias palabras acotan (AND), no amplían.
> - **Contador `N de M` y vacío accionable.** "Sin resultados" a secas deja al bodeguero parado con el producto en la mano: el vacío dice qué hacer (*"si el producto llegó igual, no lo fuerces contra otro renglón: hay que agregarlo como renglón nuevo"*), y distingue vacío-por-filtro de vacío-real.
> - **Cámara como respaldo, con fallback explícito.** ZXing con los 5 formatos de retail (mismo patrón que Conteo) + `navigator.vibrate(80)` para confirmar sin mirar la pantalla. Si `getUserMedia` no existe —http de LAN, contexto no seguro— **el botón dice por qué no abre** en vez de quedarse mudo.
> - **`--tap-min` corregido de raíz:** el token sí está definido (`tokens.css:494` = `0px`, `:504` = `44px` bajo `pointer: coarse`); lo que no alcanzaba era el **selector** de `styles.css:2877-2879`, que sólo matchea `.comm-actions button`, `.comm-actions .p-button` y `[class*="icon-btn"]`. Los botones de la barra llevan `icon-btn` **a propósito** para caer en esa regla; el input fija `52px` propios porque el andén también corre en equipos que se declaran de puntero fino.
> - **Componentes renombrados a las secciones** (`anden-llegada`, `anden-caducidad`, `anden-ubicacion`, `anden-segmented`, `scan-field`): el nombre dice qué sección es, no qué acción hace. `anden-draft.service.ts` **conserva el sufijo `.service`** — es un servicio inyectable y renombrarlo lo sacaría de la convención del resto del repo.
> - **Trampa del repo, ahora en GOTCHAS:** un backtick dentro de un comentario de `template`/`styles` rompe la compilación con `FatalDiagnosticError 1010` **sin nombrar el archivo**. Pisada dos veces en la misma sesión.
> - **Verificado:** `nx build view` verde · `nx test view` **5 suites, 52 tests**. **Falta validación visual y con pistola real**, y revisar ambos temas en claro/oscuro.
> - **Bloqueantes que siguen abiertos (este cambio no los tapa):** `uxc` ausente en **97 % de los SKU**, y `GET /policy/resolve` sin exponer para la vida útil mínima real.

> **[WMS-REC.7.2] 🧪 2026-09-01 — `evaluate()` no compensaba: existencia fantasma con el vale aparentando estar completo.** La captura hace commit **antes** de escribir el stock. Si el alta de stock fallaba, quedaba una fila `accepted` **sin movimiento**: `declared_qty` la cuenta (filtra por `status='accepted'`), el renglón se veía fechado, y la mercancía nunca entró. `authorize()` ya compensaba desde WMS-REC.4; **`evaluate()` era el camino que faltaba — y es el que usa el 100 % de las capturas verdes y amarillas.** Ahora el fallo marca la captura `rejected` (único estado del CHECK que la saca del declarado sin romper el append-only: conserva evidencia y foto, deja de contar, y el renglón vuelve a la cola de Caducidad), loguea el motivo y **re-lanza** — no se traga la falla.
> - **Códigos de tracker provisionales** (`WMS-REC.7.1` / `7.2`): falta que Edgar confirme la numeración.

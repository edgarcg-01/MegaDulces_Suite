...
| **CBW.6** | Cobranza dura — atribución al cliente | **Hallazgo (2026-08-13):** los ~80 remitentes NO son encargados internos sino los **clientes de telemarketing** (grupo Kepler `1M001 – TELEMARKETING LA PIEDAD`: dulcerías/abarrotes/personas) que **pagan por depósito** y mandan su ficha. El teléfono los identifica → se **atribuye el depósito al cliente**. Mig `20260813120000`: `customer_code` (clave Kepler, join a `analytics.erp_collections.cliente_code`) + `rfc` en `bank_capture_senders` y `bank_capture_inbox`. `resolveSender`/`capture` denormalizan la identidad; la bandeja muestra cliente+RFC. Importer extendido: parsea la lista de telemarketing (clave/nombre/tel/tel2), **expande Tel2** (2º número del mismo cliente), **excluye internos** (`TI*`/`ONLINE`), dedup por teléfono, **enriquece RFC** desde `analytics.erp_customers` por clave. CSV de los ~99 clientes generado (local, PII, no-commit). Smoke +2 aserciones. Builds api+view OK. **Diferido CBW.7:** match del depósito al cobro específico (UA0501) reusando el matcher de Fase CC. | ✅ 2026-08-13 (local, DB local caída → verificado por build) |

...
5. **QA visual**: `/compras/descuentos` (KPIs + toggle Duplicados) + `/compras/entradas` → abrir una entrada de un proveedor grande (Mondelez/Canel) → sección **"¿Por qué no cuadra?"** lista sus X-D-40/55 + `/finanzas/hallazgos` filtro regla `compra_factura_duplicada`. (Reconciliación de descuento = endpoint listo; UI en `/compras/descuentos` diferida.)

...
3. **Redeploy** api + view (push de los commits locales `3e49be9a`·`93c6e55c`·`beb1b3ae`·`c15b64f6`·`df420698`·`0cb4666c`·`e1dae914`·`6d386556` + docs). El bridge de duplicadas→hallazgos **no lleva migración** (reusa `finance.findings`; la regla se auto-registra).
...
5. **QA visual**: `/compras/descuentos` (KPIs + toggle Duplicados) + `/compras/entradas` → abrir una entrada de un proveedor grande (Mondelez/Canel) → sección **"¿Por qué no cuadra?"** lista sus X-D-40/55 + `/finanzas/hallazgos` filtro regla `compra_factura_duplicada`.

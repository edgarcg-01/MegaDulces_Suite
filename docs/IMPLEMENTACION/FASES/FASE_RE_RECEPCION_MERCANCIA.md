...
2. **Importers** (`DATABASE_URL_NEW=<prod>`): (a) desde LAN (Railway no alcanza Kepler): `import-purchase-adjustments.js` dry-run → 1,286/$20.9M → `--apply` (`ADJ_SRC` md_00); `import-supplier-payments.js --apply` (backfill `c84`: ~1,742 pagos/$12.6M). (b) **computado (corre en cualquier lado, lee solo la newdb)**: `import-supplier-discount-policy.js --apply` (147 políticas de tasa observada) — DESPUÉS de (a) porque deriva de los pagos.

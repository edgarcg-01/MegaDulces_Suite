...
## 38. Correr un importer A MANO mientras la tarea programada existe: te lo matan a los 13 minutos

**Vivido el 2026-09-08 (K.3).** Backfill de `analytics.sales_daily` con la ventana completa
(13 meses) contra prod. La corrida murió sola después de ~13 minutos con:

```
FATAL 57P01  terminating connection due to administrator command   (routine: ProcessInterrupts)
Emitted 'error' event on Client instance  ->  el proceso node se cayó, la transacción hizo ROLLBACK
```

No fue Railway ni la red. Fue **nuestro propio barrido de huérfanos**:
`database/importers/kepler/kill-stale-feeds.ps1`, que `run-prod-feeds.js` invoca al arrancar cada
modo, mata **todo `node.exe` cuyo command line contenga el nombre de uno de los scripts del modo**
y lleve más de `MaxAgeMin` (= `STEP_TIMEOUT_MIN` + 3 = **13 min**) vivo:

```powershell
Get-CimInstance Win32_Process | Where-Object {
  $_.Name -eq 'node.exe' -and $_.ProcessId -ne $SelfPid -and $_.CommandLine -match $re
} | ...  if ($age -gt $MaxAgeMin) { Stop-Process -Id $_.ProcessId -Force }
```

El barrido no distingue "corrida manual legítima de un humano" de "huérfano colgado del 5-ago". Sólo
mira el **nombre del script** y la **edad**. Y el modo `live` corre cada 15-30 min, así que la
ventana para que te barran es permanente.

**Lo engañoso es el síntoma.** El error que se imprime es un `FATAL` de Postgres, así que la primera
hipótesis fue "Railway cortó la conexión" o "hay un timeout del lado del server" — y eso manda a
tocar timeouts, SSL o tamaños de batch. Nada de eso. Y peor: si canalizás la salida por `tail`, no
ves los 12 minutos de progreso, sólo el stack final (misma trampa que §37).

**Qué hacer.** Un backfill contra prod de un importer que **también** corre por tarea programada:

1. **Medí una tajada primero.** `SALES_FACT_DAYS=30` tardó 64 s; de ahí se extrapola.
2. **Subí en escalera** (30 → 90 → 180 → 260 días) para que ninguna corrida cruce los 13 min. El
   UPSERT es churn-free e idempotente, así que repetir la banda ya cargada no escribe nada: cada
   escalón sólo paga la lectura y el staging.
3. **Un barrido es un ROLLBACK, no un daño.** La transacción es atómica: si te barren, reintentás.
   Lo que NO hay que hacer es apagar la tarea programada para "hacer lugar" — eso deja la venta
   viva de prod sin actualizar y no es una decisión de quien está haciendo el backfill.

⚠️ **Y el corolario incómodo:** durante el backfill, prod queda en un estado **mixto** — la tarea
`live` refresca los últimos 2 días con la lógica nueva mientras la historia todavía tiene la vieja.
Se ve clarísimo si medís: el canal `mayoreo` apareció en prod con **$490,494 (2 días)** cuando la
verdad de 90 días era **$14,580,181**. No es un error del cambio; es el backfill a medio camino. No
reportes esa foto como resultado.

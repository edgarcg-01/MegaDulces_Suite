...
import { ComprasService, SupplierLedgerResponse } from '../compras.service';
...
@Component({
  ...
  imports: [
    ...
    TableModule, SelectModule, DatePickerModule, SkeletonModule, MetricStripComponent,
  ],
  template: `
      ...
      @if (loading()) {
        ...
        <p-table [value]="d.rows" [loading]="false" styleClass="p-datatable-sm surf-table surf-table--sticky cq-table"
          ...
          <ng-template #body let-r>
            <tr>
              <td class="cq-prov" [title]="r.proveedor">{{ r.proveedor || '—' }}</td>

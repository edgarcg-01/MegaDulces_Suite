          ...
          <div class="ca-msg" [class.ca-user]="m.role==='user'" [class.ca-ai]="m.role==='assistant'">
            <div class="ca-bubble" [class.ca-err]="m.error">
              ...
              @else { <div class="ca-text">{{ m.content }}</div>
                @if (m.download) { <p-button type="button" icon="pi pi-file-excel" [label]="'Descargar ' + m.download.folio + ' (Excel)'" styleClass="p-button-sm ca-dl" [loading]="downloading()===m.download.id" (click)="download(m.download!)"></p-button> }

...
/**
 * CC (extensión) — "Comprobantes de Orden de Entrada". Lista las órdenes de entrada
 * de Kepler (documento X-A-40) y le adjunta a cada una la REMISIÓN/FACTURA del
 * proveedor (imagen/PDF): el capturista elige la entrada, sube la remisión, corre
 * OCR (Claude vision), el sistema compara el total OCR vs el valor de la entrada
 * (chip de cuadre) y guarda la evidencia. Validación/rechazo a nivel gestión.
 * No escribe a Kepler.
 */
@Component({
  selector: 'app-finanzas-entradas',
  standalone: true,
...
export class FinanzasEntradasComponent {
  ...
  readonly canManage = computed(() => this.auth.user()?.permissions?.[Permission.FINANCE_RECEIPTS_GESTIONAR] === true);

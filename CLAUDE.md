...
| **LT** | **Rastreo de flota GPS (MagniTracking)** | **🟢 CONSTRUIDA (beta) local 2026-07-27 — ADR-034. Trae las posiciones reales de la flota (magnitracking.net = GPS-Server.net v4, cuenta 614, ~50 devices Ruptela+Streamax) al dominio Logística. **Sin API oficial** → adapter replica el login de sesión + poll `fn_objects` (reverse-engineered). `libs/logistics/logistics-tracking`: `FleetProviderPort` + `MagniTrackingAdapter` (TZ MX -06:00) + `LogisticsTrackingService` (sync/live/history/link + auto-match placa) + `FleetPollerService` (@Cron 1min, solo si env creds) + controller (`LOGISTICS_FLEET_VER/GESTIONAR`). Migs `logistics.trackers` (RLS, última posición denorm) + `logistics.vehicle_positions` (sin RLS, dedupe, patrón route_location_pings). Frontend `/logistica/rastreo` (mapa por estado + KPIs + recorrido histórico + link). **LT.5 bootstrap** (`POST /tracking/bootstrap-vehicles` + botón "Vincular por placa"): crea vehículos desde el nombre del GPS y vincula → auto-match **2→48/50**. **LT.6 alertas server-side** (`logistics.fleet_alerts` + `FleetAlertsScannerService` @Cron 5min sin-señal 90min–24h/vel>90 + `GET /alerts`+`ack`; página lee alertas persistidas ackeables). Build api+view OK; smoke `test-newdb-logistics-tracking` 13/13. **Creds por env `MAGNI_USER/PASS` (rotar el compartido). Pendiente prod: 3 migs Railway (180000/181000/182000) + env + redeploy.** Diferido: push de alertas por WS al campo (`FLEET_NOTIFIER_PORT`). Plan en [`FASE_LT`](docs/IMPLEMENTACION/FASES/FASE_LT_RASTREO_FLOTA.md).** |
...
| **CP** | **Conector ContPAQi (SoR contable externo)** | **🔨 EN CURSO — CP.0–CP.4 ✅ local 2026-07-27 (ADR-040). Tesis: integrar, NO construir — ContPAQi = system of record contable/fiscal, la plataforma = engagement (lee del SoR, agrega Maat+CB, jamás escribe directo). Conexión decodificada: SQL Server 2022 instancia `COMPAC` @ `192.168.0.35` (server SERVCONTABILIDA), 1 empresa Contabilidad `ctLUIS_FRANCISCO_LOPEZ_GUTIERREZ` (persona física, el mismo de la Fase CB), driver `mssql` con `instanceName`, login RO `platform_ro`/`superoot` (`00-create-readonly-login.sql`). **CP.0** decode (hallazgo: contabilidad casi no segmenta por sucursal ~2% → verdad fiscal CONSOLIDADA, el detalle por sucursal se queda en Kepler). **CP.1** `analytics.contpaqi_ledger_monthly` (56,821 filas, cuadre $24.88B; filtra `Afectable=1`) + tool `maat_contpaqi_balanza`. **CP.2** `analytics.contpaqi_bank_movements` (147,952; Egresos/Cheques muertos desde 2019 → ledger vivo = movs de póliza en 102xxx) + crosswalk `finance.bank_accounts.contpaqi_cuenta` (16/18) + endpoints link/compare + **tab UI "vs ContPAQi"** en `/finanzas/bancos` + tool `maat_contpaqi_banco` (auxiliar por banco). Validado vs workbook CB (ene-2026 mismo universo). **CP.3** `analytics.contpaqi_suppliers` (3,411, 99.6% RFC) × `fiscal.sat_list_rfcs` = 109 en listas, 6 EFOS (69B); tool `maat_contpaqi_efos` + detector persistente `contpaqi_proveedor_efos`. **CP.4** tool `maat_libros_vs_operacion` (gap fiscal-vs-operación ~$12M/mes estructural). 3 staging tables + 4 tools Maat + 1 detector + 5 smokes (50 aserciones) verde. **Pendiente prod:** 4 migs newdb a Railway + 3 importers en máquina de feeds (`CONTPAQI_SQL_PASSWORD`) + redeploy api+view + `contpaqi/link`. Diferidos: CP.5 push pólizas (necesita spec formato import), materialidad CFDI↔póliza, CP.6 puerto, CP.7 SDK. Plan en [`FASE_CP`](docs/IMPLEMENTACION/FASES/FASE_CP_CONTPAQI.md).** |
| **CC** | **Comprobantes de Cobranza (ficha de depósito + OCR)** | **🔨 EN CURSO — CC.0–CC.2 ✅ local + PROD migraciones 2026-08-03. Adjunta el comprobante de **depósito** (imagen/PDF) a un **cobro de Kepler**, con **OCR**, ligado por `(sucursal, folio)`. Evidencia **read-only** sobre el ERP (no crea ni escribe cobros); captura en oficina. **Decode:** cobro = documento `Collect1`/serie `UA0501` = `kdmm` `U-A-5-1` "Cobro PUE" (C 102 Bancos / A 115 Clientes); filtro `kdm1 c2='U' AND c3='A' AND c4=5` (los otros abonos nat A son devoluciones/notas de crédito→403). Fuente CEDIS `md_00` (23,771 cobros/$369.5M; 5,227 con ficha). **CC.0** mig 20260803120000: `analytics.erp_collections` (espejo) + `finance.collection_deposits` (adjunto+OCR+`monto_match`+HITL, RLS) + importer `import-collections.js` (U-A-5, UPSERT aditiva) + OCR `LlmExtractorService.extractDepositSlip()` (Claude Haiku vision, imagen **y PDF nativo**). **CC.1** backend `libs/finance/collection-deposits` (`/finance/collections`: listCobros⋈evidencia+KPIs, uploadFile, runOcr preview, attach cuadre tolerancia $1, validar/rechazar) + permisos propios `FINANCE_COLLECTIONS_VER/GESTIONAR` (backfill mig 20260803120100 ancla a Bancos) + smoke `test-newdb-collection-deposits` 18/18. **CC.2** frontend `/finanzas/cobranza` (Operations: tabla cobros + chip cuadre + panel adjuntar OCR + validar/rechazar). Builds api+view OK. **PROD:** 5 migs aplicadas a Railway (batch 154; incluía 3 ajenas aditivas aprobadas). **Pendiente prod:** redeploy código api+view (commits locales sin push) + correr importer desde LAN (`DATABASE_URL_NEW=<prod>`, Railway no alcanza CEDIS) + re-login + rotar credencial Railway expuesta. **CC ext ✅ local 2026-08-03 — Pagos a proveedor + Órdenes de entrada** (dos módulos dedicados que calcan Cobranza). Decode en vivo: pago-a-proveedor = **`c31`=forma/tipo** → **`XD2601`** transferencia (16k/$338M) + **`XD2501`** cheque (623/$42.5M) + **`XD6001`** anticipo (147/$11.8M), filtrado a `c10 LIKE 'C%'` (proveedor de compra; excluye nómina GN/caja chica GG/banco GB). NO son pago (excluidos): XD5501=nota crédito/devolución, XD4001=devolución, XD1001=solicitud. **Corregido en 2 iteraciones** (comprobantes BBVA reales de CONVERMEX): (1) solo leía XD2501=cheques=4%; (2) faltaba XD6001=anticipos → el anticipo CONVERMEX $150,621.50 "no aparecía". Ver [[reference_kepler_supplier_payment_methods]]. `doc_prefix` en la PK (folio compartido entre doctypes). **4,010 pagos/$346M** (3,688 transferencia/194 cheque/128 anticipo). Orden de entrada = **`XA2001`** "Aplica Orden Entrada" (el doc que se firma/adjunta; `c16`=total c/IVA=remisión; póliza 511/201/122) — **corregido de X-A-40**: el folio NO es único entre doctypes (ver [[reference_kepler_orden_entrada_xa2001]]); enriquecido con su XA4001→vale XA3701; 8,373 entradas/$427M + 41,101 líneas de detalle. Mig `20260803130000` (`analytics.erp_supplier_payments` + `analytics.erp_goods_receipts` + `finance.supplier_payment_proofs` + `finance.goods_receipt_proofs` RLS) + perms `20260803130100` (`FINANCE_PAYMENTS_*` ancla a Bancos). Importers `import-supplier-payments.js`/`import-goods-receipts.js`. OCR: pagos reusan `extractDepositSlip`; entradas nuevo `extractRemision()` (cuadre por total **o** subtotal). **Pagos → Finanzas** (`/finanzas/pagos-comprobantes`, `FINANCE_PAYMENTS_*`); **Órdenes de entrada → proyecto Compras** (`/compras/entradas`, gateado `COMPRAS_VER/GESTIONAR` — es tema de compras, no finanzas). Backend `/finance/supplier-payments` + `/finance/goods-receipts` (este re-gateado a `COMPRAS_*`). Smoke `test-newdb-supplier-receipt-proofs` 42/42. **CC ext.2 ✅ PROD 2026-08-04 — pago multi-método**: mig `20260804120000` (metodo_pago + doc_prefix en PK) batch 157 en Railway + importer (25/26/60) repoblado prod (**4,010 pagos**: 3,688 transferencia/194 cheque/128 anticipo) + push `cb78df92`→redeploy. UI columna Método (Transferencia/Cheque/Anticipo); default limit 300→500. Smoke 44/44. **Pendiente prod: re-login (perms `FINANCE_PAYMENTS_*` en JWT).** Diferido: cron importer, conciliación cobro↔banco(CB)↔abono Kepler, captura desde campo/WhatsApp, grupo 7 Cobro CFDI. Plan en [`FASE_CC`](docs/IMPLEMENTACION/FASES/FASE_CC_COMPROBANTES_COBRANZA.md).** |
...
- **ADR-034** ✅ 2026-07-27 — **Rastreo de flota**: el proveedor GPS (MagniTracking/GPS-Server.net) vive detrás de `FleetProviderPort`; como **no hay API oficial**, el adapter replica el login de sesión + poll de la web. Cambiar de proveedor = cambiar solo el adapter. Hereda ADR-016. Plan en [`FASE_LT`](docs/IMPLEMENTACION/FASES/FASE_LT_RASTREO_FLOTA.md).
...
- **ADR-040** ✅ 2026-07-27 — **Conector ContPAQi** (Fase CP): integrar, NO construir. ContPAQi (SQL Server, instancia COMPAC @ .35) = SoR contable/fiscal; la plataforma = engagement (lee balanza/bancos/proveedores → Maat+CB; jamás escribe directo; push por import de archivo HITL diferido). Hereda ADR-016/028. Plan en [`FASE_CP`](docs/IMPLEMENTACION/FASES/FASE_CP_CONTPAQI.md).

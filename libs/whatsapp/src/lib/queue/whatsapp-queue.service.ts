...
export class WhatsAppQueueService implements OnModuleInit, OnModuleDestroy {
  ...
  private connection: any = null; // IORedis (solo modo Redis)

  get mode(): 'bullmq' | 'in-process' {
    return this.redisUrl ? 'bullmq' : 'in-process';
  }

  async onModuleInit(): Promise<void> {
    if (!this.redisUrl) {
      this.logger.log('REDIS_URL ausente — cola WhatsApp en modo in-process (dev/single-instance).');
      return;
    }
    try {
